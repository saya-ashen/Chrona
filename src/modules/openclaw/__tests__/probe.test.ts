...
import type { OpenClawRuntimeClient } from "@/modules/openclaw/runtime-client";
import { evaluateOpenClawGate } from "@/modules/openclaw/evaluate-gate";
import {
  collectOpenClawGateChecks,
  OPENCLAW_PROBE_PROMPT,
  renderOpenClawGateMarkdown,
} from "@/modules/openclaw/probe";

function createRuntimeClient(): OpenClawRuntimeClient {
  return {
    connect: vi.fn().mockResolvedValue({
      protocol: 1,
      methods: ["chat", "chat/stream", "health"],
    }),
    close: vi.fn(),
