...
function ClientForm({
  ...
  onSave: (data: { name: string; type: "openclaw" | "llm"; config: Record<string, unknown>; isDefault: boolean }) => void;
  onCancel: () => void;
}) {
  ...
  const [bridgeUrl, setBridgeUrl] = useState(
    (initial?.config as { bridgeUrl?: string })?.bridgeUrl ?? "http://localhost:7677",
  );
  const [timeoutSeconds, setTimeoutSeconds] = useState(
    String((initial?.config as { timeoutSeconds?: number })?.timeoutSeconds ?? 120),
  );
...
export default function AiClientsPage() {
  ...
  const handleCreate = async (data: { name: string; type: "openclaw" | "llm"; config: Record<string, unknown>; isDefault: boolean }) => {
  ...
  const handleUpdate = async (id: string, data: { name: string; type: "openclaw" | "llm"; config: Record<string, unknown>; isDefault: boolean }) => {
                ...
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <input
                      ...
                      onChange={(e) => handleToggleEnabled(client.id, e.target.checked)}
                    />
                    启用
                  </label>
                  <button
