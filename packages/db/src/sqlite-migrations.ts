...
function isPreviousReleaseFixture(
	value: unknown,
): value is MigrationReleaseMetadata["previousReleaseFixture"] {
	return (
		isRecord(value) &&
...
function isReleasedMigrationChecksums(
	value: unknown,
): value is Record<string, string> {
	return (
		isRecord(value) &&
		Object.values(value).every((checksum) => typeof checksum === "string")
	);
}
...
function releasedMigrations(
	...
	for (const migration of released) {
		const recordedChecksum =
			metadata.releasedMigrationChecksums[migration.name];
		if (
			!SHA256.test(recordedChecksum) ||
			recordedChecksum !== migration.checksum
		) {
			throw new Error(
				`Released migration checksum mismatch for ${migration.name}`,
			);
		}
	}
	return released;
...
function verifyPreviousReleaseFixture(
	...
	try {
		...
		if (
			...
			released.some(
				(migration) =>
					applied.get(migration.name)?.checksum !== migration.checksum,
			)
		) {
			throw new Error(
				`Previous-release fixture migration history mismatch: ${fixturePath}`,
			);
		}
	} finally {
		fixture.close();
...
function baselineReleasedSchema(
	...
	for (const migration of migrations.slice(0, end + 1))
		baselineMigration(db, migration);
}
...
export function ensureSqliteDatabase(
	...
	if (!sqlitePath)
		throw new Error(
			`Chrona requires a SQLite file URL, got: ${options.databaseUrl}`,
		);
