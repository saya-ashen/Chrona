...
export type HermesGatewayRestartResult = {
  ok: boolean;
...
const pluginFileNames = ["__init__.py", "tools.py", "plugin.yaml", "README.md", "smoke_test.py"] as const;
...
export function restartHermesGateway(): HermesGatewayRestartResult {
  try {
    const subprocess = Bun.spawn(["hermes", "gateway", "restart"], {
      stderr: "ignore",
      stdin: "ignore",
      stdout: "ignore",
    });
    ...
    return {
      ok: false,
      exitCode: null,
      message: cause instanceof Error ? cause.message : "Hermes gateway restart command failed.",
    };
  }
}
