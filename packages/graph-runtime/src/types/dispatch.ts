// Leaf dispatch types shared between graph-runtime command shapes and
// execution-side state. Kept here (under types/) so that commands/types.ts
// can import them without forming a cycle with execution/types.ts.
...
export type GraphExecutionCallbacks<TContext = unknown> = {
  ...
  onEvent?(event: import("../execution/types").GraphExecutionEvent): Promise<void> | void;
