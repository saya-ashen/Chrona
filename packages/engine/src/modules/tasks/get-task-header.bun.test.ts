...
describe("resolveHeaderExecutionState", () => {
	it.each(["blocked", "failed"] as const)(
		"does not expose Stop or Start for %s executions",
		(executionStatus) => {
			...
			expect(
				headerExecutionStateToStatePaths(state)["/execution/can-stop"],
			).toBe(false);
			expect(
				headerExecutionStateToStatePaths(state)["/execution/can-start"],
			).toBe(false);
		},
	);
	...
	it("keeps Stop available for active cancellable executions", () => {
		...
		expect(headerExecutionStateToStatePaths(state)["/execution/can-stop"]).toBe(
			true,
		);
	});
});
...
describe("resolveTaskHeaderViewModel — header status follows the selected occurrence", () => {
	...
	it("labels completed executions as result review instead of no-action completion", () => {
		...
		expect(headerView.statusLabel).toBe("Execution complete, awaiting review");
		expect(headerView.workspaceStateGuidance).toBe(
