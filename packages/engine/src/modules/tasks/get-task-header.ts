...
type HeaderTaskView = NonNullable<
	Awaited<ReturnType<typeof loadHeaderTaskView>>
>;
...
function taskStatusLabel(input: {
	...
	if (
		input.status === "approval-needed" ||
		input.status === "blocked" ||
		input.status === "cancelled"
	)
		return workState.label;
	if (input.status === "completed") {
		return input.taskStatus === "Done"
			? workState.label
...
export function taskHeaderStatus(input: {
	...
	if (input.taskStatus === "Cancelled" || input.executionStatus === "cancelled")
		return "cancelled";
	...
	if (input.executionStatus === "blocked" || input.executionStatus === "failed")
		return "blocked";
	if (input.hasActiveExecution && input.taskStatus === "Blocked")
		return "blocked";
	if (
...
function pickHeaderWorkBlock(
	workBlocks: HeaderWorkBlock[],
	selectedWorkBlockId: string | null,
	now: Date,
) {
	if (selectedWorkBlockId) {
		const selected = workBlocks.find(
			(block) => block.id === selectedWorkBlockId,
		);
	...
	return (
		...
		workBlocks.find(
			(block) => block.scheduledStartAt.getTime() > now.getTime(),
		) ??
		workBlocks[0]
	);
}
...
function occurrenceLabel(input: {
	title: string;
	status: string;
	start: Date | null;
	end: Date | null;
...
export function resolveHeaderExecutionState(input: {
	...
	const blockedOrFailed =
		input.executionStatus === "blocked" || input.executionStatus === "failed";
	const stoppable =
		running ||
		input.executionStatus === "waiting_for_user" ||
		input.executionStatus === "waiting_for_approval";
 ...
 */
export function headerExecutionStateToStatePaths(
	state: HeaderExecutionState,
...
function headerActions(input: {
	executionStatus: string;
	hasPlan: boolean;
	hasAcceptedPlan: boolean;
	isRunnable: boolean;
}): TaskHeaderActionInput[] {
	if (!input.hasPlan) return [{ id: "generate-plan", label: "Generate plan" }];
	if (!input.hasAcceptedPlan)
		return [{ id: "accept-plan", label: "Accept plan" }];
	if (
		input.executionStatus === "completed" ||
		input.executionStatus === "cancelled"
	)
	...
	if (
		["waiting_for_user", "waiting_for_approval"].includes(input.executionStatus)
	)
	...
	return [
		{
			id: "start",
			label: "Start",
			disabled: !input.isRunnable,
			disabledReason: input.isRunnable ? undefined : "Task is not runnable.",
		},
	];
}
export type BuildHeaderSpecInput = {
	task: HeaderTaskView;
	recurrenceSeriesTasks: Array<
		Pick<HeaderTaskView, "id" | "title" | "status" | "workBlocks">
	>;
 ...
 */
export function resolveTaskHeaderViewModel(
	input: BuildHeaderSpecInput & { now?: Date },
): TaskHeaderSpecInput {
	const {
		task,
	...
	const currentWorkBlock = pickHeaderWorkBlock(
		task.workBlocks,
		workBlockId,
		now,
	);
			...
			return occurrence.workBlocks.map((workBlock) => ({
				option: {
					...
					label: occurrenceLabel({
						...
						start: workBlock.scheduledStartAt,
						end: workBlock.scheduledEndAt,
					}),
					taskId: occurrence.id,
	...
	const occurrenceValueCurrent = occurrenceValue(
		task.id,
		currentWorkBlock?.id ?? null,
	);
	const totalSteps = savedPlan?.effectivePlan.nodes.length ?? 0;
	const completedSteps =
		savedPlan?.effectivePlan.completedNodeIds.length ??
		currentExecution.executedNodeIds.length;
	...
	const occurrenceWindow = formatOccurrenceWindow(
		currentWorkBlock?.scheduledStartAt ??
			task.projection?.scheduledStartAt ??
			task.dueAt ??
			null,
	...
	if (hasAcceptedPlan && scopedTaskStatus !== "Done")
		actions.push({ id: "restart", label: "Run plan from beginning" });
	if (task.goalId)
		actions.push({ id: "rebuild", label: "Rebuild with latest Goal assets" });
	actions.push(
		{ id: "edit", label: "Edit" },
	...
	return {
		...
		progressLabel:
			totalSteps > 0 ? `${completedSteps}/${totalSteps} steps` : "0 steps",
		priorityLabel: task.priority,
		priorityTone: priorityTone(task.priority),
		occurrenceLabel: occurrenceWindow
 ...
 */
export async function getTaskHeaderSpec(input: {
	taskId: string;
	workBlockId?: string | null;
}) {
	...
	const [currentExecution, savedPlan, recurrenceSeriesTasks] =
		await Promise.all([
			getCurrentExecution({
				taskId: input.taskId,
