...
import {
	...
	savePlanRun,
} from "@/modules/plan-execution/persistence/plan-run-store";
...
describe("getTaskPage orchestrator read model", () => {
	...
	it("returns projected block detail for task page and bootstrap", async () => {
		...
		expect(bootstrap.task.blockReason).toMatchObject({
			detail: "ACP connection closed",
		});
	});
	...
	it("returns the plan scoped to the selected recurring occurrence", async () => {
		...
		const firstPlan = {
			...makeCompiledPlan(),
			editablePlanId: "first-occurrence-plan",
			title: "First occurrence plan",
		};
		const secondPlan = {
			...makeCompiledPlan(),
			editablePlanId: "second-occurrence-plan",
			title: "Second occurrence plan",
		};
		await saveCompiledPlan({
			workspaceId: workspace.id,
			taskId: task.id,
		...
		const firstPage = await getTaskPage({
			taskId: task.id,
			workBlockId: firstBlock.id,
		});
		const secondPage = await getTaskPage({
			taskId: task.id,
			workBlockId: secondBlock.id,
		});
		...
		expect(firstPage.task.savedPlan?.id).not.toBe(
			secondPage.task.savedPlan?.id,
		);
	});
	...
	it("keeps bootstrap current work block and saved plan on the same in-window occurrence", async () => {
		const { workspace, task } = await seedTask(
			"Recurring bootstrap scoped plan task",
		);
		const now = new Date();
		...
		const scopedPlan = {
			...makeCompiledPlan(),
			editablePlanId: "current-occurrence-draft",
			title: "Current occurrence draft",
		};
	...
	it("returns recurrence series occurrences for workspace switching", async () => {
		...
		await db.task.update({
			where: { id: task.id },
			data: { recurrenceRule: "FREQ=DAILY", seriesExternalUid: task.id },
		});
		...
		expect(
			page.task.recurrenceOccurrences.map(
				(occurrence) => occurrence.workBlockId,
			),
		).toEqual(expect.arrayContaining([firstBlock.id, nextBlock.id]));
		expect(
			page.task.recurrenceOccurrences.find(
				(occurrence) => occurrence.workBlockId === firstBlock.id,
			)?.isCurrent,
		).toBe(true);
	});
	...
	it("returns one coherent execution summary from the effective plan graph", async () => {
		...
		expect(page.task.graphNodeStates).toContainEqual(
			expect.objectContaining({
				id: "answer",
	...
	it("keeps an accepted Done task authoritative over its completed work block", async () => {
		...
		await db.task.update({
			...
			data: {
				status: "Done",
				completedAt: new Date("2026-07-15T03:00:00.000Z"),
			},
		});

		const page = await getTaskBootstrap({
			taskId: task.id,
	...
	it("does not fabricate a block reason from a projection with no block fields", async () => {
		const { workspace, task } = await seedTask(
			"Completed occurrence without blocker",
		);
		const compiledPlan = makeCompiledPlan();
		await saveCompiledPlan({
			workspaceId: workspace.id,
	...
	it("maps persisted provider events into structured activity items", async () => {
			...
			data: [
				...
				},
				{
					...
					payload: {
						...
						provider: "anthropic",
						runId: "run-1",
						event: { type: "provider_opaque", rawEventType: "provider.opaque" },
					},
		...
		for (const sensitiveValue of ["native-run-1", "run-1", "provider.opaque"]) {
			...
			data: [
				{
					...
					payload: {
						...
						event: {
							...
							callId: "call-1",
							label: "Read source",
							input: { path: "src/app.ts", apiKey: "secret-value" },
	...
	it("preserves provider text events in paged node activity", async () => {
			...
			data: [
				{
					...
					payload: {
						...
						provider: "anthropic",
						runId: "run-1",
						event: { type: "text_delta", text: "Prepare text" },
					},
		...
		const page = await getTaskActivityPage({
			taskId: task.id,
			scope: "node",
			nodeId: "prepare",
			limit: 10,
