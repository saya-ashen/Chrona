...
async function deleteTaskTreeGoalAssets(
...
export async function deleteTaskTreeRecords(
	...
	await tx.taskDependency.deleteMany({
		where: {
			OR: [{ taskId: currentTaskId }, { dependsOnTaskId: currentTaskId }],
		},
	});
	...
	await tx.taskAssistantMessage.deleteMany({
		where: { taskId: currentTaskId },
	});
...
export async function deleteTask(
	taskId: string,
	expected: { expectedTaskIds: string[]; expectedAssetIds: string[] },
) {
	const task = await db.task.findUnique({
		where: { id: taskId },
	...
	const actualTaskIds = [...impact.taskIds].sort((left, right) =>
		left.localeCompare(right),
	);
	const actualAssetIds = impact.assets
		.map((asset) => asset.id)
		.sort((left, right) => left.localeCompare(right));
	const expectedTaskIds = [...expected.expectedTaskIds].sort((left, right) =>
		left.localeCompare(right),
	);
	const expectedAssetIds = [...expected.expectedAssetIds].sort((left, right) =>
		left.localeCompare(right),
	);
	if (
