...
import { captureTaskPlanGenerationSnapshot, type TaskPlanGenerationSnapshot } from "./task-plan-generation-persistence";
...
async function recordPlanGenerationEvent(input: {
  ...
  await withSchedulerWorkOwnership(currentSchedulerWorkContext(), async (tx) => {
    await appendCanonicalEvent({
      ...
      payload: { generation_id: input.generationId, ...(input.payload ?? {}) },
      occurredAt: new Date(),
      dedupeKey: ["plan_generation", input.task.id, input.generationId, input.type, input.dedupeSuffix].filter(Boolean).join(":"),
    }, tx);
  });
}
...
export async function* generateTaskPlanManualStream(input: {
  ...
  const snapshot = input.snapshot ?? await captureTaskPlanGenerationSnapshot({ taskId: input.taskId, workBlockId: input.workBlockId });
  if (!snapshot) {
  ...
  let sequence = 0;
  const status = async function* (phase: "loading_task" | "requesting_provider", message: string) {
    await recordPlanGenerationEvent({
      type: "status", task, workBlockId: snapshot.workBlockId, generationId,
      payload: { phase, message, sequence: ++sequence }, dedupeSuffix: `${sequence}:${phase}`,
    });
    yield { type: "status", phase, message } as GeneratePlanSSEEvent;
  };

  await recordPlanGenerationEvent({
    type: "started", task, workBlockId: snapshot.workBlockId, generationId,
    payload: { force_refresh: input.forceRefresh ?? false, head_state_version: snapshot.head.stateVersion },
  });
  yield* status("loading_task", "Loading frozen task and plan-head context...");
  if (input.signal?.aborted) {
    await recordPlanGenerationEvent({ type: "cancelled", task, workBlockId: snapshot.workBlockId, generationId, dedupeSuffix: "before_start" });
  ...
  try {
    ...
    if (typeof receipt?.planId !== "string" || typeof receipt.headStateVersion !== "number") {
      const message = "Plan generation completed without an atomic commit receipt.";
      await recordPlanGenerationEvent({ type: "failed", task, workBlockId: snapshot.workBlockId, generationId, payload: { code: "INTERNAL_ERROR", message }, dedupeSuffix: "missing_receipt" });
      yield { type: "failed", code: "INTERNAL_ERROR", message };
      return;
    }
    await recordPlanGenerationEvent({
      type: "completed", task, workBlockId: snapshot.workBlockId, generationId,
      payload: { plan_id: receipt.planId, head_state_version: receipt.headStateVersion },
    });
    yield { type: "committed", planId: receipt.planId, headStateVersion: receipt.headStateVersion };
    yield { type: "done" };
  } catch (cause) {
    let runtimeCode: string | undefined;
    if (cause && typeof cause === "object" && "detail" in cause && cause.detail && typeof cause.detail === "object") {
      if ("code" in cause.detail && typeof cause.detail.code === "string") runtimeCode = cause.detail.code;
    }
    const stale = runtimeCode === "stale_plan_baseline";
    const message = stale ? "Task plan changed while generation was running." : "Unable to generate task plan.";
    await recordPlanGenerationEvent({ type: "failed", task, workBlockId: snapshot.workBlockId, generationId, payload: { code: stale ? "STALE_GENERATION" : "INTERNAL_ERROR", message }, dedupeSuffix: stale ? "stale" : "internal" });
    yield stale
