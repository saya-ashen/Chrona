...
import { defineAiFeature, stableJsonHash } from "../../ai";
...
const frozenGoalAssetSchema = z
	...
	.strict();
const frozenGoalAcceptedResultSchema = z
	.object({
	...
	.strict();
const frozenGoalTaskContextSchema = z
	.object({
...
export type TaskPlanGenerateOutput = z.infer<
	typeof taskPlanGenerateOutputSchema
>;

export const TASK_PLAN_GENERATE_FEATURE = {
	id: "task.plan.generate",
	version: 1,
} as const;
export const TASK_PLAN_GENERATE_PROPOSE_ACTION = {
	id: "task.plan.blueprint.propose",
	version: 1,
} as const;
...
export const taskPlanGenerateFeature = defineAiFeature({
	manifest: {
		...
		description:
			"Generate one structurally valid task-plan blueprint from frozen task and plan-head observations.",
		input: { id: "task.plan.generate.input", version: 1 },
		observations: [
			{
				observation: { id: "task.plan.generate.task", version: 1 },
				delivery: { kind: "seed" },
				required: true,
				maxBytes: 512 * 1024,
			},
			{
				observation: { id: "task.plan.generate.current-head", version: 1 },
				delivery: { kind: "seed" },
				required: true,
				maxBytes: 128 * 1024,
			},
	...
	buildObjective: (input) => {
		...
		return {
			...
			successCriteria: [
				"Return exactly one proposal containing a structurally valid PlanBlueprint.",
			],
	...
	validateCompletion: ({
		...
		const actionValid =
			...
			stableJsonHash(proposal.input) ===
				stableJsonHash({ blueprint: result.output.blueprint });
		return {
			valid:
				featureInput.success &&
				validation.ok &&
				actionValid &&
					...
					: [
							{
								code: "input_invalid",
								path: "/input",
								message: "Generation requires a valid frozen task-plan input.",
							},
						]),
				...validation.issues,
				...(actionValid
					...
					: [
							{
								...
								message:
									"Generation must return exactly one matching plan proposal.",
							},
						]),
			],
		};
	...
	commitResult: async (context) => {
		if (
			context.terminal.result.status !== "completed" ||
...
export function taskPlanGenerateCandidate(
