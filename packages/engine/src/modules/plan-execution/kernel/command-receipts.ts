...
export function canonicalReceiptResult(
	result: PlanExecutionResult,
): PlanExecutionResult {
	try {
		return JSON.parse(canonicalJsonString(result)) as PlanExecutionResult;
	} catch (error) {
		throw new Error("Failed to canonicalize execution result", {
			cause: error,
		});
	}
}
...
type InternalPlanExecutionResult = PlanExecutionResult & {
	[AUTHORITATIVE_COMMIT]?: boolean;
};

export function markAuthoritativeExecutionResult(
	result: PlanExecutionResult,
): PlanExecutionResult {
...
export function isAuthoritativeExecutionResult(
	result: PlanExecutionResult,
): result is InternalPlanExecutionResult {
