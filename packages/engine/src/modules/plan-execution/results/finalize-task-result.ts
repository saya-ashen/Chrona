...
const FINALIZED_RESULT_INSTRUCTIONS = [
	...
	"Use ResultSection to create meaningful regions with stack, grid, split, or rail layout. Use density compact for short supporting sections when an outer tone/background would only repeat the child component's framing. Use ResultComparison for bounded option trade-offs and always set its rowHeaderLabel to the semantic meaning of the first column (for example, Dimension or Repository), never Rank by default. Use ResultTimeline for dates or ordered milestones, ResultChecklist for operational steps, ResultMetricGrid only for exact manifest-supported values, and ResultChangeSummary for concrete code, configuration, or document changes.",
	"Use ResultDeliverable only for current deliverables worth featuring in the narrative and set artifactRef to its opaque manifest artifactRef. At most one may have role primary and at most three deliverables may appear in the Spec. The finalized Spec is the authoritative result body; include every deliverable needed to understand or use the result, while omitting supporting files that add no decision value. Never repeat artifactRefs or expose paths as prose.",
	"When a result contains multiple downloadable artifacts, make the primary deliverable clear and provide a concise supporting-files section or explicit file list so every user-facing download has a corresponding result artifact.",
	...
	"Keep the first viewport useful: place the concise outcome first, then any readiness limitation, the primary deliverable or decision surface, and only then supporting comparisons, observations, and evidence. Keep supporting observations and raw evidence collapsible when they are not needed to act immediately. Keep the complete Spec under 48 elements, nesting at most five levels, and avoid long prose when a comparison, checklist, timeline, metric group, or Artifact preview expresses the result better.",
...
const FINALIZED_RESULT_REVIEW_INSTRUCTIONS = [
	...
	"Preserve the candidate's valid content-driven information architecture unless changing it materially improves usefulness. Do not replace it with a fixed report template.",
	"Correct omissions of primary result content, duplicated facts, repeated element references or root descendants, weak first-viewport priorities, misleading readiness, inappropriate component choices, excessive card repetition, and raw evidence that should be subordinate.",
	...
	"Every transformed manifest statement must retain exact sourceKeys. Keep the result under 48 elements and five nesting levels, and return only the complete revised Spec.",
].join("\n");
