...
describe("runProviderRequest", () => {
	...
	it("does not expose invalid events to observers", async () => {
		...
		await expect(
			runProviderRequest(provider, request, { onEvent: observer }),
		).rejects.toMatchObject({ code: "internal" });
		expect(observer).not.toHaveBeenCalled();
	});

	it("rejects structured requests that have no terminal tool", async () => {
		const provider = {
			provider: "test",
			getCapabilities: async () => providerCapabilities,
			startRun: async () => ({
		...
		).rejects.toMatchObject({ code: "terminal_tool_missing" });
	});
});
