...
import {
  ...
  isActionableAutoStartSkipReason,
...
const MAX_AUTOMATION_TIMING_OFFSET_MS = Math.max(
  ...AUTOMATION_TIMING_PRESETS.map((preset) =>
    automationTimingOffsetMs(preset),
  ),
);
...
export type AutoStartScheduledPlanResult = {
  ...
  skipped: Array<{
    taskId: string;
    workBlockId: string;
    reasonCode: AutoStartSkipReason | "occurrence_already_claimed";
    reason: string;
    actionable: boolean;
  }>;
...
export async function autoStartScheduledPlanTasks(input?: {
  ...
  const windowUpperBound = new Date(
    now.getTime() + MAX_AUTOMATION_TIMING_OFFSET_MS,
  );
    ...
    try {
      ...
      if (!eligibility.ok) {
        result.skipped.push({
          ...
          reason: eligibility.disabledReason,
          actionable: isActionableAutoStartSkipReason(eligibility.reason),
        });
      ...
      if (claim.count !== 1) {
        result.skipped.push({
          ...
          reason: "This occurrence was already claimed.",
          actionable: false,
        });
        continue;
      }
      ...
      result.started.push({
        taskId: task.id,
        workBlockId: block.id,
        runId: startedRun.planId ?? task.id,
      });
    } catch (parentError) {
      const message =
