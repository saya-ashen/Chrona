...
type OpenClawOrchestratorOptions = {
...
export class OpenClawOrchestrator {
  ...
  private readonly onEvent:
    | ((event: OpenClawOrchestratorEvent) => void)
    | undefined;
  ...
  async getSessionStatus(
    ...
    try {
      ...
      return {
        ...
        lastMessage:
          exists && history.messages.length > 0
    ...
    while (Date.now() < deadline) {
      ...
      if (isTerminalStatus(snapshot.status)) {
        ...
        const result: OpenClawExecuteTaskResult = {
          ...
          elapsedMs: Date.now() - startTime,
          error:
            snapshot.status === "Failed" ? snapshot.lastMessage : undefined,
        };
