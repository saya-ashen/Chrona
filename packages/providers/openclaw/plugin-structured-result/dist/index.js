...
function normalizePriority(value) {
  return PriorityEnum.includes(value) ? value : "Medium";
}
function normalizeSuggestions(input) {
  ...
  return input.filter((item) => Boolean(item) && typeof item === "object").map((item) => ({
    ...
    estimatedMinutes: typeof item.estimatedMinutes === "number" && Number.isFinite(item.estimatedMinutes) ? Math.max(5, Math.round(item.estimatedMinutes)) : 30,
    tags: normalizeStringArray(item.tags),
    suggestedSlot: item.suggestedSlot && typeof item.suggestedSlot === "object" ? {
      startAt: typeof item.suggestedSlot.startAt === "string" ? String(item.suggestedSlot.startAt) : "",
      endAt: typeof item.suggestedSlot.endAt === "string" ? String(item.suggestedSlot.endAt) : ""
    } : undefined
...
function hasNonEmptyPlanGraph(graph) {
  return graph.summary.trim().length > 0 && graph.nodes.length > 0;
}
var SuggestTaskCompletionsSchema = {
  type: "object",
  additionalProperties: false,
  description: "Generate user-visible task suggestions for schedule quick create and smart auto-complete.",
  required: ["input"],
  properties: {
    input: {
      type: "string",
      minLength: 1,
      description: "Raw user input. Preserve the full original text; do not truncate or silently rewrite it."
    },
    workspaceId: {
    ...
    context: {
      ...
      description: "Optional contextual information such as existing tasks or schedule state."
    }
  }
};
var TaskPlanNodeSchema = {
  type: "object",
...
var GenerateTaskPlanGraphSchema = {
  type: "object",
  additionalProperties: false,
  description: "LLM must put the generated plan graph directly into this tool input. Chrona treats this tool input as the business source of truth for generate_plan.",
  required: ["title", "summary", "nodes", "edges"],
  properties: {
    taskId: {
      anyOf: [{ type: "string" }, { type: "null" }],
      description: "Optional persisted task id when decomposing an existing task."
    },
    title: {
      type: "string",
      minLength: 1,
...
var src_default = definePluginEntry({
  ...
  description: "Provides readable Chrona business tools for suggestions and task-plan graphs.",
  reload: { restartPrefixes: ["gateway", "plugins"] },
  register(api) {
    api.registerTool({
      name: "suggest_task_completions",
      label: "Suggest Task Completions",
      description: "Business tool for schedule quick-create and auto-complete. Use this to generate readable task suggestions from the raw user input.",
      ...
      async execute(_toolCallId, params) {
        ...
        const suggestions = normalizeSuggestions(context.precomputedSuggestions ?? [
          {
            title: baseTitle,
        ...
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify({ ok: true, suggestionCount: suggestions.length }, null, 2)
            }
          ],
          details: {
            suggestions,
