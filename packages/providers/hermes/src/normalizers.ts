...
/* eslint-disable complexity, max-lines-per-function, @typescript-eslint/no-unnecessary-condition -- Hermes event normalization maps an intentionally broad external protocol. */
...
export function mapCapabilities(
	...
	return {
		...
		recovery: {
			...
			crossProcessDurable: Boolean(
				features.run_status && features.run_events_sse,
			),
			mode: "authoritative_run_lookup",
...
export function mapSnapshot(
	...
	const structuredPayload = parseStructuredPayload(body.output);
	...
	switch (type) {
		...
		case "approval.request":
			return {
				...
				approval: normalizeHermesApproval(
					event,
		...
		case undefined: {
			if (options.strictUnknown) {
				throw new Error(
					`Unknown Hermes stream event type: ${type ?? "<missing>"}`,
				);
			}
			return undefined;
		}
...
function buildEventMetadata(input: {
	...
	const rawEventType =
		stringValue(input.body.type) ?? stringValue(input.body.event);
	const timestampValue =
		stringValue(input.body.timestamp) ?? stringValue(input.body.time);
...
function normalizeHermesApproval(
	...
	return {
		...
		subject: command
			...
			: undefined,
		choices,
		defaultChoice: choices.includes("approve_once")
			? "approve_once"
			: undefined,
...
function hermesApprovalRisk(
	providerKind: string,
): ProviderApprovalRequest["riskLevel"] {
	switch (providerKind) {
		case "execute_code":
		case "shell_command":
			return "high";
...
function detectApprovalCommandLanguage(command: string): string | undefined {
	if (
		command.startsWith("execute_code <<'PY'") ||
		command.startsWith('execute_code <<"PY"')
	) {
		return "python";
	}
