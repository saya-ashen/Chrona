...
export const providerToolPolicySchema = z.enum([
	"full",
	"read_only",
	"terminal_only",
]);

export const providerTerminalToolCallSchema = z
	.object({
		name: z.string().min(1),
...
export const providerConfigurationCapabilitiesSchema = z
	.object({
		model: z
			.object({ supported: z.boolean(), taskOverride: z.boolean() })
			.strict(),
		context: z
			.object({
				supported: z.boolean(),
...
export const providerOutcomeCodeSchema = z.enum([
	...
	"terminal_tool_duplicate",
	"terminal_tool_payload_invalid",
]);
	...
	.superRefine((value, context) => {
		if (value.result === undefined && !value.error) {
			context.addIssue({
				...
				message: "Provider tool result requires result or error",
			});
		}
		if (value.result !== undefined && value.error) {
			context.addIssue({
				code: z.ZodIssueCode.custom,
				message: "Provider tool result cannot include both result and error",
			});
		}
	});
...
export class ProviderOperationError extends Error {
	...
	constructor(input: {
		code: z.infer<typeof providerOutcomeCodeSchema>;
		message: string;
		provider?: string;
		cause?: unknown;
	}) {
...
export function assertProviderStartSupported(
	...
	input: Pick<
		StartRunInput,
		"tools" | "structuredOutputSchema" | "terminalToolName" | "toolPolicy"
	>,
	provider?: string,
): void {
	if (input.structuredOutputSchema) {
		const terminalTool = input.terminalToolName
			? input.tools?.find(({ name }) => name === input.terminalToolName)
			: undefined;
		if (!input.terminalToolName || !terminalTool) {
			throw new ProviderOperationError({
				code: "terminal_tool_missing",
				provider,
				message: `${provider ?? "Provider"} structured result requests require a declared terminal tool`,
			});
		}
		if (input.toolPolicy !== "terminal_only" && input.toolPolicy !== "full") {
			throw new ProviderOperationError({
				code: "provider_capability_mismatch",
				provider,
				message: `${provider ?? "Provider"} terminal result requests require terminal_only or full tool policy`,
			});
		}
	}
	if (
		...
		const nonTerminalTools = input.tools.filter(
			({ name }) => name !== input.terminalToolName,
		);
		if (nonTerminalTools.length > 0) {
			throw new ProviderOperationError({
				code: "provider_capability_mismatch",
				provider,
				message: `${provider ?? "Provider"} terminal_only requests cannot declare non-terminal tools`,
			});
		}
	}
	if (
		input.tools?.length &&
		input.toolPolicy !== "terminal_only" &&
		capabilities.actionInvocation !== "engine_managed"
	) {
		throw new ProviderOperationError({
			code: "provider_capability_mismatch",
			provider,
...
export function supportsDurableFeatureRuntime(
	capabilities: ProviderCapabilities,
): boolean {
	const recovery = capabilities.recovery;
	return (
		capabilities.startIdempotency === "client_operation_id" &&
...
export const startRunInputSchema = z
	.object({
		...
		toolPolicy: providerToolPolicySchema.optional(),
...
export type StartRunInputWithControl = z.infer<
	typeof startRunInputWithControlSchema
>;
...
export const providerRunEventSchema = z.discriminatedUnion("type", [
	...
	z
		.object({
			...
			terminalToolCall: providerTerminalToolCallSchema.optional(),
...
>;
export type ProviderApprovalChoice = z.infer<
	typeof providerApprovalChoiceSchema
>;
export type ProviderApprovalRiskLevel = z.infer<
	typeof providerApprovalRiskLevelSchema
>;
export type ProviderApprovalSubject = z.infer<
	typeof providerApprovalSubjectSchema
>;
...
>;
export type ProviderApprovalRequest = z.infer<
	typeof providerApprovalRequestSchema
>;

export type ProviderConfigurationCapabilities = z.infer<
	typeof providerConfigurationCapabilitiesSchema
>;
export type ProviderRuntimeDiagnostics = z.infer<
	typeof providerRuntimeDiagnosticsSchema
>;
...
export type ExistingRunStreamInput = z.infer<
	typeof existingRunStreamInputSchema
>;
...
export type ProviderActionInvocationMode = z.infer<
	typeof providerActionInvocationModeSchema
>;
export type ProviderStartIdempotency = z.infer<
	typeof providerStartIdempotencySchema
>;
...
>;
export type ProviderTerminalToolCall = z.infer<
	typeof providerTerminalToolCallSchema
>;
export type FindRunByClientOperationInput = z.infer<
	typeof findRunByClientOperationInputSchema
>;
export type ProviderToolResultInput = z.infer<
	typeof providerToolResultInputSchema
>;
export type ProviderToolResultOutcome = z.infer<
	typeof providerToolResultOutcomeSchema
>;
...
export interface AgentProviderClient {
	...
	getRuntimeDiagnostics?():
		...
		| Promise<ProviderRuntimeDiagnostics>;
	...
	inspectConversation?(sessionRef: string): Promise<ProviderConversationState>;
