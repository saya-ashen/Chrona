...
describe("chrona structured result plugin tools", () => {
  ...
  it("registers readable business tools alongside submit_structured_result", () => {
    expect(source).toContain('name: "suggest_task_completions"');
    expect(source).toContain('name: "generate_task_plan_graph"');
    expect(source).toContain('name: "submit_structured_result"');
  });
  ...
  it("generate_task_plan_graph now requires the model to pass graph fields directly in tool input", () => {
    expect(source).toContain('Chrona treats this tool input as the business source of truth for generate_plan');
    expect(source).toContain('nodes: Array.isArray(value.nodes) ? value.nodes : []');
