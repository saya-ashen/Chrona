...
import { candidateSourceFiles, scanFiles } from "./lib/chronicle/source-scan";
import { loadRules } from "./lib/chronicle/rules";
import { testRefsForSource } from "./lib/chronicle/test-map-adapter";
import {
  createChronicle,
...
function docPathFor(sourcePath: string): string {
  ...
  return path.join(parsed.dir, `${parsed.name}.chron.md`);
}
...
function frontMatterFor(scan: SourceFileScan, symbols: SourceSymbol[], tests: TestRefs, existing?: ChronicleFrontMatter): ChronicleFrontMatter {
  ...
  return ChronicleFrontMatterSchema.parse({
    chronicle_version: 1,
...
function updateDoc(scan: SourceFileScan, createMissing: boolean): { path: string; changed: boolean; created: boolean } | undefined {
  ...
  if (!fs.existsSync(docPath)) {
    ...
    fs.writeFileSync(docPath, createChronicle(fm, symbols, tests));
    return { path: docPath, changed: true, created: true };
  }
  const before = fs.readFileSync(docPath, "utf8");
  const parsed = parseChronicle(before);
...
function runScaffold(basePath?: string) {
  ...
  console.log(`chronicle:scaffold docs=${results.length} changed=${results.filter((r) => r!.changed).length} created=${results.filter((r) => r!.created).length}`);
}

function runSync(basePath?: string) {
  const results = scansFor(basePath).map((scan) => updateDoc(scan, false)).filter(Boolean);
  console.log(`chronicle:sync docs=${results.length} changed=${results.filter((r) => r!.changed).length}`);
}
...
function discoverDocs(basePath?: string): string[] {
  ...
  const walk = (dir: string) => {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (entry.isFile() && entry.name.endsWith(".chron.md") && (!prefix || rel.startsWith(prefix))) out.push(rel);
    }
  };
  walk(prefix || ".");
  return out.sort();
...
function renderIndex(docs = discoverDocs()): string {
  const rows = docs.map((doc) => {
    const fm = parseChronicle(fs.readFileSync(doc, "utf8")).frontMatter;
  ...
  const lines = ["# Chronicle index", "", "Generated by `bun run chronicle:index`. Do not edit manually.", ""];
  let current = "";
...
function runIndex() {
  ...
  console.log(`chronicle:index ${before === next ? "fresh" : "updated"}`);
}
...
function runCheck(basePath: string | undefined, strictAi: boolean) {
  ...
  if (basePath) {
    for (const scan of scans) {
      ...
      if (!fs.existsSync(doc)) errors.push(`${scan.filePath}: missing chronicle ${doc}`);
    }
  }
  ...
  if (actualIndex !== expectedIndex) errors.push(`${INDEX_PATH}: stale; run bun run chronicle:index`);
  if (errors.length) {
    console.error(errors.join("\n"));
    process.exit(1);
  }
  console.log(`chronicle:check ok docs=${discoverDocs(basePath).length}`);
}
