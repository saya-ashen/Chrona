...
AgentDashboard 采用 **CQRS（命令查询职责分离）** 结合 **事件溯源（Event Sourcing）** 的架构模式。所有的状态变更都通过命令（Command）触发，产生规范事件（Canonical Event）持久化到事件存储，然后重建物化投影（Materialized Projection）供查询层读取。
...
│  │  React 19  │  │  agentdash │  │ OpenClaw CLI Bridge│  │
│  └─────┬──────┘  └─────┬──────┘  └────────┬───────────┘  │
└────────┼───────────────┼──────────────────┼──────────────┘
         │               │                  │
         ▼               ▼                  ▼
