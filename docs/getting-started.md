...
```bash
bun src/cli/index.ts task create \
  -w <workspaceId> \
  --title "分析用户数据" \
...
```bash
bun src/cli/index.ts schedule apply \
  -t <taskId> \
...
# 任务管理
bun src/cli/index.ts task list -w <workspaceId>
bun src/cli/index.ts task get -t <taskId>

# AI 功能
bun src/cli/index.ts ai decompose -t <taskId>      # AI 任务分解
bun src/cli/index.ts ai auto-complete --title "分析" # 标题自动补全

# 排期
bun src/cli/index.ts schedule view -w <workspaceId> # 查看排期
bun src/cli/index.ts schedule conflicts -w <wId>    # 冲突分析
```
