...
- `DAILY-USE-02 结果检索增强`：Results 新增日期、状态（Needs review/Accepted）和来源任务三维筛选；历史结果行继续显示 occurrence、source task、executed time、AI、run id 和结果状态。新增两周跨度 recurrence fixture 行为测试，验证状态与来源组合筛选后仍可定位 occurrence 和打开 source task。focused 1 test、`bun run typecheck`、`bun run check:ui-foundation` 通过。完整键盘路径、Dialog/Sheet 焦点返回、axe、三档 viewport 和真人 30 秒指标尚未关闭，工作包保持 `IN PROGRESS`。
