...
describe("openclaw structured plugin install command", () => {
  it("installation script exists and plugin package is wired", () => {
    ...
    const pluginPkg = resolve(process.cwd(), "packages/providers/openclaw/plugin-structured-result/package.json");
    const pluginEntry = resolve(process.cwd(), "packages/providers/openclaw/plugin-structured-result/src/index.ts");
