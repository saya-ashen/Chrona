import { FeatureDocFrontMatterSchema, type FeatureDocFrontMatter, type SourceSymbol, type TestRefs } from "./types";
...
export function stringifyFrontMatter(frontMatter: FeatureDocFrontMatter): string {
  return renderObject(frontMatter as unknown as Record<string, unknown>).join("\n") + "\n";
}

export function parseFeatureDoc(content: string): ParsedFeatureDoc {
  const match = content.match(FRONT_MATTER);
  if (!match) throw new Error("Missing YAML front matter");
  const frontMatter = FeatureDocFrontMatterSchema.parse(parseYamlLike(match[1]!));
  return { frontMatter, body: content.slice(match[0].length) };
}

export function updateFrontMatter(content: string, frontMatter: FeatureDocFrontMatter): string {
...
export function createFeatureDoc(frontMatter: FeatureDocFrontMatter, symbols: SourceSymbol[], tests: TestRefs): string {
