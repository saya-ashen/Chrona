...
import type { FeatureDocFrontMatter, FeatureDocsRules, FeatureDocSymbolKind, SourceFileScan, SourceSymbol } from "./types";
...
function selectedEntryFor(file: string, rules: FeatureDocsRules) {
  return rules.selection.files.find((entry) => entry.path === file);
}

export function candidateSourceFiles(rules: FeatureDocsRules, basePath?: string): string[] {
...
function layerFor(file: string): FeatureDocFrontMatter["layer"] {
...
function classifyKind(name: string, baseKind: FeatureDocSymbolKind, file: string): FeatureDocSymbolKind {
...
function scoreSymbol(symbol: Omit<SourceSymbol, "score" | "reason" | "describe">, rules: FeatureDocsRules): Pick<SourceSymbol, "score" | "reason" | "describe"> {
...
function excludedSymbol(name: string, kind: FeatureDocSymbolKind, rules: FeatureDocsRules): boolean {
...
function addSymbol(out: SourceSymbol[], name: string, baseKind: FeatureDocSymbolKind, exported: boolean, file: string, node: Node, rules: FeatureDocsRules) {
...
export function scanFiles(files: string[], rules: FeatureDocsRules): SourceFileScan[] {
...
function scanSourceFile(sf: SourceFile, file: string, rules: FeatureDocsRules): SourceFileScan {
  ...
  for (const vd of sf.getVariableDeclarations()) {
    ...
    const baseKind: FeatureDocSymbolKind = init?.isKind(SyntaxKind.ArrowFunction) || init?.isKind(SyntaxKind.FunctionExpression) ? "function" : "const";
