...
async function smokeEntrypoint(
	entrypoint: EntryPoint,
	tempDir: string,
): Promise<void> {
	const output: string[] = [];
	...
	const outputCollection = Promise.all([
	...
	if (child.exitCode !== 0 && child.exitCode !== 143) {
		throw new Error(
			`${entrypoint.name} exited with ${child.exitCode}\n${output.join("")}`,
		);
	}
...
try {
	await smokeEntrypoint(
		{
			name: "server:start",
			command: ["bun", "run", "server:start"],
			url: "http://127.0.0.1:34101/health",
		},
		tempDir,
	);
	await smokeEntrypoint(
		{
			...
			url: "http://127.0.0.1:34102/health",
		},
		tempDir,
	);
	await smokeEntrypoint(
