...
test.describe("Task workspace node drawer reliability", () => {
	test("[WORK-006] reopens the correct node detail after refresh", async ({
		page,
		request,
	}, testInfo) => {
		const viewport =
			testInfo.project.name === "tablet" || testInfo.project.name === "mobile"
		...
		const task = await createTaskWorkspaceTask(request, {
			...
			description:
				"Verify task workspace node drawer open, close, and reopen reliability.",
		});
		await generateTaskWorkspacePlan(request, task.taskId);
		await page.goto(`/en/tasks/${task.taskId}`);

		await expect(
			page.getByRole("heading", { name: `E2E Drawer Reliability ${viewport}` }),
		).toBeVisible();
		await expect(page.getByTestId("accepted-plan-surface")).toBeVisible();

		await clickPlanNode(page);
		await expect(
			page.getByText("Inspecting step: Collect boundary context", {
				exact: true,
			}),
		).toBeVisible();
		await expect(page.getByTestId("accepted-plan-surface")).toBeVisible();
		...
		await expect(
			page.getByText("Inspecting step: Route execution", { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText("Inspecting step: Collect boundary context", {
				exact: true,
