...
const GOLDEN_PATH_BLUEPRINT: PlanBlueprint = {
  ...
  nodes: [
    {
      ...
      required: true,
      inputFields: [
        { key: "scenario_label", label: "Scenario label", inputType: "text", required: true },
        { key: "include_slow_wait", label: "Include slow wait path", inputType: "boolean", required: true },
        {
          key: "priority",
          label: "Priority",
          inputType: "choice",
  ...
  edges: [
    ...
    { from: "route_execution", to: "wait_for_external_event", label: "slow wait" },
...
export async function startMockTaskPlanProvider(): Promise<MockTaskPlanProvider> {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/v1/runs") {
        ...
        const sessionId = typeof body.session_id === "string" ? body.session_id : runId;
        ...
        runs.set(runId, { runId, sessionId, status: "running", terminalOutput });
        writeJson(response, 200, { run_id: runId, session_id: sessionId, status: "running" });
        return;
      }
      ...
      writeJson(response, 404, { error: `No mock provider route for ${request.method} ${url.pathname}` });
    } catch (error) {
  ...
  if (!address || typeof address === "string") throw new Error("Could not bind mock task-plan provider");
...
export async function bindTaskPlanProvider(
  ...
  taskId: string,
  baseUrl: string,
  features: readonly ("task.plan" | "task.result_finalization")[] = ["task.plan"],
): Promise<void> {
  ...
  if (!createResponse.ok()) {
    throw new Error(`Task-plan client creation failed: HTTP ${createResponse.status()} ${await createResponse.text()}`);
  }
  ...
  if (!bindResponse.ok()) {
    throw new Error(`Task-plan client binding failed: HTTP ${bindResponse.status()} ${await bindResponse.text()}`);
  }
}
