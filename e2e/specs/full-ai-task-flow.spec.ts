...
test("full deterministic AI task flow persists plan updates", async ({
  ...
  const bindingsResp = await request.put(
    `/api/ai/clients/${createdClient.id}/bindings`,
    {
  ...
  await page
    ...
    .fill("Create a checklist, review it, and prepare final rollout notes.");
  ...
  await request.post("/api/ai/generate-task-plan", {
