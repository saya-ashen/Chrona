...
test.describe("external calendar source setup", () => {
  test("adds a read-only calendar source and rejects an invalid link", async ({
    page,
  }, testInfo) => {
    ...
    await expect(
      page.getByRole("heading", { name: /connect external calendar/i }),
    ).toBeVisible();
    ...
    let connectDialog = page.getByRole("dialog", {
      name: /connect external calendar/i,
    });
    for (let index = 0; index < 8; index += 1) {
      await page.keyboard.press("Tab");
      await expect(connectDialog.locator(":focus")).toHaveCount(1);
    }
    await page.keyboard.press("Escape");
    await expect(connectDialog).not.toBeVisible();
    await expect(connectButton).toBeFocused();
    await page.keyboard.press("Enter");
    connectDialog = page.getByRole("dialog", {
      name: /connect external calendar/i,
    });

    await connectDialog.getByLabel(/display name/i).fill(sourceName);
    await connectDialog
      .getByLabel(/calendar url/i)
      .fill(
        fixtureUrl(`External standup ${testInfo.project.name}`, sourceName),
      );
    await connectDialog
      .getByRole("button", { name: /connect calendar/i })
      .click();
    await expect(
      page.getByRole("listitem").filter({ hasText: sourceName }),
    ).toBeVisible();
    ...
    await invalidDialog
      ...
      .fill(`Bad calendar ${testInfo.project.name}`);
    await invalidDialog
      .getByLabel(/calendar url/i)
      .fill("ftp://example.test/private.ics");
    await invalidDialog
      .getByRole("button", { name: /connect calendar/i })
      .click();
