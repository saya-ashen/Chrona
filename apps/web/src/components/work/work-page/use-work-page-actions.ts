  ...
  const submitWorkbenchInput = useCallback(
    async (rawInputText: string) => {
      ...
      const didSucceed = await runScopedAction(async () => {
        ...
        if (!planExecution) {
          throw new Error(
            "Execution state is unavailable. Refresh the work page and try again.",
          );
        }
        ...
        if (planExecution.status === "completed") {
          throw new Error(
            "Plan execution is complete. Reopen or create a follow-up task.",
          );
        }
