...
import { AUTO_FULL_MODE_MIN_WIDTH, DEFAULT_GRAPH_COPY, MAX_VIEWPORT_HEIGHT } from "./constants";
import { buildCompactViewModel, CompactFocusStack, CompactStageStrip } from "./compact-view";
...
function GraphShell({
  ...
  nodes: FlowGraphNode[];
  edges: ReturnType<typeof buildFlowLayout>["edges"];
...
export function TaskPlanGraph({ plan, mode = "full", maxViewportHeight = MAX_VIEWPORT_HEIGHT }: TaskPlanGraphProps) {
  ...
  const layout = useMemo(
    ...
    [graphCopy, handleSelectNode, maxViewportHeight, plan, selectedNodeId],
  );
  ...
  }, [graphCopy, handleSelectNode, plan.analytics.reachableFromActiveIds, selectedNodeId, setNodes]);
  ...
  const resolvedMode: Exclude<TaskPlanGraphMode, "auto"> = mode === "auto" ? (containerWidth >= AUTO_FULL_MODE_MIN_WIDTH ? "full" : "compact") : mode;

  if (plan.state !== "ready" || plan.nodes.length === 0) return null;

  const overviewItems = [
    { label: graphCopy.overviewNodes, value: plan.nodes.length },
    { label: graphCopy.overviewActive, value: plan.analytics.activeNodeIds.length },
    { label: graphCopy.overviewAttention, value: plan.analytics.attentionNodeIds.length },
    { label: graphCopy.overviewDone, value: plan.nodes.filter((node) => node.status === "done" || node.status === "skipped").length },
    {
        ...
        <div ref={(node) => {
          ...
          <div className="rounded-[24px] border border-border/50 bg-muted/[0.16] p-4" data-graph-mode="compact" data-testid="task-plan-graph">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">{graphCopy.compactTitle}</p>
                <p className="mt-1 text-sm text-muted-foreground">{graphCopy.compactDescription}</p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 space-y-4">
              ...
              <section className="space-y-2">
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">{graphCopy.focusTitle}</p>
                  <p className="mt-1 text-sm text-muted-foreground">{graphCopy.focusDescription}</p>
                </div>
                <CompactFocusStack items={compact.focusItems} selectedNodeId={selectedNodeId} onSelect={handleSelectNode} graphCopy={graphCopy} />
              </section>
            </div>
          </div>
        </div>
            ...
            >
              <header className="flex items-start justify-between gap-4 border-b border-border/60 px-6 py-5">
                <div className="space-y-1">
                  <h1 className="text-lg font-semibold tracking-tight text-foreground">{graphCopy.fullTitle}</h1>
                  <p className="text-sm text-muted-foreground">{graphCopy.fullDescription}</p>
                </div>
                <button
                  type="button"
