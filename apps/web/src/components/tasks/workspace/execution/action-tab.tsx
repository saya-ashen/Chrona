...
export function useActionSpecRenderConfig({
  ...
  const hasEmptyRequired = useMemo(
    () => fields.some((f) => f.required && !requiredFieldValues[f.key]),
  ...
  const spec = useMemo(() => node ? buildActionSpec({
    ...
    disabledReason: disabledActionReason,
    disabledButton: !isReadOnly && hasEmptyRequired,
  }) : null, [fields, normalizedActions, isReadOnly, node, disabledActionReason, hasEmptyRequired]);
