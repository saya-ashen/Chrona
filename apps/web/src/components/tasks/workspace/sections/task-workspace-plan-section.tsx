    ...
    >
      ...
      <CardContent className="space-y-2 px-3">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <NodeDetailRow
            label="Mode"
            value={node.executionMode ?? node.interactionType ?? null}
          />
        ...
        {node.availableActions?.length ? (
          <div
            className="space-y-1.5 rounded-lg border border-border/55 bg-background/70 p-2"
        ...
        {node.options?.length || node.branchLabels?.length ? (
          <div
            className="space-y-1.5 rounded-lg border border-border/55 bg-muted/25 p-2"
            data-ui-surface-kind="runtime-control"
          >
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">
              Decision options
            ...
            <div className="flex flex-wrap gap-1.5">
              {[...(node.options ?? []), ...(node.branchLabels ?? [])].map(
                (option) => (
...
export function PlanSetupPanel({
  ...
  const requiredReady = readiness.checks.filter(
    (check) => check.level === "required" && check.state === "passed",
  ).length;
  const requiredTotal = readiness.checks.filter(
    (check) => check.level === "required",
  ).length;
  const improvements = readiness.checks.filter(
          ...
          <section aria-labelledby="plan-setup-brief-heading">
            ...
            <dl className="mt-3 grid overflow-hidden rounded-xl border border-border/70 bg-background/60 lg:grid-cols-2">
              ...
              <div className="border-b border-border/60 px-4 py-4 lg:border-b-0 lg:border-r">
                ...
                <dd className="mt-1 line-clamp-4 text-sm leading-6 text-foreground">
              ...
              <div className="px-4 py-4">
                ...
                <dd className="mt-1 text-sm font-medium text-foreground">
                  {provider?.name ??
                    pageData.task.executionRuntime ??
                    "Not connected"}
                </dd>
              </div>
            </dl>
          ...
          <div className="sticky top-4 space-y-6 rounded-2xl border border-primary/20 bg-card p-5 shadow-sm">
            <div className="space-y-2">
              ...
              <p className="text-lg font-semibold text-foreground">
                {requiredReady}/{requiredTotal} required checks ready
              </p>
...
function PlanReviewDecisionPanel({
  ...
  const humanSteps = graphPlan.nodes.filter((node) =>
    Boolean(
  ...
  const estimatedMinutes = graphPlan.nodes.reduce(
        ...
        <CardContent className="space-y-2 px-3">
          <dl className="grid grid-cols-3 gap-2 rounded-lg border border-border/60 bg-background/75 p-2 text-center">
            <div>
              ...
              <dd className="mt-1 font-semibold">{graphPlan.nodes.length}</dd>
            </div>
            <div>
              <dt className="text-[10px] uppercase tracking-wide text-muted-foreground">
                Time
            ...
            <div>
              ...
              <dd className="mt-1 font-semibold">{humanSteps}</dd>
            </div>
          </dl>
          <div className="rounded-lg border border-border/60 bg-background/75 px-3 py-2 text-xs leading-5 text-muted-foreground">
            ...
            Accepting saves this plan. Execution does not start until you
        ...
        >
          <CardHeader className="gap-1 px-3">
            <CardTitle className="text-sm">
              What should Chrona change?
            </CardTitle>
            <p className="text-xs text-muted-foreground">
              Choose the scope explicitly. Selecting a step for inspection does
              not change it.
            </p>
          </CardHeader>
          <CardContent className="space-y-3 px-3">
            ...
            <Button
              ...
              onClick={() =>
                onRevisePlan(
                  revisionScope === "step" ? (selectedNode?.id ?? null) : null,
                )
              }
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div
          className="min-w-[15rem] max-w-sm rounded-xl border border-border/70 bg-background/85 px-3 py-2.5 shadow-sm"
        ...
        {graphPlan.nodes.map((node, index) => {
          ...
          return (
            <li key={node.id}>
              ...
              >
                <span
                  className={`flex size-7 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${state === "completed" ? "bg-success/15 text-success" : state === "current" ? "bg-primary text-primary-foreground" : state === "blocked" ? "bg-destructive/15 text-destructive" : "bg-muted text-muted-foreground"}`}
                >
...
export function TaskWorkspacePlanSection({
  ...
  const [submittedRevisionInstruction, setSubmittedRevisionInstruction] =
    useState<string | null>(null);
  const [selectedNode, setSelectedNode] = useState<PlanNodeDataModel | null>(
    null,
  );
  const [isRequestingResultChanges, setIsRequestingResultChanges] =
    useState(false);
  const [resultChangeInstruction, setResultChangeInstruction] = useState("");
  const [resultChangeError, setResultChangeError] = useState<string | null>(
    null,
  );
  ...
  const recoveryIssue =
    pageData.reconciliation?.issues.find(
      (issue) => issue.severity === "error",
    ) ?? null;
  const recoveryCurrentNodeId =
    pageData.reconciliation?.currentNodeId ?? undefined;
  ...
  const hasTaskCompleted =
    isCompletedTaskStatus(pageData.task.status) ||
    hasCompletedGraphExecution(graphPlan);
  useEffect(() => {
  ...
  const persistedGenerationInstruction =
    plan?.prompt?.trim() || generationUserInstruction?.trim() || null;
  const visibleGenerationInstruction =
    submittedRevisionInstruction ?? persistedGenerationInstruction;
  const commandCenterScopeKey =
    pageData.task.currentWorkBlock?.id ?? pageData.task.id;
  ...
  const operationState = resolveTaskWorkspaceOperationState({
    ...
    shouldShowCurrentOperation:
      shouldShowCurrentOperation && Boolean(currentOperationNode),
  ...
  const lastCompletedResultNode =
    [...(graphPlan?.nodes ?? [])]
      .reverse()
      .find((node) => isCompletedGraphNode(node.status)) ?? null;
  const submitResultChanges = async () => {
    const instruction = resultChangeInstruction.trim();
    if (!instruction) {
      setResultChangeError(
        copy.requestChangesRequired ??
          "Describe the required change before starting the rerun.",
      );
      return;
    }
    if (!lastCompletedResultNode) {
      setResultChangeError(
        copy.requestChangesUnavailable ??
          "No completed result step is available to rerun.",
      );
      return;
    }
    ...
    try {
      await onDispatchExecutionAction({
        action: "retry_node",
        nodeId: lastCompletedResultNode.id,
        prompt: instruction,
      });
      ...
      {displayState.layout === "brief_focus" ? (
        ...
        <div className="min-h-[560px] flex-1 p-3 xl:min-h-0">
          <div className="flex min-h-0 flex-col gap-3">
            ...
            displayState.resultReview?.phase === "pending_acceptance" ? (
            ...
            >
              <TaskWorkspaceInspector
                ...
                commandCenter={
                  isGeneratingPlan ? null : (commandCenter ?? null)
                }
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/70 bg-card/70 px-4 py-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="outline">
                    {displayState.runPreview.planVersionLabel}
                  </Badge>
                </div>
              ...
              <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                <span>
                  {displayState.runPreview.stepCount} {copy.stepsUnit}
                </span>
                {displayState.runPreview.estimatedMinutes ? (
                  <span>
                    · {copy.launchAbout}{" "}
                    {displayState.runPreview.estimatedMinutes}{" "}
                    {copy.launchMinutes}
                ...
                <span>
                  · {displayState.runPreview.expectedStops.length}{" "}
                  {copy.launchStopsShort}
                </span>
              </div>
            </div>
            <div
        ...
        <div className="flex min-h-[560px] flex-1 flex-col overflow-hidden">
          ...
          <div className="grid min-h-0 flex-1 gap-4 p-4 xl:grid-cols-[minmax(17rem,0.42fr)_minmax(36rem,1.58fr)]">
            ...
            <TaskWorkspaceInspector
              ...
              onAction={focusNodeActions}
              operationPanel={
                operationState.status !== "execution-running" ? (
          ...
          >
            <div className="min-h-0 flex-1">
              <TaskWorkspacePlanContent
                ...
                onSelectedNodeChange={setSelectedNode}
              />
              {displayState.panels.planReviewSummary ||
              displayState.panels.runPreview ? (
                <div className="space-y-3 border-t border-border bg-card/65 p-3">
                  {displayState.panels.planReviewSummary &&
                  displayState.planReviewSummary ? (
                    <PlanReviewSummaryCard
                      summary={displayState.planReviewSummary}
                    />
                  ) : null}
                </div>
              ) : null}
