...
const fallbackCopy = {
	autoSuggestions: "AI task title suggestions",
	autoSuggestionsDescription:
		"Send typed task titles to the configured AI provider and show reviewable suggestions. Off by default; each suggestion is applied only when selected.",
	autoPlanGeneration: "Auto-generate plan after saving",
	autoPlanGenerationDescription:
		"Start task plan generation after saving a task. On by default; disable to require manual Regenerate.",
	defaultAutoExecute: "Default task auto-execution",
	defaultAutoExecuteDescription:
		"Preselect auto-execute when creating scheduled tasks. Off by default so tasks only run automatically after opt-in.",
...
export function ScheduleAiSettingsPanel({
	...
	const [savingKey, setSavingKey] = useState<
		keyof ScheduleAiPreferences | null
	>(null);
	const clearSavingTimerRef = useRef<number | null>(null);

	useEffect(
		() => () => {
			if (clearSavingTimerRef.current !== null)
				window.clearTimeout(clearSavingTimerRef.current);
		},
		[],
	);

	const updatePreference = (
		key: keyof ScheduleAiPreferences,
		value: boolean,
	) => {
		setSavingKey(key);
	...
	return (
		<Card role="region" aria-labelledby="schedule-ai-settings-title">
			<CardHeader>
			...
			<CardContent>
				<FieldGroup className="gap-3">
					<PreferenceToggle
						...
						copy={copy}
						onChange={(checked) =>
							updatePreference("autoSuggestionsEnabled", checked)
						}
					/>
					<PreferenceToggle
						...
						copy={copy}
						onChange={(checked) =>
							updatePreference("autoPlanGenerationEnabled", checked)
						}
					/>
					<PreferenceToggle
						...
						copy={copy}
						onChange={(checked) =>
							updatePreference("defaultAutoExecuteEnabled", checked)
						}
					/>
				</FieldGroup>
			</CardContent>
