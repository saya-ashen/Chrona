...
import type { Prisma } from "@chrona/db/generated/prisma/client";
...
async function fetchDashboard(workspaceId: string): Promise<DashboardBody> {
	const res = await app().request(
		`http://local/api/dashboard?workspaceId=${workspaceId}`,
	);
			...
			planRun: {
				...
				mutableGraph: {
					planOutput: {
						spec: {
							root: "root",
							elements: { root: { type: "Text", props: { text: summary } } },
						},
						revision: 1,
...
describe("GET /api/dashboard", () => {
	...
	it("derives attention kind, reason, and next step per bucket", async () => {
		...
		const { taskId: blocked } = await seedTask(workspaceId, {
			title: "Blocked",
			status: "Blocked",
		});
		...
		const { taskId: approval } = await seedTask(workspaceId, {
			title: "Approval",
			status: "WaitingForApproval",
		});
		await seedProjection(workspaceId, approval, {
		...
		const byTask = new Map(
			body.needsAttention.map((item) => [item.taskId, item]),
		);
		expect(byTask.get(blocked)?.kind).toBe("blocked");
		expect(byTask.get(blocked)?.reason).toBe(
			"Needs confirmation to keep scraping data",
		);
	...
	it("picks the highest-impact non-terminal task as the focus headline", async () => {
		...
		const { taskId: idle } = await seedTask(workspaceId, {
			title: "Idle low",
			status: "Ready",
			priority: "Low",
		});
		await seedProjection(workspaceId, idle, { persistedStatus: "Ready" });
		const { taskId: failing } = await seedTask(workspaceId, {
			title: "Failing urgent",
			status: "Failed",
			priority: "Urgent",
		});
		await seedProjection(workspaceId, failing, {
			persistedStatus: "Failed",
			blockDetail: "GitHub API request failed",
		});
	...
	it("lists auto-completed tasks newest-first with their outputs", async () => {
		...
		const { taskId: older } = await seedTask(workspaceId, {
			title: "Older done",
			status: "Completed",
		});
		...
		const { taskId: newer } = await seedTask(workspaceId, {
			title: "Newer done",
			status: "Done",
		});
		...
		const body = await fetchDashboard(workspaceId);
		expect(body.autoCompleted.map((item) => item.taskId)).toEqual([
			newer,
			older,
		]);
		expect(body.autoCompleted[0]?.output?.title).toBe(
	...
	it("surfaces only readable, task-scoped events in the recent feed", async () => {
		...
		await seedEvent(
			...
			"Started safely",
			{
				providerRequest: { prompt: "private provider prompt" },
				apiKey: "secret-value",
			},
		);
		...
		expect(
			body.recentEvents.every(
				(event) => event.taskTitle === "GitHub trendings",
			),
		).toBe(true);
		expect(body.recentEvents.map((event) => event.category).sort()).toEqual([
			"plan",
			"started",
		]);
		expect(JSON.stringify(body.recentEvents)).not.toContain(
			"private provider prompt",
		);
		expect(JSON.stringify(body.recentEvents)).not.toContain("secret-value");
		expect(JSON.stringify(body.recentEvents)).not.toContain("providerRequest");
	});
});
