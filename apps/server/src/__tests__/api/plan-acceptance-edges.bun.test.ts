...
describe("plan acceptance edges", () => {
  ...
  it("accepting the same plan twice is idempotent — second call returns 200, no error", async () => {
    ...
    const planRows = await db.taskPlan.findMany({ where: { taskId: task.id, planId: "idem-plan" } });
    expect(planRows).toHaveLength(1);
  });

  it("accepting a non-existent plan returns 404", async () => {
  ...
  it("stopping a generation with no active session returns stopped:false without error", async () => {
    ...
    const response = await app().request(`http://local/api/tasks/${task.id}/plan/generations/stop`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("GET /plan/generations/active returns well-formed response on a fresh task", async () => {
    ...
    const response = await app().request(`http://local/api/tasks/${task.id}/plan/generations/active?workspaceId=${workspaceId}`);
