...
describe("Recurring task lifecycle", () => {
	...
	it("creates a daily series with N work-block occurrences", async () => {
		...
		expect(
			blocks.map((b: { recurrenceKey: string | null }) => b.recurrenceKey),
		).toEqual([
		...
		expect(
			blocks.every((b: { status: string }) => b.status === "Scheduled"),
		).toBe(true);
		expect(
			blocks.every((b: { title: string }) => b.title === "Daily standup"),
		).toBe(true);
	});
	...
	it("expands a weekly series bounded by COUNT and skips past occurrences", async () => {
		...
		const secondPass = await runRecurringWorkBlockExpansionWorker({
			now: anchor,
		});
	...
	it("isolates a failed execution on occurrence A from occurrence B's header", async () => {
		...
		const bodyB = (await headerB.json()) as {
	...
	it("expands a monthly series bound by UNTIL and respects the upper bound", async () => {
		...
		expect(
			blocks.map((b: { scheduledStartAt: Date }) =>
