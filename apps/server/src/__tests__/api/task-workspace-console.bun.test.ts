...
describe("task workspace console read data", () => {
	...
	it("returns approvals and artifacts needed by human-review workspace cards", async () => {
		...
		expect(page.approvals).toContainEqual(
		...
		expect(page.artifacts).toContainEqual(
			expect.objectContaining({
				title: "Generated patch",
				type: "patch",
				uri: "file://patch.diff",
	...
	it("returns command center Trail items from persisted database activity", async () => {
		const { workspaceId } = await seedWorkspace(
			"Workspace Command Center Trail Activity",
		);
		const { taskId } = await seedTask(workspaceId, {
			title: "Render command center trail",
		});
			...
			data: [
				{
					...
					payload: {
						generation_id: "generation-command-center",
					...
					state?: {
						trail?: {
							items?: Array<{
								title?: string;
								description?: string;
								activityGroup?: { kind?: string; id?: string };
							}>;
						};
					};
				};
	...
	it("returns persisted provider runtime activity for the workspace activity timeline", async () => {
		...
		const { taskId } = await seedTask(workspaceId, {
			title: "Stream provider activity",
		});
		const run = await db.run.create({
			data: {
				taskId,
	...
	it("404s the header endpoint for unknown task ids", async () => {
		const response = await app().request(
			"/api/tasks/missing-task/workspace/header",
		);
