...
describe("Goal API", () => {
	...
	it("atomically and idempotently creates a direct Goal with its first bounded Task", async () => {
		...
		const command = {
			...
			firstTaskTitle: "Draft the launch brief",
			additionalContext: "Start with evidence",
			priority: "High",
			idempotencyKey: "direct-goal-entry-1",
		};

		const first = await responseJson<GoalTaskResponse>(
			await requestJson(app, "/goals/with-first-task", command),
		);
		const second = await responseJson<GoalTaskResponse>(
			await requestJson(app, "/goals/with-first-task", command),
		);
		...
		expect(
			await db.task.count({ where: { workspaceId, goalId: first.goal.id } }),
		).toBe(1);
		const createdGoal = await db.goal.findUniqueOrThrow({
		...
		expect(
			await db.taskOccurrence.count({ where: { taskId: first.taskId } }),
		).toBe(1);
		const createdTask = await db.task.findUniqueOrThrow({
			where: { id: first.taskId },
			include: { sessions: true, projection: true },
		});
		...
		expect(createdTask.sessions[0]?.sessionKey).toBe(
			`chrona:task:${first.taskId}:default`,
		);
		expect(createdTask.projection).not.toBeNull();
		expect(
			await db.event.count({
				where: { taskId: first.taskId, eventType: "task.created" },
			}),
		).toBe(1);
	});
	...
	it("coalesces concurrent retries into one Goal and first Task", async () => {
		...
		const command = {
			workspaceId,
			title: "One durable outcome",
			firstTaskTitle: "One bounded Task",
			idempotencyKey: "concurrent-direct-goal",
		};

		const responses = await Promise.all([
			requestJson(app, "/goals/with-first-task", command),
			requestJson(app, "/goals/with-first-task", command),
		]);
		expect(responses.map((response) => response.status)).toEqual([201, 201]);
		const payloads = await Promise.all(
			responses.map((response) => responseJson<GoalTaskResponse>(response)),
		);
	...
	it("rolls back the Goal when canonical Task validation fails", async () => {
		...
		const response = await requestJson(app, "/goals/with-first-task", {
			workspaceId,
			title: "Never persist partially",
			firstTaskTitle: "   ",
			idempotencyKey: "invalid-direct-goal",
		});
		...
		expect(
			await db.event.count({
				where: {
					dedupeKey: "goal.created_with_first_task:invalid-direct-goal",
				},
			}),
		).toBe(0);
	});
	...
	it("creates, reads, pauses, resumes, and explicitly achieves a Goal", async () => {
		...
		const paused = await responseJson<GoalData>(
			await requestJson(app, `/goals/${created.id}/actions`, {
				action: "pause",
			}),
		);
		...
		const resumed = await responseJson<GoalData>(
			await requestJson(app, `/goals/${created.id}/actions`, {
				action: "resume",
			}),
		);
		...
		const unconfirmedResponse = await requestJson(
			app,
			`/goals/${created.id}/actions`,
			{
				action: "achieve",
		...
		const confirmedResponse = await requestJson(
			app,
			`/goals/${created.id}/criteria/outcome/confirm`,
			{
		...
		const achieved = await responseJson<GoalData>(
			await requestJson(app, `/goals/${created.id}/actions`, {
				action: "achieve",
		...
		expect(
			await db.event.count({
				where: { eventType: "goal.achieved", workspaceId },
			}),
		).toBe(1);
	});
	...
	it("creates Goal review work as a bounded task", async () => {
		...
		const app = createApiRouter(createChronaEngine());
		const created = await responseJson<GoalData>(
			await requestJson(app, "/goals", {
				workspaceId,
		...
		expect(
			(await db.task.findUniqueOrThrow({ where: { id: body.taskId } })).goalId,
		).toBe(created.id);
		expect(
			await db.event.count({
				where: { eventType: "goal.review_task_created", taskId: body.taskId },
			}),
		).toBe(1);
		const read = await responseJson<GoalData>(
			await app.request(`/goals/${created.id}`),
		);
	...
	it("targets the planned Task when active Goal work can continue", async () => {
		...
		const taskResponse = await responseJson<GoalTaskResponse>(
			await requestJson(app, `/goals/${created.id}/tasks`, {
		...
		expect(taskResponse.goal.primaryAction).toEqual({
			kind: "continue_work",
	...
	it("persists a Goal brief and immutable accepted-result context automatically", async () => {
		...
		const created = await responseJson<GoalData>(
			await requestJson(app, "/goals", {
				...
				description:
					"Build an evidence-backed application over several bounded steps.",
				successCriteria: [
					{
						id: "submitted",
						kind: "user_confirmed",
		...
		const sourceTask = await responseJson<GoalTaskResponse>(
			await requestJson(app, `/goals/${created.id}/tasks`, {
				kind: "task",
		...
		const nextTask = await responseJson<GoalTaskResponse>(
			await requestJson(app, `/goals/${created.id}/tasks`, {
				kind: "task",
		...
		const formalAssets =
			persisted.goalContext &&
			typeof persisted.goalContext === "object" &&
			!Array.isArray(persisted.goalContext) &&
			"assets" in persisted.goalContext
				? persisted.goalContext.assets
				: null;
		const firstAsset = Array.isArray(formalAssets) ? formalAssets[0] : null;
		const formalAssetRef =
			firstAsset && typeof firstAsset === "object" && "ref" in firstAsset
				? firstAsset.ref
				: null;
		...
		const exactGoalResult =
			await createChronaEngine().goals.readAcceptedResults({
				taskId: nextTask.taskId,
				workspaceId,
		...
		await db.goalAssetVersion.create({
			data: {
				...
				version: 2,
				parentVersionId: (
					await db.goalAssetVersion.findFirstOrThrow({
						where: { assetId: formalAsset.id, version: 1 },
					})
				).id,
				source: "manual",
		...
		const taskAfterPublication = await responseJson<GoalTaskResponse>(
			await requestJson(app, `/goals/${created.id}/tasks`, {
				kind: "task",
		...
		const futureContext = (
			await db.task.findUniqueOrThrow({
				where: { id: taskAfterPublication.taskId },
			})
		).goalContext as { assets?: unknown[] } | null;
		expect(futureContext?.assets).toEqual([]);
		await expect(
			createChronaEngine().goals.readAcceptedResults({
		...
		const acceptedResults =
			...
			typeof persisted.goalContext === "object" &&
			!Array.isArray(persisted.goalContext) &&
			"acceptedResults" in persisted.goalContext
				? persisted.goalContext.acceptedResults
				: null;
		expect(Array.isArray(acceptedResults)).toBe(true);
		const firstAcceptedResult = Array.isArray(acceptedResults)
			? acceptedResults[0]
			: null;
		const acceptedResultRef =
			firstAcceptedResult &&
			typeof firstAcceptedResult === "object" &&
			"ref" in firstAcceptedResult
		...
		expect(bootstrap.task.goal).toEqual({
			id: created.id,
			title: created.title,
		});
	});

	it("[GOAL-023] saves immutable operational brief revisions and updates the current brief", async () => {
		const { workspaceId } = await seedWorkspace("Goal brief revisions");
		const app = createApiRouter(createChronaEngine());
		const goal = await responseJson<GoalData>(
			await requestJson(app, "/goals", {
				workspaceId,
				title: "Ship a durable outcome",
				successCriteria: [criterion],
			}),
		);
		const firstBrief = {
			outcome: "Ship version one",
			currentFocus: "Validate scope",
			strategy: "Use accepted evidence",
			constraints: ["Keep changes bounded"],
		};
		const secondBrief = { ...firstBrief, currentFocus: "Ship approved scope" };

		for (const brief of [firstBrief, secondBrief]) {
			const response = await app.request(`/goals/${goal.id}/brief`, {
				method: "PUT",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ brief }),
			});
		...
		const storedGoal = await db.goal.findUniqueOrThrow({
			where: { id: goal.id },
		});
		const revisions = await db.goalBriefRevision.findMany({
			where: { goalId: goal.id },
			orderBy: { createdAt: "asc" },
		});
		expect(storedGoal.operationalBrief).toEqual(secondBrief);
		expect(revisions).toHaveLength(2);
		expect(revisions[0]?.brief).toEqual(firstBrief);
		expect(revisions[1]?.brief).toEqual(secondBrief);
	});

	it("[GOAL-024] updates criteria and next review time without changing execution history", async () => {
		const { workspaceId } = await seedWorkspace("Goal metadata update");
		const app = createApiRouter(createChronaEngine());
		const goal = await responseJson<GoalData>(
			await requestJson(app, "/goals", {
				workspaceId,
				title: "Review durable metadata",
				successCriteria: [criterion],
			}),
		);
		const task = await responseJson<GoalTaskResponse>(
			await requestJson(app, `/goals/${goal.id}/tasks`, {
				kind: "task",
				title: "Preserve execution history",
				priority: "Medium",
				autoPlanGeneration: false,
			}),
		);
		const { run } = await seedAcceptedResult(workspaceId, task.taskId);
		const nextReviewAt = "2030-05-01T09:00:00.000Z";
		const updatedCriteria = [
			{ ...criterion, description: "Updated outcome criterion" },
		];

		const response = await app.request(`/goals/${goal.id}`, {
			method: "PATCH",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({ successCriteria: updatedCriteria, nextReviewAt }),
		});
		expect(response.status).toBe(200);

		const updated = await responseJson<GoalData>(response);
		expect(updated.successCriteria).toMatchObject(updatedCriteria);
		...
		).toEqual([{ id: run.id, status: "Completed" }]);
		expect(
			(await db.task.findUniqueOrThrow({ where: { id: task.taskId } })).goalId,
		).toBe(goal.id);
	});
	...
	it("captures every active formal asset with bounded metadata", async () => {
		...
		const app = createApiRouter(createChronaEngine());
		const created = await responseJson<GoalTaskResponse>(
			await requestJson(app, "/goals/with-first-task", {
				workspaceId,
		...
		const seededSource = await seedTask(workspaceId, {
			title: "Create source artifacts",
			status: "Completed",
		});
		await db.task.update({
			where: { id: seededSource.taskId },
			data: { goalId: created.goal.id },
		});
		const sourceTask = await db.task.findUniqueOrThrow({
			where: { id: seededSource.taskId },
		});
		const run = await db.run.create({
			data: {
		...
		for (let index = 0; index < 50; index += 1) {
			...
			const asset = await db.goalAsset.create({
				data: {
					...
					label:
						index === 0 ? `Title ${"t".repeat(220)}` : `Reference ${index}`,
					description:
						index === 0
							? `Description ${"d".repeat(500)}`
							: `Description ${index}`,
		...
		const task = await responseJson<GoalTaskResponse>(
			await requestJson(app, `/goals/${created.goal.id}/tasks`, {
				kind: "task",
		...
		const context = persisted.goalContext as {
			assets?: Array<Record<string, unknown>>;
		} | null;
		expect(context?.assets).toHaveLength(49);
		const longest = context?.assets?.find(
			(asset) =>
				typeof asset.title === "string" && asset.title.startsWith("Title "),
		);
		expect(longest?.title).toHaveLength(160);
		expect(longest?.description).toHaveLength(400);
		expect(
			context?.assets?.every((asset) => typeof asset.version === "number"),
		).toBe(true);
	...
	it("atomically promotes one accepted result and is idempotent", async () => {
		...
		expect(
			(await db.task.findUniqueOrThrow({ where: { id: taskId } })).goalId,
		).toBe(firstBody.id);
		expect(
			await db.artifact.count({
				where: { id: artifact.id, contentPreview: "Immutable final result" },
			}),
		).toBe(1);
	});
	...
	it("processes accepted results, confirms criterion evidence, and applies a structured review", async () => {
		...
		const extracted = await responseJson<{ candidates: Array<{ id: string }> }>(
			await requestJson(app, `/goals/${goal.id}/inbox/extract`, {
				taskId,
		...
		await responseJson(
			await requestJson(
				app,
				`/goals/${goal.id}/inbox/${extracted.candidates[0]!.id}/resolve`,
				{
					workspaceId,
					action: "create_asset",
		...
		const processed = await responseJson<GoalData>(
			await requestJson(app, `/goals/${goal.id}/results/${taskId}/process`, {
		...
		const confirmed = await responseJson<GoalData>(
			await requestJson(app, `/goals/${goal.id}/criteria/outcome/confirm`, {
				artifactIds: [accepted.artifact.id],
				note: "Accepted evidence proves the outcome.",
			}),
		);
		expect(confirmed.outcome.criteria[0]).toMatchObject({
			satisfied: true,
			evidenceArtifactIds: [accepted.artifact.id],
		});

		const reviewGoal = await db.goal.findUniqueOrThrow({
			where: { id: goal.id },
			include: {
		...
		const taskDependency = {
			kind: "task_candidate",
			goalUpdatedAt: reviewGoal.updatedAt.toISOString(),
			tasks: reviewGoal.tasks
				.map((task: { id: string; status: string; updatedAt: Date }) => ({
					id: task.id,
					status: task.status,
				...
				.sort(
					(
						left: { id: string; status: string; updatedAt: string },
						right: { id: string; status: string; updatedAt: string },
		...
		const proposal = await db.goalReviewProposal.create({
			data: {
				...
				status: "Ready",
				inputSnapshot: {},
				inputSnapshotHash: "review-proposal-test-snapshot",
				requestIdempotencyKey: "goal-progression-review-request",
				items: {
					create: [
						{
							workspaceId,
							...
							itemId: "final-focus",
							kind: "brief_field",
							payload: { field: "currentFocus", value: "Final verification" },
							rationale: "Keep the Goal focused on final verification.",
							evidenceRefs: [],
							warnings: [],
							dependencySnapshot: {
								kind: "brief_field",
						...
						},
						{
							workspaceId,
							goalId: goal.id,
							itemId: "verify-package",
							kind: "task_candidate",
							payload: {
								title: "Verify final package",
								description: "Check the retained evidence.",
								expectedOutcome: "A verified final package",
							},
							rationale: "A bounded verification task completes the review.",
							evidenceRefs: [],
							warnings: [],
							dependencySnapshot: taskDependency,
							dependencyHash: stableJsonHash(taskDependency),
						},
					],
				},
			},
		...
		const reviewed = await requestJson(
			app,
			`/goals/${goal.id}/review-proposals/${proposal.id}/apply`,
			{
				idempotencyKey: "goal-progression-review-apply",
				expectedVersion: proposal.stateVersion,
				expectedGoalUpdatedAt: reviewGoal.updatedAt.toISOString(),
				dependencyHashes: Object.fromEntries(
					proposal.items.map(
						(item: { itemId: string; dependencyHash: string }) => [
							item.itemId,
							item.dependencyHash,
						],
					),
				...
				decisions: [
					{ itemId: "final-focus", action: "accept" },
					{ itemId: "verify-package", action: "accept" },
				],
			},
		);
		expect(reviewed.status).toBe(200);
		const refreshed = await responseJson<GoalData>(
			await app.request(`/goals/${goal.id}`),
		);
		expect(refreshed.workbench.brief?.currentFocus).toBe("Final verification");
		expect(
			refreshed.tasks.some((task) => task.title === "Verify final package"),
		).toBe(true);
		expect(
			await db.event.count({
				where: { eventType: "goal.review_proposal_applied", workspaceId },
			}),
		).toBe(1);
	});

	it("applies an itemized Goal Review Proposal through the proposal-scoped contract", async () => {
		const { workspaceId } = await seedWorkspace("Goal review proposal");
		const app = createApiRouter(createChronaEngine());
		const goal = await responseJson<GoalData>(
			await requestJson(app, "/goals", {
				workspaceId,
		...
		const currentGoal = await db.goal.findUniqueOrThrow({
			where: { id: goal.id },
		});
		const dependency = {
			kind: "brief_field",
		...
		const proposal = await db.goalReviewProposal.create({
			data: {
				...
				goalId: goal.id,
				status: "Ready",
				inputSnapshot: { mode: "progress" },
				inputSnapshotHash: "proposal-scoped-review-snapshot",
				requestIdempotencyKey: "proposal-scoped-review-request",
				items: {
					create: {
						workspaceId,
						goalId: goal.id,
						...
						rationale: "Focus the next bounded outcome.",
						evidenceRefs: [],
						warnings: [],
						dependencySnapshot: dependency,
						dependencyHash: stableJsonHash(dependency),
					},
				},
			},
			include: { items: true },
		...
		const applyCommand = {
			...
			expectedVersion: proposal.stateVersion,
			expectedGoalUpdatedAt: currentGoal.updatedAt.toISOString(),
			dependencyHashes: Object.fromEntries(
				proposal.items.map(
					(item: { itemId: string; dependencyHash: string }) => [
						item.itemId,
		...
		expect(refreshed.workbench.brief?.currentFocus).toBe(
			"Review the next bounded outcome",
		);
		expect(refreshed.reviewProposals[0]?.status).toBe("Applied");
		expect(refreshed.reviewProposals[0]?.items[0]?.decision).toBe("Accepted");
		expect(refreshed.reviewProposals[0]?.version).toBe(
			(
				await db.goalReviewProposal.findUniqueOrThrow({
		...
		expect(
			await db.goalBriefRevision.count({ where: { goalId: goal.id } }),
		).toBe(1);
	});
	...
	it("marks conflicting Goal Review items stale without overwriting current state", async () => {
		...
		await db.goal.update({
			...
			data: {
				operationalBrief: {
					outcome: "Outcome",
					currentFocus: "User changed focus",
					strategy: "Strategy",
					constraints: [],
				},
			},
		...
		const response = await requestJson(
			...
			`/goals/${goal.id}/review-proposals/${proposal.id}/apply`,
			{
				...
				expectedVersion: proposal.stateVersion,
				expectedGoalUpdatedAt: currentGoal.updatedAt.toISOString(),
				dependencyHashes: { "focus-change": "sha256:not-current" },
		...
		expect(storedGoal.operationalBrief).toMatchObject({
			currentFocus: "User changed focus",
		});
		const item = await db.goalReviewProposalItem.findFirstOrThrow({
	...
	it("rejects criterion evidence until every selected Artifact is formalized in the Inbox", async () => {
		...
		const { taskId } = await seedTask(workspaceId, {
			title: "Produce pending evidence",
			status: "Completed",
		});
		const accepted = await seedAcceptedResult(workspaceId, taskId);
		const app = createApiRouter(createChronaEngine());
		...
		const response = await requestJson(
			app,
			`/goals/${goal.id}/results/${taskId}/process`,
			{
				artifactIds: [accepted.artifact.id],
				criterionId: "outcome",
			},
		...
		expect(
			await db.event.count({
				where: { eventType: "goal.result_processed", workspaceId },
			}),
		).toBe(0);
	});
	...
	it("keeps proposed criteria outside progress until explicit review", async () => {
		...
		const reviewed = await responseJson<GoalData>(
			await requestJson(app, `/goals/${goal.id}/criteria/outcome/review`, {
				description: "User-approved outcome definition",
			}),
		);
		expect(reviewed.outcome.criteria[0]).toMatchObject({
	...
	it("rolls back promotion when an artifact is not owned by the accepted result", async () => {
		...
		const accepted = await seedAcceptedResult(workspaceId, taskId);
		const { taskId: otherTaskId } = await seedTask(workspaceId, {
			title: "Other task",
			status: "Completed",
		});
	...
	it("reads only accepted results scoped through the current Goal Task", async () => {
		...
		const otherWorkspace = await seedWorkspace(
			"Isolated Goal result retrieval",
		);
		...
		const rejectedTask = await seedTask(workspaceId, {
			title: "Rejected evidence",
			status: "Completed",
		});
		await db.task.update({
			where: { id: rejectedTask.taskId },
			data: { goalId: created.goal.id },
		});
		await db.run.create({
			data: {
		...
		const firstPage = await engine.goals.readAcceptedResults({
		...
		expect(
			acceptedRead && "artifacts" in acceptedRead
				? acceptedRead.artifacts[0]
				: null,
		).toEqual({
			title: "Accepted final result",
			type: "summary",
		...
		await expect(
			engine.goals.readAcceptedResults({
				taskId: reader.taskId,
				workspaceId: otherWorkspace.workspaceId,
				limit: 5,
			}),
		).rejects.toThrow("Task not found");
		const standalone = await seedTask(workspaceId, {
		...
		await expect(
			...
			linked: false,
			message: "Current Task is not linked to a Goal.",
			results: [],
			nextCursor: null,
		});
		await expect(
