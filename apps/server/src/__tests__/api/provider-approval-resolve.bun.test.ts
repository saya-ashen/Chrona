...
import { aiClientRegistry, createChronaEngine, stableJsonHash } from "@chrona/engine";
...
function approvalScope(approval: { workBlockId: string | null; executionScope: string }) {
  if (!approval.workBlockId) throw new Error("Expected approval work block scope");
  return { workBlockId: approval.workBlockId, executionScope: approval.executionScope };
}

function approvalQuery(approval: { workBlockId: string | null; executionScope: string }, status: "pending" | "all" = "pending") {
  return new URLSearchParams({ ...approvalScope(approval), status }).toString();
}

async function seedPendingApproval(workspaceId: string, taskId: string, options: { taskScoped?: boolean } = {}) {
  ...
  const workBlock = options.taskScoped ? null : await db.workBlock.create({
    data: {
      workspaceId,
      taskId,
  ...
  await db.executionSession.updateMany({
    ...
    data: { status: "Abandoned", activeScopeKey: null, completedAt: new Date() },
  });
  ...
  return { approval: { ...approval, executionScope: planRun.executionScopeId }, planRun, providerRun, runtimeRun, workBlock };
}

async function resolveApproval(
  taskId: string,
  approval: { id: string; workBlockId: string | null; executionScope: string },
  choice: "approve_once" | "approve_session" | "approve_always" | "deny" = "approve_once",
  ...
  return app().request(
    `http://local/api/tasks/${taskId}/provider-approvals/${approval.id}/resolve`,
    {
      ...
      body: JSON.stringify({ workBlockId: approval.workBlockId, executionScope: approval.executionScope, choice, idempotencyKey }),
    },
  );
}
...
function setMockFetch(handler: MockFetchHandler) {
  globalThis.fetch = Object.assign(handler, { preconnect: realFetch.preconnect }) as typeof fetch;
}
...
describe("provider approval resolve", () => {
  ...
  it("GET /provider-approvals requires a selected occurrence scope", async () => {
    ...
    const missingScope = await app().request(`http://local/api/tasks/${taskId}/provider-approvals?status=pending`);
    expect(missingScope.status).toBe(400);

    const response = await app().request(`http://local/api/tasks/${taskId}/provider-approvals?${approvalQuery(approval)}`);
  ...
  it("status=all returns resolved approvals; status=pending does not", async () => {
    ...
    const publicApproval = allBody.approvals.find((item) => item.id === approval.id);
  ...
  it("keeps listing, timeout reconciliation, and resolution within the selected occurrence", async () => {
    ...
    expect((await listed.json() as { approvals: { id: string }[] }).approvals.map((approval) => approval.id)).toEqual([second.approval.id]);

    const firstAfterListingSecond = await db.taskPlanProviderApproval.findUniqueOrThrow({ where: { id: first.approval.id } });
    ...
    expect(await db.taskPlanProviderApproval.findUniqueOrThrow({ where: { id: first.approval.id } })).toMatchObject({ status: "pending" });
    expect(await db.taskPlanProviderApproval.findUniqueOrThrow({ where: { id: second.approval.id } })).toMatchObject({ status: "pending" });
  });
  it("resolve on a missing approvalId returns 404", async () => {
    ...
    const response = await app().request(
      `http://local/api/tasks/${taskId}/provider-approvals/nope/resolve`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...approvalScope(approval), choice: "approve_once", idempotencyKey: crypto.randomUUID() }),
      },
    );
  ...
  it("resolve with inactive provider marks approval failed and clears pending list", async () => {
    ...
    const pending = await app().request(`http://local/api/tasks/${taskId}/provider-approvals?${approvalQuery(approval)}`);
  ...
  it("claims same-key concurrent resolves so provider RPC runs once", async () => {
    ...
    const second = resolveApproval(taskId, approval, "approve_once", "same-key");
    release.resolve();
    ...
    expect(statuses.some((status) => status === "resolved" || status === "in_flight")).toBe(true);
  });
  ...
  it("serializes different idempotency keys for the same pending approval", async () => {
    ...
    const first = resolveApproval(taskId, approval, "approve_once", "first-key");
    await rpcEntered.promise;
    const second = await resolveApproval(taskId, approval, "deny", "second-key");

    expect(second.status).toBe(200);
    expect((await second.json() as { status: string }).status).toBe("in_flight");
    expect(rpcCount).toBe(1);
    release.resolve();
    expect((await (await first).json() as { status: string }).status).toBe("resolved");
    expect(rpcCount).toBe(1);
  });
  ...
  it("fails closed without replaying an expired approval RPC with an unknown outcome", async () => {
    ...
    const { taskId } = await seedTask(ws.workspaceId);
    const { approval, planRun, providerRun } = await seedPendingApproval(ws.workspaceId, taskId);
    ...
    const response = await resolveApproval(taskId, approval, "approve_once", resolutionKey);

    expect(response.status).toBe(200);
    expect((await response.json() as { status: string }).status).toBe("failed");
    expect(rpcCount).toBe(0);
    expect(await db.taskPlanProviderApprovalResolution.findUniqueOrThrow({ where: { id: receipt.id } })).toMatchObject({
      status: "failed",
      activeClaimKey: null,
      leaseOwner: null,
      leaseExpiresAt: null,
  ...
  it("replays same-key canonical result without another provider RPC", async () => {
    ...
    const first = await resolveApproval(taskId, approval, "approve_once", "replay-key");
    const second = await resolveApproval(taskId, approval, "approve_once", "replay-key");

    expect((await first.json() as { status: string }).status).toBe("resolved");
    expect((await second.json() as { status: string }).status).toBe("resolved");
    expect(rpcCount).toBe(1);
    const receipt = await db.taskPlanProviderApprovalResolution.findFirstOrThrow({ where: { approvalId: approval.id } });
  ...
  it("rejects same-key different digest before provider RPC", async () => {
    ...
    const first = await resolveApproval(taskId, approval, "approve_once", "digest-key");
    const second = await resolveApproval(taskId, approval, "deny", "digest-key");
  ...
  it("does not call provider for obsolete approval scope", async () => {
    ...
    const response = await resolveApproval(taskId, approval, "approve_once", "obsolete-key");
  ...
  it("fails closed when the persisted provider client configuration drifts", async () => {
    await seedHermesApprovalClient();
    if (!approvalClientIdentity) throw new Error("Expected approval client identity");
    ...
    const response = await resolveApproval(taskId, approval, "approve_once", "config-drift-key");
  ...
  it("resolves task-scoped null workBlock approvals", async () => {
    ...
    const { taskId } = await seedTask(ws.workspaceId);
    const { approval, providerRun } = await seedPendingApproval(ws.workspaceId, taskId, { taskScoped: true });
    setMockFetch(async () => jsonProviderResponse({ resolved: 1 }));

    const response = await resolveApproval(taskId, approval, "approve_session", "task-scope-key");
    ...
    expect(updatedApproval).toMatchObject({ status: "approved", choice: "approve_session" });
    expect(updatedProviderRun.status).toBe("running");
  });
});
