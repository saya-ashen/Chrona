...
const realGetAiClientForFeature =
	aiClientRegistry.getForFeature.bind(aiClientRegistry);
		...
		nodes: [
			{
				...
				config: {
					expectedOutput: "Single provider result",
	...
	return {
		...
		nodes: [
			...
			},
			{
				...
				config: {
					expectedOutput: "Approved branch output",
				} satisfies TaskConfig,
...
function runtimeInputFromRequest(
	request: Parameters<AgentProviderClient["startRun"]>[0],
): RuntimeInput {
...
function structuredPayloadForRequest(
	request: Parameters<AgentProviderClient["startRun"]>[0],
) {
	...
	if (runtimeInput.node?.type !== "condition") {
		return {
			...
			summary: outputForNode(String(runtimeInput.node?.title ?? "Node")),
		};
	}

	const branch =
		runtimeInput.branchOptions?.find((option) => option.label === "approve") ??
...
function createScriptedProviderClient() {
	...
	const requestsByRunId = new Map<
		string,
		Parameters<AgentProviderClient["startRun"]>[0]
	>();
	let runCounter = 0;
	...
	const client: AgentProviderClient = {
		...
		async startRun(request): Promise<ProviderRunRef> {
			...
			if (typeof runtimeInput.node?.title === "string")
				calls.nodeTitles.push(runtimeInput.node.title);
		...
		async *streamRun(request): AsyncIterable<ProviderRunEvent> {
			...
			yield {
				...
				sequence: 3,
				timestamp,
				toolName: startRequest.terminalToolName,
			};
			yield {
				type: "run_completed",
				provider: "hermes",
...
async function postExecutionAction(
	...
	const response = await server.request(
		`http://local/api/tasks/${taskId}/execution/actions`,
		{
			method: "POST",
			headers: {
			...
			body: JSON.stringify({
				idempotencyKey: `execution:${taskId}:${String(body.action ?? "action")}`,
				...body,
			}),
		},
	);
...
async function postCheckpointAction(
	...
	const response = await server.request(
		`http://local/api/tasks/${taskId}/execution/checkpoint/${checkpointId}/actions`,
		{
			...
			body: JSON.stringify({
				idempotencyKey: `checkpoint:${checkpointId}:${String(body.action ?? "action")}`,
				...body,
			}),
		},
	);
...
function expectRuntimeSseForNodes(events: SseEntry[], nodeIds: string[]) {
	...
	for (const nodeId of nodeIds) {
		...
		expect(nodeEvents).toContainEqual(
		...
		expect(JSON.stringify(nodeEvents)).toContain(
			"completed by module provider",
		);
	}
}
...
async function expectProviderEvents(taskId: string, nodeIds: string[]) {
	...
	expect(new Set(events.map((event: Event) => event.dedupeKey)).size).toBe(
		events.length,
	);
	...
	for (const nodeId of nodeIds) {
		...
		expect(nodeEvents.map((event: Event) => event.eventType)).toEqual(
		...
		expect(
			nodeEvents.every((event: Event) => event.nodeTitle && event.runId),
		).toBe(true);
	}
	...
	expect(typedAttempts).toHaveLength(nodeIds.length);
	expect(typedAttempts.map((attempt) => attempt.nodeId).toSorted()).toEqual(
		nodeIds.toSorted(),
	);
	expect(
		typedAttempts.every((attempt) => attempt.planRunId === planRun.id),
	).toBe(true);
	expect(
		typedAttempts.every(
			(attempt) => attempt.executionEpoch <= planRun.executionEpoch,
		),
	).toBe(true);
	...
	const scopedEvents = events.filter(
		(event: Event) => event.nodeId && nodeIds.includes(event.nodeId),
	);
	expect(
		scopedEvents.every(
			(event: Event) =>
...
async function expectNoProviderEventsForNode(taskId: string, nodeId: string) {
	const count = await db.event.count({
		where: { taskId, source: "provider", nodeId },
	});
	expect(count).toBe(0);
}

async function expectConversationHistory(
	taskId: string,
	expectedAssistantTexts: string[],
) {
	...
	expect(
		entries.filter((entry: ConversationEntry) => entry.role === "user").length,
	).toBeGreaterThanOrEqual(expectedAssistantTexts.length);
...
async function getCurrentExecution(server: Hono, taskId: string) {
	const response = await server.request(
		`http://local/api/tasks/${taskId}/execution/current`,
	);
	expect(response.status).toBe(200);
	return json<{
		status: string;
		...
		message?: string;
		ui?: { currentOperationSpec?: unknown | null };
	}>(response);
}

async function setupModuleExecutionTest(
	compiledPlan: CompiledPlan,
	title: string,
) {
...
describe("task execution module API integration", () => {
	...
	it("executes a single task through API and records provider events, SSE, history, and current state", async () => {
		...
		const events = await postExecutionAction(server, taskId, {
			action: "start_manual",
		});
		...
		await expectConversationHistory(taskId, [
			"Execute module task node completed by module provider",
		]);
	...
	it("does not duplicate execution when start is submitted twice", async () => {
		const { server, taskId, provider } = await setupModuleExecutionTest(
			makeSingleTaskPlan("module_duplicate_start"),
			"Module duplicate start",
		);

		const first = await postExecutionAction(server, taskId, {
			action: "start_manual",
		});
		expectExecutionSse(first, "completed");
		const callsAfterFirstStart = provider.calls.nodeTitles.length;
		const runsAfterFirstStart = await db.run.count({ where: { taskId } });

		await postExecutionAction(server, taskId, { action: "start_manual" });

		expect(provider.calls.nodeTitles).toHaveLength(callsAfterFirstStart);
		expect(await db.run.count({ where: { taskId } })).toBe(runsAfterFirstStart);
	});
	...
	it("executes independent entry tasks once each without duplicate provider starts", async () => {
		...
		expectExecutionSse(events, "completed");
		expect(
			provider.calls.nodeTitles.toSorted((left, right) =>
				left.localeCompare(right),
			),
		).toEqual(["Collect architecture facts", "Collect documentation facts"]);
	...
	it("routes AI condition branches by branchRef and does not call provider for skipped branches", async () => {
		...
		await expectProviderEvents(taskId, [
			"prepare_task",
			"route_condition",
			"approved_task",
		]);
	...
	it("pauses on input checkpoint then resumes through checkpoint API into provider-backed work", async () => {
		...
		const initialEvents = await postExecutionAction(server, taskId, {
			action: "start_manual",
		});
		...
		expect(currentBeforeInput).toMatchObject({
		...
		expect(currentBeforeInput.ui?.currentOperationSpec).toMatchObject({
			root: "root",
			elements: expect.any(Object),
		});

		const checkpointEvents = await postCheckpointAction(
			server,
			taskId,
			checkpointId!,
			{
		...
		expect(provider.calls.nodeTitles).toEqual([
			"Finalize checkpoint-driven specification",
		]);
		await expectProviderEvents(taskId, ["spec_task"]);
		await expectConversationHistory(taskId, [
			"Finalize checkpoint-driven specification completed by module provider",
		]);
		await expect(getCurrentExecution(server, taskId)).resolves.toMatchObject({
			status: "completed",
			currentNodeId: null,
