...
import {
	aiClientRegistry,
	createChronaEngine,
	waitForGoalAssetOwnershipGeneration,
} from "@chrona/engine";
...
async function seedGoalResult() {
	...
	const goal = await db.goal.create({
		data: {
			workspaceId,
			title: "Publish launch kit",
			description: "Durable launch assets",
			successCriteria: [],
			status: "Active",
		},
	});
...
async function seedStructuredGoalResultWithoutArtifacts() {
	...
	const spec = {
		...
		elements: {
			...
			timeline: {
				...
				props: {
					title: "Weekend",
					items: [{ label: "Day 1", title: "Arrive in 日照" }],
				},
			},
			checklist: {
				...
				props: {
					...
					items: [{ label: "Book hotel", status: "todo" }],
				},
			},
			changes: {
	...
	await db.event.create({
		data: {
			...
			source: "provider",
			payload: {},
			dedupeKey: `provider-completed:${run.id}`,
			ingestSequence: 1,
		},
	});
	await db.event.create({
...
describe("Goal Workbench API", () => {
	...
	it("extracts an accepted artifact candidate and formalizes an immutable version", async () => {
		...
		const { candidates } = (await extracted.json()) as {
			candidates: Array<{ id: string; kind: string; reason: string }>;
		};
		...
		const resolved = await post(
			app,
			`/goals/${seeded.goal.id}/inbox/${candidates[0]!.id}/resolve`,
			{
		...
		const assetsResponse = await app.request(
			`/goals/${seeded.goal.id}/assets?workspaceId=${seeded.workspaceId}`,
		);
		const { assets } = (await assetsResponse.json()) as {
			assets: Array<{
				kind: string;
				versions: Array<{ version: number; source: string }>;
			}>;
		};
		expect(assets[0]).toMatchObject({
			kind: "document",
			versions: [{ version: 1, source: "inbox" }],
		});
	});
	...
	it("automatically syncs accepted finalized results into a reviewable Inbox candidate", async () => {
		...
		expect(
			await db.goalAsset.count({ where: { goalId: seeded.goal.id } }),
		).toBe(0);
	});
	...
	it("preserves a structured accepted result as an immutable json-render asset", async () => {
		...
		const { candidates } = (await extracted.json()) as {
			candidates: Array<{
				id: string;
				kind: string;
				content: { format: string; summary: string; spec: unknown };
			}>;
		};
		expect(candidates[0]).toMatchObject({
			kind: "structured_result",
			content: {
				...
				summary: expect.stringContaining("日照＋临沂沂蒙山"),
				spec: { root: "workspace" },
			},
		});
		...
		expect(resolved.status).toBe(200);
		const version = await db.goalAssetVersion.findFirstOrThrow({
			where: { goalId: seeded.goal.id },
		});
		...
		expect(
			(
				await db.goalAsset.findFirstOrThrow({
					where: { goalId: seeded.goal.id },
				})
			).kind,
		...
		stored.mutableGraph.planOutput.finalizedResult.spec.elements.summary.props.text =
			"推荐青岛，新的最终结果。";
		...
		expect(
			await db.goalInboxCandidate.count({
				where: { goalId: seeded.goal.id, sourceRunId: seeded.run.id },
			}),
		).toBe(2);
		expect(
			await db.goalInboxCandidate.findMany({
				where: { goalId: seeded.goal.id, sourceRunId: seeded.run.id },
	...
	it("registers generated files and persists only opaque structured references", async () => {
		...
		spec.elements.summary = {
			type: "FileRef",
			props: { path: csvPath, title: "destination-shortlist.csv" },
		};
		...
		const structured = candidates.find(
			(candidate) => candidate.kind === "structured_result",
		)!;
		const fileCandidate = candidates.find(
			(candidate) => candidate.sourceArtifact?.id === artifact.id,
		)!;
		...
		const resolved = await post(
			app,
			`/goals/${seeded.goal.id}/inbox/${structured.id}/resolve`,
			{
				workspaceId: seeded.workspaceId,
				action: "create_asset",
				label: "Selected destination",
		...
		const assetResponse = await app.request(
			`/goals/${seeded.goal.id}/assets/${assetId}`,
		);
		expect(assetResponse.status).toBe(200);
		expect(await assetResponse.json()).toMatchObject({
			linkedAssets: [
				{
					ref: structured.content.artifactRefs![0]!.ref,
		...
		const download = await app.request(
			`/goals/${seeded.goal.id}/assets/${assetId}/artifacts/${structured.content.artifactRefs![0]!.ref}/download?versionId=${version.id}`,
		);
		expect(download.status).toBe(200);
		expect(await download.text()).toContain("日照＋临沂沂蒙山");
		await post(
			app,
			`/goals/${seeded.goal.id}/inbox/${fileCandidate.id}/resolve`,
			{
				workspaceId: seeded.workspaceId,
				action: "create_asset",
				label: fileCandidate.sourceArtifact!.id,
			},
		);
		expect(
			await db.goalAsset.count({
				where: { goalId: seeded.goal.id, sourceArtifactId: artifact.id },
			}),
		).toBe(1);
	});
	...
	it("accepts file-backed tables without persisting task-scoped download links", async () => {
		...
		await db.artifact.create({
			data: {
				...
				metadata: {
					checksum: createHash("sha256")
						.update(new Uint8Array(tableContent))
						.digest("hex"),
	...
	it("rejects execution controls in structured result assets", async () => {
		...
		stored.mutableGraph.planOutput.finalizedResult.spec.elements.summary = {
			...
			props: { title: "Run again", taskId: seeded.task.id },
			on: { click: { action: "dispatchExecution" } },
		};
		await db.taskPlanRun.update({
			where: { id: row.id },
			data: { planRun: stored },
		...
		const app = createApiRouter(createChronaEngine());
		const response = await post(app, `/goals/${seeded.goal.id}/inbox/extract`, {
			taskId: seeded.task.id,
			runId: seeded.run.id,
		});
		expect(response.status).toBe(400);
		expect(await response.json()).toMatchObject({
			error: expect.stringContaining("component is not allowed"),
		});
	});
	...
	it("exports structured result assets to Markdown, PDF, and JSON", async () => {
		...
		for (const format of ["md", "pdf", "json"] as const) {
			const exported = await post(
				app,
				`/goals/${seeded.goal.id}/assets/${assetId}/jobs`,
				{
					workspaceId: seeded.workspaceId,
			...
			expect(await exported.json()).toMatchObject({
				status: "Completed",
				format,
				outputUri: expect.stringContaining(`.${format}`),
			});
			const download = await app.request(
				`/goals/${seeded.goal.id}/assets/${assetId}/download?versionId=${version.id}&mode=export&format=${format}`,
			);
			expect(download.status).toBe(200);
			const body = await download.arrayBuffer();
			expect(body.byteLength).toBeGreaterThan(format === "pdf" ? 1_000 : 40);
			if (format === "md")
				expect(new TextDecoder().decode(body)).toContain("日照＋临沂沂蒙山");
		}
	}, 15_000);

	it("[ASSET-020] records failed exports and allows a clean retry without changing the asset", async () => {
		const seeded = await seedStructuredGoalResultWithoutArtifacts();
		const app = createApiRouter(createChronaEngine());
		const extracted = await post(
			app,
		...
		const originalAsset = await db.goalAsset.findUniqueOrThrow({
			where: { id: assetId },
		});
		...
		expect(
			await db.goalAssetJob.findFirstOrThrow({
				where: { assetId },
		...
		expect(
			await db.goalAsset.findUniqueOrThrow({ where: { id: assetId } }),
		).toEqual(originalAsset);
		expect(await db.goalAssetVersion.count({ where: { assetId } })).toBe(1);
		...
		expect(
			await db.goalAssetJob.count({ where: { assetId, status: "Failed" } }),
		).toBe(1);
		expect(
			await db.goalAssetJob.count({ where: { assetId, status: "Completed" } }),
		).toBe(1);
	}, 15_000);
	...
	it("generates a bounded AI ownership proposal and applies it only after user confirmation", async () => {
		...
		const aiClient = await db.aiClient.create({
			data: {
				name: "Asset ownership debug provider",
				type: "debug",
		...
		await db.aiFeatureBinding.create({
			data: { feature: "goal.asset_ownership", clientId: aiClient.id },
		});
		...
		const generated = await post(
			app,
			`/goals/${seeded.goal.id}/inbox/${candidateId}/ownership-proposals`,
			{
				workspaceId: seeded.workspaceId,
		...
		const proposal = await db.goalAssetOwnershipProposal.findUniqueOrThrow({
			where: { id: started.proposalId },
		});
		...
		const applied = await post(
			app,
			`/goals/${seeded.goal.id}/inbox/${candidateId}/ownership-proposals/${proposal.id}/apply`,
			{
				workspaceId: seeded.workspaceId,
		...
		expect(((await applied.json()) as { status: string }).status).toBe(
			"Applied",
		);
		expect(
			await db.goalAsset.count({ where: { goalId: seeded.goal.id } }),
	...
	it("marks an ownership proposal stale when the frozen candidate set changes", async () => {
		...
		const generated = await post(
			app,
			`/goals/${seeded.goal.id}/inbox/${candidateId}/ownership-proposals`,
			{ workspaceId: seeded.workspaceId, idempotencyKey: "stale-generate" },
		);
		const { proposalId } = (await generated.json()) as { proposalId: string };
		await waitForGoalAssetOwnershipGeneration(proposalId);

		await db.goalInboxCandidate.update({
			where: { id: candidateId },
			data: { label: "Renamed after review" },
		});
		const apply = await post(
			app,
			`/goals/${seeded.goal.id}/inbox/${candidateId}/ownership-proposals/${proposalId}/apply`,
			{
				workspaceId: seeded.workspaceId,
		...
		expect(
			(
				await db.goalAssetOwnershipProposal.findUniqueOrThrow({
					where: { id: proposalId },
				})
			).status,
		).toBe("Stale");
	...
	it("rejects resolving the same Inbox candidate twice without duplicate versions", async () => {
		...
		expect(
			await db.goalAssetVersion.count({ where: { goalId: seeded.goal.id } }),
		).toBe(1);
	});

	it("discards only the active draft for the requested asset", async () => {
		const seeded = await seedGoalResult();
		const firstAsset = await db.goalAsset.create({
			data: {
				workspaceId: seeded.workspaceId,
				goalId: seeded.goal.id,
				sourceArtifactId: seeded.artifact.id,
				currentArtifactId: seeded.artifact.id,
				kind: "document",
		...
		const secondArtifact = await db.artifact.create({
			data: {
				workspaceId: seeded.workspaceId,
				taskId: seeded.task.id,
				runId: seeded.run.id,
				type: "report",
				title: "Second brief source",
				uri: "generated://tests/second-brief.md",
				contentPreview: "formal two",
			},
		});
		const secondAsset = await db.goalAsset.create({
			data: {
				workspaceId: seeded.workspaceId,
				goalId: seeded.goal.id,
		...
		const firstVersion = await db.goalAssetVersion.create({
			data: {
				workspaceId: seeded.workspaceId,
				goalId: seeded.goal.id,
				...
				source: "inbox",
				content: "formal one",
				contentHash: "formal-one",
				authorType: "user",
			},
		});
		const secondVersion = await db.goalAssetVersion.create({
			data: {
				workspaceId: seeded.workspaceId,
				goalId: seeded.goal.id,
				...
				source: "inbox",
				content: "formal two",
				contentHash: "formal-two",
				authorType: "user",
			},
		});
		const firstDraft = await db.goalAssetDraft.create({
			data: {
				workspaceId: seeded.workspaceId,
				goalId: seeded.goal.id,
				assetId: firstAsset.id,
				...
				content: "draft one",
				contentHash: "draft-one",
				authorType: "user",
				authorId: "test",
			},
		});
		const secondDraft = await db.goalAssetDraft.create({
			data: {
				workspaceId: seeded.workspaceId,
				goalId: seeded.goal.id,
				assetId: secondAsset.id,
		...
		const discarded = await post(
			app,
			`/goals/${seeded.goal.id}/assets/${firstAsset.id}/drafts/discard`,
			{ workspaceId: seeded.workspaceId, draftId: firstDraft.id },
		);
		expect(discarded.status).toBe(200);
		expect(
			(
				await db.goalAssetDraft.findUniqueOrThrow({
	...
	it("detects optimistic conflicts and recovers old versions as new versions", async () => {
		...
		const first = await db.goalAssetVersion.create({
			data: {
				workspaceId: seeded.workspaceId,
				goalId: seeded.goal.id,
				assetId: asset.id,
		...
		const draftResponse = await post(
			app,
			`/goals/${seeded.goal.id}/assets/${asset.id}/drafts`,
			{
				workspaceId: seeded.workspaceId,
		...
		const conflicted = await post(
			app,
			`/goals/${seeded.goal.id}/assets/${asset.id}/drafts/submit`,
			{
				workspaceId: seeded.workspaceId,
				draftId: draft.id,
				changeSummary: "Publish two",
			},
		);
		expect(conflicted.status).toBe(409);
		expect(
			(await db.goalAssetDraft.findUniqueOrThrow({ where: { id: draft.id } }))
				.status,
		).toBe("Conflict");

		const restored = await post(
			app,
			`/goals/${seeded.goal.id}/assets/${asset.id}/versions/${first.id}/restore`,
			{ workspaceId: seeded.workspaceId, changeSummary: "Recover one" },
		);
		expect(restored.status).toBe(200);
		const recovered = (await restored.json()) as {
			version: number;
			source: string;
	...
	it("[ASSET-014] keeps asset identity and references stable while renaming list and detail projections", async () => {
		const seeded = await seedGoalResult();
		const asset = await db.goalAsset.create({
			data: {
				workspaceId: seeded.workspaceId,
		...
		const renamed = await app.request(
			`/goals/${seeded.goal.id}/assets/${asset.id}`,
			{
				method: "PATCH",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ label: "Renamed brief" }),
			},
		);
		...
		expect(
			await (
				await app.request(`/goals/${seeded.goal.id}/assets/${asset.id}`)
			).json(),
		).toMatchObject({
			id: asset.id,
			label: "Renamed brief",
			versions: [
				expect.objectContaining({ id: version.id, content: "Stable content" }),
			],
		});
		const listed = (await (
			await app.request(
				`/goals/${seeded.goal.id}/assets?workspaceId=${seeded.workspaceId}&state=active&sort=name_asc`,
			)
		).json()) as { assets: Array<{ id: string; label: string }> };
		expect(listed.assets).toContainEqual(
			expect.objectContaining({ id: asset.id, label: "Renamed brief" }),
		);
		expect(
			await db.goalAssetVersion.count({ where: { assetId: asset.id } }),
		).toBe(1);
	});
	...
	it("submits version-bound Forms, writes exports, and preserves archive history", async () => {
		...
		const version = await db.goalAssetVersion.create({
			data: {
				...
				version: 1,
				source: "inbox",
				content: { fields: [{ id: "name", type: "text", required: true }] },
				contentHash: "form-v1",
				authorType: "user",
			},
		});
		...
		const submission = await post(
			app,
			`/goals/${seeded.goal.id}/assets/${asset.id}/submissions`,
			{
				workspaceId: seeded.workspaceId,
				versionId: version.id,
				content: { name: "Chrona" },
		...
		expect(((await submission.json()) as { versionId: string }).versionId).toBe(
			version.id,
		);
		...
		const missingRequired = await post(
			app,
			`/goals/${seeded.goal.id}/assets/${asset.id}/submissions`,
			{ workspaceId: seeded.workspaceId, versionId: version.id, content: {} },
		);
		...
		const unknownField = await post(
			...
			`/goals/${seeded.goal.id}/assets/${asset.id}/submissions`,
			{
				workspaceId: seeded.workspaceId,
				versionId: version.id,
				content: { name: "Chrona", previewSubmission: true },
			},
		);
		expect(unknownField.status).toBe(400);
		expect(
			await db.goalFormSubmission.count({ where: { assetId: asset.id } }),
		).toBe(1);

		const exported = await post(
			app,
			`/goals/${seeded.goal.id}/assets/${asset.id}/jobs`,
			{
				workspaceId: seeded.workspaceId,
				versionId: version.id,
				kind: "export",
		...
		expect(await exported.json()).toMatchObject({
			status: "Completed",
			outputUri: expect.stringContaining(`/v1.json`),
		});

		const sourceDownload = await app.request(
			`/goals/${seeded.goal.id}/assets/${asset.id}/download?versionId=${version.id}&mode=source`,
		);
		expect(sourceDownload.status).toBe(200);
		expect(sourceDownload.headers.get("content-disposition")).toContain(
			"Launch%20intake-v1.json",
		);
		expect(await sourceDownload.text()).toContain('"fields"');

		const exportDownload = await app.request(
			`/goals/${seeded.goal.id}/assets/${asset.id}/download?versionId=${version.id}&mode=export`,
		);
		expect(exportDownload.status).toBe(200);
		expect(exportDownload.headers.get("content-disposition")).toContain(
			"v1.json",
		);
		...
		expect(
			(
				await post(app, `/goals/${seeded.goal.id}/assets/${asset.id}/archive`, {
					workspaceId: seeded.workspaceId,
					action: "archive",
				})
			).status,
		...
		expect(
			(await db.goalAsset.findUniqueOrThrow({ where: { id: asset.id } }))
				.archivedAt,
		).not.toBeNull();
		expect(
			await db.goalAssetVersion.count({ where: { assetId: asset.id } }),
		...
		const activeAfterArchive = (await (
			await app.request(
				`/goals/${seeded.goal.id}/assets?workspaceId=${seeded.workspaceId}&state=active&sort=updated_desc`,
			)
		).json()) as { assets: Array<{ id: string }> };
		const archivedAfterArchive = (await (
			await app.request(
				`/goals/${seeded.goal.id}/assets?workspaceId=${seeded.workspaceId}&state=archived&sort=updated_desc`,
			)
		).json()) as { assets: Array<{ id: string }> };
		expect(activeAfterArchive.assets).not.toContainEqual(
			expect.objectContaining({ id: asset.id }),
		);
		expect(archivedAfterArchive.assets).toContainEqual(
			expect.objectContaining({ id: asset.id }),
		);
		...
		const archivedAfterRestore = (await (
			await app.request(
				`/goals/${seeded.goal.id}/assets?workspaceId=${seeded.workspaceId}&state=archived&sort=updated_desc`,
			)
		).json()) as { assets: Array<{ id: string }> };
		expect(activeAfterRestore.assets).toContainEqual(
			expect.objectContaining({ id: asset.id }),
		);
		expect(archivedAfterRestore.assets).not.toContainEqual(
			expect.objectContaining({ id: asset.id }),
		);
	});
	...
	it("rejects malformed and escaping completed export references", async () => {
		...
		const version = await db.goalAssetVersion.create({
			data: {
				...
				contentHash: "secure-v1",
				authorType: "user",
			},
		});
		const app = createApiRouter(createChronaEngine());

		for (const outputUri of [
			"generated://goals/../secret.txt",
			"generated://goals//secret.txt",
			"file:///tmp/secret.txt",
		]) {
			await db.goalAssetJob.deleteMany({ where: { assetId: asset.id } });
			await db.goalAssetJob.create({
				data: {
					workspaceId: seeded.workspaceId,
					goalId: seeded.goal.id,
					assetId: asset.id,
	...
	it("creates bounded metadata-only asset Tasks", async () => {
		...
		for (const route of ["ai-modification-task", "use-task"]) {
			const body =
				route === "ai-modification-task"
					? {
							workspaceId: seeded.workspaceId,
							versionId: version.id,
							instruction: "Clarify the recommendation",
							expectedOutcome: "A reviewed clearer brief",
						}
					: {
							workspaceId: seeded.workspaceId,
							versionId: version.id,
							title: "Use launch brief",
							instruction: "Prepare the next decision",
							expectedOutcome: "A decision memo",
						};
			const response = await post(
				app,
				`/goals/${seeded.goal.id}/assets/${asset.id}/${route}`,
				body,
			);
			expect(response.status).toBe(200);
			const { taskId } = (await response.json()) as { taskId: string };
			const created = await db.task.findUniqueOrThrow({
				where: { id: taskId },
			});
			...
			expect(JSON.stringify(created.goalContext)).not.toContain(
				"SECRET ASSET BODY",
			);
	...
	it("records append-only version-specific asset verification", async () => {
		...
		const response = await post(
			app,
			`/goals/${seeded.goal.id}/assets/${asset.id}/reviews`,
			{
				workspaceId: seeded.workspaceId,
				versionId: version.id,
				verifiedAt: "2026-07-29T10:00:00.000Z",
				nextReviewAt: "2026-08-29T10:00:00.000Z",
				summary: "Checked source links",
			},
		);
		expect(response.status).toBe(200);
		...
		expect(
			await db.goalAssetReview.count({ where: { assetId: asset.id } }),
		).toBe(1);
	});

	it("rejects appending an Inbox candidate when the selected base version is stale", async () => {
		const seeded = await seedGoalResult();
		const target = await db.goalAsset.create({
			data: {
				workspaceId: seeded.workspaceId,
				goalId: seeded.goal.id,
				sourceArtifactId: seeded.artifact.id,
		...
		const response = await post(
			...
			`/goals/${seeded.goal.id}/inbox/${candidates[0]!.id}/resolve`,
			{
				...
				baseVersionId: first.id,
				changeSummary: "Append accepted result",
			},
		);
		expect(response.status).toBe(409);
		expect(
			await db.goalAssetVersion.count({ where: { assetId: target.id } }),
		).toBe(2);
		expect(
			(
				await db.goalInboxCandidate.findUniqueOrThrow({
