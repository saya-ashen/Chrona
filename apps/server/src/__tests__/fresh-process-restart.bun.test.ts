...
async function run(command: string[]) {
	...
	if (exitCode !== 0) {
		throw new Error(
			`${command.join(" ")} failed: ${await new Response(process.stderr).text()}`,
		);
	}
}
...
function seedRestartState() {
	...
	db.query(
		"INSERT INTO TaskPlan (id, workspaceId, taskId, planId, revision, status, compiledPlan, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
	).run(
		"restart-plan-record",
		workspaceId,
		taskId,
	...
	db.query(
		"INSERT INTO ExecutionSession (id, workspaceId, taskId, planId, status, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)",
	).run(sessionId, workspaceId, taskId, "restart-plan", "Active", now, now);
	db.query(
		"INSERT INTO Run (id, taskId, runtimeName, runtimeRunRef, status, triggeredBy, syncStatus, retryable, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
	).run(
		runId,
		taskId,
		"debug",
...
describe("fresh process restart recovery", () => {
	...
	it("preserves waiting work and does not duplicate its run after a real server restart", async () => {
		...
		const session = db
			.query<{ status: string; pauseReason: string | null }, []>(
				"SELECT status, pauseReason FROM ExecutionSession WHERE id = 'restart-session'",
			)
			.get();
		const runs = db
			.query<{ count: number }, []>(
				"SELECT COUNT(*) AS count FROM Run WHERE taskId = 'restart-task'",
			)
			.get();
		const run = db
			.query<{ status: string }, []>(
				"SELECT status FROM Run WHERE id = 'restart-run'",
			)
			.get();
		db.close();
