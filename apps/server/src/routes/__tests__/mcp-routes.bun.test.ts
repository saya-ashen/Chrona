...
async function callPersistedTool(
	...
	return {
		agentTools: {
			registry: () => ({
				tools: [
					{
						name: "chrona.execution.read",
						mutates: false,
						description: "Read execution.",
					},
					{
						name: "chrona.node.complete",
						mutates: true,
						description: "Complete task node.",
					},
				],
			}),
			...
			execute: async (operation: unknown) => {
				...
				const input = (
					operation as { input: { idempotencyKey?: string; taskId?: string } }
				).input;
				return {
...
describe("MCP routes", () => {
	...
	it("lists minimal external tool schemas with descriptions", () => {
		const conditionSelect =
			__mcpRouteTestHooks.externalTools.chrona_condition_select;

		expect(conditionSelect.description).toBe(
	...
	it("exports bounded Goal result search without backend Goal identity", () => {
		const goalResults =
			__mcpRouteTestHooks.externalTools.chrona_goal_results_read;
		...
		expect(
			goalResults.inputSchema.safeParse({ query: "research", limit: 5 }).success,
		).toBe(true);
	...
	it("does not expose hidden context fields in any public tool schema", () => {
		for (const tool of Object.values(__mcpRouteTestHooks.externalTools) as Array<{
	...
	it("builds Chrona input from visible payload and hidden context", () => {
		const input = __mcpRouteTestHooks.toChronaInput("chrona.node.complete", {
		...
		const goalResultInput = __mcpRouteTestHooks.toChronaInput(
			"chrona.goal.results.read",
			{
				query: "research",
				limit: 3,
				_meta: { sessionId: hiddenContextArguments.sessionId },
			},
		);
		expect(goalResultInput.payload).toEqual({
			query: "research",
			offset: 0,
			maxChars: 12_000,
			limit: 3,
	...
	it("returns bounded Goal results without internal operation metadata", async () => {
		...
		expect(JSON.stringify(expectStructuredContent(result))).not.toContain(
			"operationId",
		);
		expect(JSON.stringify(expectStructuredContent(result))).not.toContain(
			"taskId",
	...
	it("[MCP-002] validates and dispatches one atomic node completion", async () => {
		const completeTool = __mcpRouteTestHooks.externalTools.chrona_node_complete;
		expect(completeTool.inputSchema.safeParse({ summary: "Done" }).success).toBe(
			true,
		);
		expect(completeTool.inputSchema.safeParse({ summary: "" }).success).toBe(
			false,
		);
		expect(
			Object.hasOwn(__mcpRouteTestHooks.externalTools, "chrona_node_output"),
		).toBe(false);
		...
		expect(operations).toEqual([
			expect.objectContaining({
				...
				input: expect.objectContaining({
					sessionId: "chrona:task:task-1:execute:35faa86e",
				}),
			}),
		]);
	});
	...
	it("fails fast when mutating Chrona tools are called without sessionId", async () => {
		await expect(
			callTool("chrona.node.complete", { summary: "Done" }),
	...
	it("fails fast when Chrona tool context uses unsupported session_id", async () => {
		await expect(
			callTool("chrona.node.complete", {
				summary: "Done",
				_meta: { session_id: "chrona:task:task-1:execute" },
			}),
		).rejects.toThrow(
			"arguments._meta.session_id is not supported; expected sessionId",
		);
	});
	...
	it("accepts hidden context injected into every public tool call", async () => {
		...
		for (const [toolName, sessionId, args, expectedPayload] of cases) {
			...
			await callTool(
				toolName,
				{ ...args, ...hiddenContextArguments, sessionId },
				{ operations },
			);
	...
	it("does not expose the removed shared plan output tool", () => {
		expect(
			(__mcpRouteTestHooks.externalTools as Record<string, unknown>)
				.chrona_plan_output,
		).toBeUndefined();
	});
	...
	it("rejects branch target IDs in public condition terminal schema", async () => {
		await expect(
			callTool("chrona.node.condition_select", {
				nodeId: "condition-node",
				branchRef: "B20260516-01-A",
				nextNodeId: "node-2",
