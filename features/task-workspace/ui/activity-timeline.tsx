...
function isPlanGenerationEvent(item: WorkspaceActivityItem) {
	return (
		item.activityGroup?.kind === "plan_generation" ||
		item.id.includes("plan_generation.")
	);
}
...
function collectPlanGenerationGroup(
	...
	for (let index = startIndex; index < items.length; index += 1) {
		if (planGenerationGroupKey(items[index]) === groupKey)
...
function getToolPair(
	...
	if (next?.kind === "tool_completed" && isSameToolActivity(item, next))
		return next;
	return items.find(
		(candidate) =>
...
function getToolProgress(
	items: WorkspaceActivityItem[],
	started: WorkspaceActivityItem,
) {
	return items.filter(
		(candidate) =>
			candidate.kind === "tool_progress" &&
			isSameToolActivity(started, candidate),
	);
}
...
function hasCompletedTool(
	...
	return items.some(
		(candidate) =>
			candidate.kind === "tool_completed" &&
			isSameToolActivity(candidate, item),
	);
}

function hasToolProgress(
	items: WorkspaceActivityItem[],
	item: WorkspaceActivityItem,
) {
	return items.some(
...
export function buildRenderList(
	...
	for (let i = 0; i < visibleItems.length; i++) {
		...
		if (planGroupKey) {
			...
			result.push({
				type: "plan_phase",
				key: `plan:${planGroupKey}:${group[0]?.id ?? item.id}:${group.length}`,
				items: group,
			});
			continue;
		}
		...
		if (item.kind === "tool_started") {
			if (transcript && (completedTool || hasToolProgress(visibleItems, item)))
				continue;
			result.push({
				type: "tool_pair",
				key: item.id,
			...
			<div className="mt-2 space-y-2">
				{payloadItems.map((item) => (
					<div key={item.id} className="space-y-1">
						...
						{item.providerInput !== undefined ? (
							<pre className="max-h-48 overflow-auto whitespace-pre-wrap break-words rounded bg-background/70 p-2 font-mono text-[10px] text-foreground">
								Input: {format(item.providerInput)}
							</pre>
						) : null}
						{item.providerOutput !== undefined ? (
...
function SpineIcon({
	...
	return (
		<div className="relative flex justify-center">
			{!isLast && (
				<span className="absolute -bottom-3 top-8 mx-auto w-px bg-border/50" />
			)}
			<span
				className={cn(
					"relative z-10 mt-0.5 flex size-7 shrink-0 items-center justify-center ring-4 ring-background shadow-sm",
					shape,
					toneIconClass(tone),
				)}
			>
...
function SingleEventRow({
	...
	const text =
		item.kind === "provider_run" && item.summary === item.provider?.label
			? undefined
	...
	return (
		<article className="relative grid grid-cols-[1.5rem_minmax(0,1fr)] gap-x-2">
			<SpineIcon
				tone={iconTone}
				shape={isNodeEvent ? "rounded-md" : "rounded-full"}
				isLast={isLast}
			>
			...
			<div className="min-w-0 pb-2.5 pt-0.5">
				<div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
					...
					<p className="min-w-0 break-words text-xs font-medium leading-snug text-foreground">
						{item.title}
					</p>
					{toolState && (
						<span
				...
				<ActivityPayloadDetails items={[item]} />
			</div>
		</article>
	);
}
	...
	return (
		<article className="relative grid grid-cols-[1.5rem_minmax(0,1fr)] gap-x-2">
			...
			<div className="min-w-0 pb-2.5 pt-0.5">
				...
				<ActivityPayloadDetails
					items={[started, ...progress, ...(completed ? [completed] : [])]}
				/>
			</div>
		</article>
	);
...
function getPlanPhaseEventText(item: WorkspaceActivityItem) {
	return item.summary && item.summary !== item.title
		? item.summary
		: item.title;
}

function getPlanPhaseEventDetail(
	item: WorkspaceActivityItem,
	text: string,
	milestone: boolean,
	compact: boolean,
...
function PlanGenerationPhaseRow({
	...
	const [expanded, setExpanded] = useState(
		transcript ? summarizePlanPhase(items) !== "completed" : true,
	);
				...
				<div className="rounded-2xl border border-primary/20 bg-primary-soft/40 px-3 py-2.5 shadow-sm">
					...
					{expanded && (
						<div className="mt-3 space-y-2 border-l border-primary/25 pl-3">
							{items.map((item) => (
								<PlanPhaseEvent key={item.id} item={item} compact={compact} />
							))}
						</div>
					)}
				</div>
...
function RunDividerRow({
	...
	return (
		<div
			...
			role="separator"
			aria-label={`${restarted ? "Run restarted from beginning" : "Execution started"} · Run ${runNumber}`}
		>
			<span className="h-px flex-1 bg-border" />
			<span className="shrink-0 rounded-full border border-border bg-muted/60 px-2.5 py-1 text-[11px] font-semibold text-foreground">
				{restarted ? "Run restarted from beginning" : "Execution started"} · Run{" "}
				{runNumber}
				{time ? ` · ${time}` : ""}
			</span>
			<span className="h-px flex-1 bg-border" />
...
function railTitle(entry: RenderEntry) {
	switch (entry.type) {
		...
		case "tool_pair":
			return (
				entry.completed?.tool?.label ??
				entry.started.tool?.label ??
				entry.completed?.tool?.name ??
				entry.started.tool?.name ??
				entry.started.title
			);
		case "plan_phase":
...
function railDetail(entry: RenderEntry) {
	switch (entry.type) {
		...
		case "tool_pair": {
			...
			const duration = fmtDuration(tool?.durationMs);
			if (entry.completed?.tone === "danger" || tool?.state === "failed")
				return "failed";
			if (entry.completed) return duration ? `done · ${duration}` : "done";
			return (
				entry.progress.at(-1)?.summary ?? entry.started.summary ?? "running"
			);
		}
		case "plan_phase": {
		...
		case "single": {
			...
			if (item.tool?.state === "started" || item.tool?.state === "progress")
				return item.summary || "running";
			return item.summary && item.summary !== item.title
				? item.summary
				: (item.provider?.label ?? formatActivityTime(item.timestamp));
		}
	}
}
...
function railDotClass(tone: Tone) {
	if (tone === "danger")
		return "border-destructive bg-destructive shadow-[0_0_0_4px_color-mix(in_oklab,var(--destructive)_18%,transparent)]";
	if (tone === "warning")
		return "border-warning bg-warning shadow-[0_0_0_4px_color-mix(in_oklab,var(--warning)_18%,transparent)]";
	if (tone === "success")
		return "border-success bg-success shadow-[0_0_0_4px_color-mix(in_oklab,var(--success)_16%,transparent)]";
	if (tone === "info")
		return "border-primary bg-primary shadow-[0_0_0_4px_color-mix(in_oklab,var(--primary)_18%,transparent)]";
...
function isConcreteRunningEntry(entry: RenderEntry) {
	...
	if (entry.type === "plan_phase")
		return summarizePlanPhase(entry.items) === "running";
	if (entry.type === "single")
		return (
			entry.item.tool?.state === "started" ||
			entry.item.tool?.state === "progress"
		);
	return false;
}
...
function isProviderRunFallback(entry: RenderEntry) {
	return (
		entry.type === "single" &&
		entry.item.kind === "provider_run" &&
		entry.item.tone === "info"
	);
}

function activeRailEntryIndex(entries: RenderEntry[]) {
	const concreteIndex = entries.findIndex(isConcreteRunningEntry);
	return concreteIndex >= 0
		? concreteIndex
		: entries.findIndex(isProviderRunFallback);
}
		...
		<div className="space-y-0.5">
			{entries.map((entry, index) => {
				...
				return (
					<article
						key={entry.key}
						className="relative grid grid-cols-[1rem_minmax(0,1fr)] gap-x-2 py-1.5"
					>
						<div className="relative flex justify-center">
							{!isLast ? (
								<span
									className={cn(
										"absolute bottom-[-0.625rem] top-4 w-0.5 rounded-full",
										railLineClass(tone),
									)}
								/>
							) : null}
							{isActiveLatest ? (
								<span
									aria-label="Latest activity running"
									className={cn(
										"relative z-10 mt-0.5 size-3.5 rounded-full border-2 border-transparent border-t-current animate-spin",
										tone === "danger"
											? "text-destructive"
											: tone === "warning"
												? "text-warning"
								...
								<span
									className={cn(
										"relative z-10 mt-1 size-2.5 rounded-full border",
										railDotClass(tone),
									)}
								/>
							)}
...
export function ActivityTimeline({
	...
	if (rail)
		return <ActivityRailTimeline entries={renderList} active={active} />;
