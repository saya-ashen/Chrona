...
import { recordProp, stringField, stringProp } from "./workspace-registry-utilities";

export function ResultComparison({ props }: { props: Record<string, unknown> }) {
...
export function ResultChangeSummary({ props }: { props: Record<string, unknown> }) {
  const items = Array.isArray(props.items)
    ? props.items.filter(
        (
          item,
