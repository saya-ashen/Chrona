...
function TableHeader({
	tableState,
	wide,
}: Pick<TablePresentationProps, "tableState"> & { wide: boolean }) {
	const { table, tableColumns } = tableState;
	return (
		<thead
			className={cn(
				"bg-muted/55 [&_tr]:border-b",
				wide && "sticky top-0 z-20 shadow-sm",
			)}
		>
...
function TableRows({
	props,
	tableState,
}: Omit<TablePresentationProps, "error">) {
...
function TableGrid({
	...
	const tableWidth = Math.max(1, tableColumns.length) * columnWidth;
	const needsHorizontalScroll = tableColumns.length > 2;
	return (
		<>
			<div
				ref={props.wide ? scrollRef : undefined}
				data-result-table-scroll={props.wide ? "virtual" : "true"}
				aria-label={
					needsHorizontalScroll
						? "Table preview; scroll horizontally to see all columns"
						: "Table preview"
				}
				className={cn(
					"min-w-0 w-full max-w-full rounded-md border border-border/80 bg-background overflow-x-auto overscroll-x-contain",
					props.wide && "max-h-[60vh] overflow-y-auto",
				)}
			>
				<table
					className={cn(
					...
					style={
						props.wide || needsHorizontalScroll
...
export function WorkspaceTablePresentation({
	...
	return (
		<section className="min-w-0 w-full max-w-full space-y-2 text-sm">
