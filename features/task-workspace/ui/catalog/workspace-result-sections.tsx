...
function resultSectionBodyClassName(layout: string, density: string): string {
	return cn(
		...
		layout === "stack" && (density === "compact" ? "space-y-3" : "space-y-4"),
	);
}

function resultSectionToneClassName(tone: string, density: string): string {
	return cn(
		"min-w-0",
		tone === "subtle" &&
			density !== "compact" &&
			"rounded-2xl bg-muted/35 p-4 sm:p-5",
		tone === "accent" &&
			density !== "compact" &&
...
export function ResultSection({
	...
	const density = stringProp(props.density) ?? "comfortable";
	const body = (
		<div className={resultSectionBodyClassName(layout, density)}>
			{children}
		</div>
	);
	...
	return (
		<section className={resultSectionToneClassName(tone, density)}>
			<h2
				className={cn(
