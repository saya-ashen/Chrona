...
it("normalizes a descendant that is also listed beside its parent", () => {
	...
	expect(normalized.elements.root?.children).toEqual(["section"]);
...
it("preserves Finalizer branch order while optimizing secondary sections", () => {
	const spec: UiDocument = {
		...
		elements: {
			...
			overview: {
				type: "ResultOverview",
				props: { title: "Outcome", summary: "Ready" },
			},
			deliverables: {
				type: "ResultSection",
			...
			insights: {
				type: "ResultSection",
				props: { title: "Observations" },
				children: ["insight"],
			},
			insight: {
				type: "ResultInsight",
				props: { title: "Signal", summary: "Supporting context" },
			},
		},
	};

	const prioritized = prioritizeResultSpec(spec);
...
it("removes repeated overview metrics and compacts supporting sections", () => {
	const spec: UiDocument = {
		root: "root",
		elements: {
			root: {
				type: "Stack",
				props: {},
				children: ["metrics", "overview", "quality"],
			},
			overview: {
				type: "ResultOverview",
				props: {
					...
					metrics: [
						...
						{ label: "Links", value: "17/17" },
						{ label: "Duplicates", value: "0" },
						{ label: "Python", value: "5" },
					],
					sourceKeys: ["records", "links", "duplicates"],
				},
			},
			metrics: {
				...
				props: {
					...
					items: [
						{ label: "Rows", value: "17" },
						{ label: "HTTP", value: "17/17 HTTP 200" },
						{ label: "Duplicates", value: "0" },
						{ label: "Python", value: "5" },
					],
					sourceKeys: ["records", "links", "duplicates"],
			...
			checklist: {
				...
				props: {
					title: "Checks",
					items: [{ label: "Complete", status: "done" }],
				},
			},
		},
	};
	...
	expect(prioritized.elements.root?.children).toEqual(["overview", "quality"]);
	expect(prioritized.elements.quality?.props.density).toBe("compact");

	render(<SpecRenderer spec={spec} resultPresentation />);
	expect(screen.queryByText("Verification metrics")).not.toBeInTheDocument();
...
describe("workspace result registry", () => {
	...
	it("renders the operational result hierarchy with semantic surfaces", async () => {
		...
		fireEvent.click(
			within(primaryDeliverable as HTMLElement).getByRole("button", {
				name: "Preview",
			}),
		);
		...
		fireEvent.click(
			within(
				screen.getByText("Source table").closest("article") as HTMLElement,
			).getByRole("button", { name: "Preview" }),
		);
		const csvPreview = screen.getByRole("dialog");
		expect(within(csvPreview).getByRole("table")).toBeInTheDocument();
		expect(
			within(csvPreview).getByRole("columnheader", { name: /official_url/i }),
		).toBeInTheDocument();
		expect(within(csvPreview).getByRole("table").parentElement).toHaveAttribute(
			"data-result-table-scroll",
			"virtual",
		);
		expect(
			within(csvPreview).getByText("2 rows · scroll to explore"),
		).toBeInTheDocument();
		expect(
			within(csvPreview).queryByRole("button", { name: "Next" }),
		).not.toBeInTheDocument();
		expect(
			await within(csvPreview).findByRole("link", {
				name: "https://www.jku.at/",
			}),
		).toHaveAttribute("href", "https://www.jku.at/");
		expect(
			await within(csvPreview).findByText("Agent, tool use, and safety"),
		).toBeInTheDocument();
		expect(
			within(csvPreview).queryByText(/name,official_url,note/),
		).not.toBeInTheDocument();
		fireEvent.click(
			within(csvPreview).getByRole("button", { name: "Close preview" }),
		);
	...
	it("renders an adaptive comparison workspace without the legacy result template", () => {
		...
		expect(
			screen.queryByRole("columnheader", { name: "Rank" }),
		).not.toBeInTheDocument();
