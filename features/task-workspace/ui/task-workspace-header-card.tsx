...
function hideHeaderActions(
	spec: UiDocument,
	input: { generatePlan?: boolean; acceptPlan?: boolean },
): UiDocument {
	const generateAction = spec.elements["action:generate-plan"];
	const acceptAction = spec.elements["action:accept-plan"];
	if (
		(!input.generatePlan || !generateAction) &&
		(!input.acceptPlan || !acceptAction)
	)
		return spec;
	return {
		...spec,
		elements: {
			...spec.elements,
			...(input.generatePlan && generateAction
				? { "action:generate-plan": { ...generateAction, visible: false } }
				: {}),
			...(input.acceptPlan && acceptAction
				? { "action:accept-plan": { ...acceptAction, visible: false } }
				: {}),
		},
	};
}
function localizeHeaderResultStatus(
	spec: UiDocument,
		...
		elements: Object.fromEntries(
			Object.entries(spec.elements).map(([key, element]) => {
				...
				const localizedProps = {
					...
					...(props.statusLabel === defaultStatus
						? { statusLabel: localizedStatus }
						: {}),
				};
				return [
					key,
					localizedProps.text !== props.text ||
					localizedProps.statusLabel !== props.statusLabel
						? { ...element, props: localizedProps }
						: element,
				];
			}),
		),
...
function findActionLabel(spec: UiDocument, actionId: HeaderActionId) {
	...
	const label = (spec.elements[key]?.props as { label?: string } | undefined)
		?.label;
...
export function TaskWorkspaceHeaderCard({
	...
	const [pendingActionId, setPendingActionId] = useState<HeaderActionId | null>(
		null,
	);
	const [actionStatus, setActionStatus] = useState<string | null>(null);
	const [restartConfirmOpen, setRestartConfirmOpen] = useState(false);
	const [deleteConfirmationStep, setDeleteConfirmationStep] = useState<1 | 2>(
		1,
	);
	...
	const handlers = useMemo(
		() => ({
			"edit-task": () => ref.current.onEdit(),
			"delete-task": () => ref.current.onStartDeleteConfirm(),
			...
			[UI_ACTION.acceptPlan]: async () =>
				Promise.resolve(ref.current.onAcceptPlan()),
			[UI_ACTION.generatePlan]: async () =>
				Promise.resolve(ref.current.onGeneratePlan()),
			[UI_ACTION.stopPlanGeneration]: async () =>
				Promise.resolve(ref.current.onStopPlanGeneration()),
			[UI_ACTION.dispatchExecution]: async (
				...
				try {
					...
					setActionStatus(
						error instanceof Error
							? error.message
							: `${currentCopy.actionFailedPrefix} ${label.toLowerCase()}.`,
					);
				} finally {
			...
			[UI_ACTION.recoveryRetry]: async () =>
				Promise.resolve(ref.current.onRecoveryRetry()),
			[UI_ACTION.recoveryEditInstruction]: async () =>
				Promise.resolve(ref.current.onRecoveryEditInstruction()),
			[UI_ACTION.recoveryCancel]: () => ref.current.onRecoveryCancel(),
		}),
		[],
	);
	...
	return (
		<>
			<header className="relative z-30 min-w-0 overflow-hidden border-y border-panel-border bg-muted/70 px-4 py-2.5 before:absolute before:inset-y-0 before:left-0 before:w-1 before:bg-primary [&_h1]:w-full [&_h1]:min-w-0 [&_h1]:break-words [&_h1]:text-xl [&_h1]:font-semibold [&_h1]:tracking-tight sm:px-5 sm:py-3 sm:[&_h1]:text-2xl">
				...
				<SpecRenderer
					spec={localizeHeaderResultStatus(
						hideHeaderActions(spec, {
							generatePlan: hideGeneratePlan,
							acceptPlan: hideAcceptPlan,
						}),
						copy,
						resultAccepted,
					)}
				...
				<DialogContent className="sm:max-w-md">
					...
					<div className="rounded-xl border border-destructive/30 bg-destructive/5 p-3 text-sm text-muted-foreground">
						...
						<p className="mt-2 font-medium text-destructive">
							{copy.rebuildTaskIrreversibleWarning}
						</p>
					</div>
					<DialogFooter>
						<Button
							type="button"
							variant="outline"
							onClick={onCancelRebuildConfirm}
			...
			>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						<DialogTitle>
							{copy.deleteConfirmTitlePrefix} &ldquo;{task.title}&rdquo;
							{copy.deleteConfirmTitleSuffix}
						</DialogTitle>
						<DialogDescription>
							{deleteConfirmationStep === 1
								? copy.deleteConfirmDescription
								: copy.deleteFinalConfirmDescription}
						</DialogDescription>
					</DialogHeader>
					{deleteConfirmationStep === 2 ? (
						<div className="space-y-3 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm">
							...
							{deleteImpactError ? (
								<p className="text-destructive">{deleteImpactError}</p>
							) : null}
							{deleteImpact ? (
								<>
									...
									{deleteImpact.assets.length > 0 ? (
										<ul
											className="max-h-48 list-disc space-y-1 overflow-y-auto pl-5"
											aria-label={copy.deleteAssetListLabel}
										>
											{deleteImpact.assets.map((asset) => (
					...
					<DialogFooter>
						...
						>
							{isDeleting
								? copy.deleting
								: deleteConfirmationStep === 1
									? copy.reviewDeleteImpact
									: copy.deleteTaskConfirm}
						</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
