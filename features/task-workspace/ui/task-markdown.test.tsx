...
import {
	...
	MarkdownContent,
} from "../../../shared/ui/markdown-content";
...
describe("MarkdownContent", () => {
	...
	it("renders GFM answers as semantic HTML", () => {
		...
		expect(
			screen.getByRole("heading", { name: "DeepTutor", level: 2 }),
		).toBeVisible();
		expect(screen.getByText("值得试用：", { selector: "strong" })).toHaveClass(
			"font-bold",
		);
		...
		expect(
			screen.queryByRole("link", { name: "生成报告" }),
		).not.toBeInTheDocument();
