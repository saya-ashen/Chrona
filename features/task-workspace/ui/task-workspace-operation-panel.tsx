...
function formatRuntimeEvent(
	event: TaskWorkspaceOperationState["runtimeEvents"][number],
) {
	...
	switch (value.type) {
		...
		case "tool_completed":
			return value.error
				? `Tool: ${value.label} failed`
				: `Tool: ${value.label} completed`;
	...
	return (
		<div className="rounded-xl border border-border/60 bg-background/70 px-3 py-2 text-xs">
			<div className="font-semibold text-muted-foreground">
				{copy.selectedNode ?? "Selected node"}
			</div>
...
function taskActionButtonLabel(
	state: Extract<TaskWorkspaceOperationState, { status: "task-action" }>,
) {
	return state.taskPrimaryAction.label;
}

function taskActionButtonVariant(
	state: Extract<TaskWorkspaceOperationState, { status: "task-action" }>,
) {
...
function stateHelpText(workState: WorkStateView) {
	if (
		...
		return `Next: ${workState.nextActionLabel}`;
	if (workState.blocker)
		return `${workState.blocker.reason} · Scope: ${workState.blocker.scope}`;
	if (workState.currentNodeLabel)
		return `Current step: ${workState.currentNodeLabel}`;
...
function DecisionRecoveryCard({ workState }: { workState: WorkStateView }) {
	...
	return (
		<div
			className="rounded-2xl border border-foreground/15 bg-background/75 px-3 py-3 text-sm"
			data-testid="current-operation-decision-card"
		>
			<div className="flex flex-wrap items-start justify-between gap-3">
				...
				<Badge
					variant={
						failure.category === "approval" || failure.category === "input"
							? "secondary"
							: "destructive"
					}
				>
			...
			<dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2">
				<div>
					<dt className="font-medium text-muted-foreground">Retained</dt>
					<dd>{failure.retainedProgress.join(" ")}</dd>
				</div>
				<div>
					<dt className="font-medium text-muted-foreground">Retry from</dt>
					<dd>{failure.retryFrom ?? "Current execution"}</dd>
				</div>
				{failure.duplicateSideEffectRisk ? (
					<div className="sm:col-span-2">
						<dt className="font-medium text-muted-foreground">
							Before retrying
						</dt>
						<dd>{failure.duplicateSideEffectRisk}</dd>
					</div>
				) : null}
			</dl>
			{failure.technicalDetail ? (
				<details className="mt-3">
					...
					<div className="mt-2 rounded-lg bg-muted/60 p-2 font-mono text-xs text-muted-foreground">
						{failure.technicalDetail}
						{failure.diagnosticRef ? ` · ${failure.diagnosticRef}` : ""}
					</div>
				</details>
			) : null}
		</div>
...
export function TaskWorkspaceOperationPanel({
	...
	const showRecoveryOptions =
		canRecoverExecution &&
		["failed", "blocked", "cancelled"].includes(workState.state);
	const showPlanActions =
		hasAcceptedPlan &&
		Boolean(onRestartPlan || onRegeneratePlan) &&
		...
		>
			<CardHeader className="gap-3 px-3 sm:grid sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
				<div className="flex min-w-0 gap-3">
					{isRunning ? (
						<span
							className="mt-1 size-4 shrink-0 animate-spin rounded-full border-2 border-primary/20 border-t-primary"
							aria-label="Current operation running"
						/>
					) : null}
					<div className="min-w-0 space-y-1">
						<div className="flex flex-wrap items-center gap-2">
							<p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-foreground">
								{copy.currentOperation ?? "Current operation"}
							</p>
							{state.statusLabel ? (
						...
						<>
							{state.hasGraphExecutionStarted ? (
								<Button
									type="button"
									variant="outline"
									onClick={() => openRecovery("restart")}
									disabled={!onRestartPlan}
								>
							...
							<Button type="button" onClick={onStartPlan}>
								{state.hasGraphExecutionStarted
									? (copy.continuePlanAction ?? "Continue plan")
									: (copy.startPlanAction ?? "Start plan")}
							</Button>
						</>
					) : null}
					{showPlanActions ? (
						<Button
							type="button"
							variant="outline"
							onClick={() => openRecovery("regenerate")}
			...
			<CardContent className="space-y-3 px-3">
				...
				{state.status === "task-action" ? (
					<div className="space-y-2">
						<div
							className="flex flex-col gap-2 rounded-xl border border-destructive/25 bg-background/80 px-3 py-2 shadow-sm sm:flex-row sm:items-center sm:justify-between"
							data-testid="current-operation-primary-action"
						>
							<div className="min-w-0 text-xs">
								<p className="font-semibold text-destructive">
									{workState.state === "failed" ||
									state.statusLabel === "run_failed"
										? "Failed"
										: workState.label}
								</p>
				...
				{state.status === "execution-action" ||
				state.status === "execution-blocked" ? (
					<SpecRenderer
						spec={state.operationSpec}
						handlers={state.actionHandlers}
						onStateChange={state.onActionStateChange}
					/>
				) : null}
				{latestEvents.length > 0 ? (
					<div className="rounded-xl border border-border/60 bg-background/70 px-3 py-2 text-xs">
						<div className="font-semibold text-muted-foreground">
							{copy.liveEvents ?? "Live"}
						</div>
						<ul className="mt-1 space-y-1">
							{latestEvents.map((event, index) => (
								<li key={`${event}-${index}`} className="text-foreground">
									{event}
								</li>
							))}
						</ul>
				...
				{state.status !== "plan-review" ? (
					<SelectedNodeCue node={state.selectedNode} copy={copy} />
				) : null}
			...
			<Dialog
				open={recoveryOpen}
				onOpenChange={(open) => {
					if (!isRecovering) setRecoveryOpen(open);
				}}
			>
				<DialogContent className="sm:max-w-lg">
					<DialogHeader>
						<DialogTitle>
							{copy.recoveryDialogTitle ?? "Choose how to recover this task"}
						</DialogTitle>
						<DialogDescription>
							{copy.recoveryDialogDescription ??
								"Keep the current plan and run it again, or replace it with a newly generated draft."}
						</DialogDescription>
					</DialogHeader>
					<div className="grid gap-2 sm:grid-cols-2">
						<Button
							type="button"
							variant={recoveryAction === "restart" ? "default" : "outline"}
							className="h-auto min-h-20 whitespace-normal p-3 text-left"
							onClick={() => setRecoveryAction("restart")}
						>
							<span>
								...
								<span className="mt-1 block text-xs opacity-80">
									{copy.runPlanFromBeginningDescription ??
										"Keep the accepted plan. Reset execution progress and start at its first step."}
								</span>
							</span>
						</Button>
						<Button
							type="button"
							variant={recoveryAction === "regenerate" ? "default" : "outline"}
							className="h-auto min-h-20 whitespace-normal p-3 text-left"
						...
						>
							<span>
								...
								<span className="mt-1 block text-xs opacity-80">
									{copy.generateNewPlanDescription ??
										"Stop this execution and create a new draft for review before running."}
								</span>
							</span>
						</Button>
					</div>
					<div className="rounded-xl border bg-muted/35 p-3 text-xs text-muted-foreground">
						<p className="font-medium text-foreground">
							{recoveryAction === "restart"
								? (copy.recoveryKeepsPlan ??
									"Keeps the task, accepted plan, history, and artifacts.")
								: (copy.recoveryKeepsHistory ??
									"Keeps the task, previous plan, history, and artifacts.")}
						</p>
						<p className="mt-1">
							{recoveryAction === "restart"
								? (copy.recoveryResetsExecution ??
									"Resets current node progress, execution context, pending checkpoints, and result finalization.")
								: (copy.recoveryRequiresReview ??
									"The new plan remains a draft until you review and accept it.")}
						</p>
						<p className="mt-1 font-medium text-warning-foreground">
							{copy.recoverySideEffectWarning ??
								"Completed steps may already have changed external systems. Running again can repeat those actions."}
						</p>
					</div>
					<label className="space-y-1.5 text-sm font-medium">
						<span>
							{recoveryAction === "restart"
								? (copy.restartInstructionLabel ??
									"Instructions for the new run (optional)")
								: (copy.replanInstructionLabel ??
									"What should the new plan change? (optional)")}
						</span>
						<Textarea
							value={recoveryInstruction}
							onChange={(event) => setRecoveryInstruction(event.target.value)}
							rows={3}
							placeholder={
								recoveryAction === "restart"
									? (copy.restartInstructionPlaceholder ??
										"For example: collect fresh data and do not reuse earlier search results.")
									: (copy.replanInstructionPlaceholder ??
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								isRecovering ||
								(recoveryAction === "regenerate" && !onRegeneratePlan)
							}
						>
							{isRecovering
								? (copy.recoveryStarting ?? "Starting...")
								: recoveryAction === "restart"
									? (copy.runPlanFromBeginning ?? "Run plan from beginning")
									: (copy.generateNewPlan ?? "Generate a new plan")}
						</Button>
