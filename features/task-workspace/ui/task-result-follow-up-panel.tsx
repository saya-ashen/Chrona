...
export function TaskResultFollowUpPanel({
  ...
  const { state, setMode, setDraft, submit } = useTaskResultFollowUp(
    taskId,
    true,
  );
  ...
  const sessionLabel = !sourceSession?.available
    ? (copy.followUpSessionUnavailable ??
      "Original session unavailable — answers use the accepted result.")
    : sessionHealth === "compacted"
      ? (copy.followUpSessionCompacted ??
        "Continuing the original session · earlier context was compacted")
      : (copy.followUpSessionActive ??
        "Continuing the original execution conversation");
  ...
  return (
    <section
      ...
      data-testid="result-follow-up-panel"
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Badge
          variant={sourceSession?.available ? "secondary" : "outline"}
          className="w-fit"
        >
          {sessionLabel}
        </Badge>
      ...
      {state.mode === "create_task" ? (
        <details className="rounded-xl border border-border/60 bg-muted/20 p-3 text-sm">
          <summary className="cursor-pointer font-medium text-foreground">
            {copy.followUpContextOptions ?? "Context options"}
          </summary>
          <p className="mt-1 text-xs text-muted-foreground">
            {copy.followUpContextOptionsDescription ??
              "Chrona chooses the reliable default. Change it only when the next task should use the final result without earlier background."}
          </p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            >
              ...
              <span>
                <span className="block font-medium">
                  {copy.followUpHandoffSession ?? "Keep this task's background"}
                </span>
                <span className="text-xs text-muted-foreground">
                  {sourceSession?.supportsHandoff
                    ? (copy.followUpHandoffSessionDescription ??
                      "Carry a focused summary of this task into the next one.")
                    : (copy.followUpHandoffUnavailable ??
                      "This background cannot be carried forward.")}
                </span>
              </span>
            </label>
            <label className="flex cursor-pointer gap-2 rounded-lg border border-border/60 bg-background p-2.5">
              ...
              <span>
                <span className="block font-medium">
                  {copy.followUpFreshSession ?? "Use only the accepted result"}
                </span>
                <span className="text-xs text-muted-foreground">
                  {copy.followUpFreshSessionDescription ??
                    "Start the next task from this result without earlier conversation context."}
                </span>
              </span>
            </label>
          </div>
      ...
      {state.mode === "ask" &&
      state.state?.entries.some((entry) => entry.intent === "ask") ? (
        <div
          className="max-h-64 space-y-3 overflow-y-auto rounded-xl bg-muted/55 p-3 text-sm"
          aria-live="polite"
        >
          {state.state.entries
            .filter((entry) => entry.intent === "ask")
            .map((entry) => (
              <div key={entry.id} className="space-y-1">
                ...
                {entry.answer ? (
                  <>
                    ...
                    <MarkdownContent className="py-0">
                      {entry.answer}
                    </MarkdownContent>
                    <p className="text-[11px] text-muted-foreground">
                      {entry.contextSource === "source_session"
                        ? (copy.followUpAnsweredFromSession ??
                          "Answered from the original execution conversation")
                        : (copy.followUpAnsweredFromResult ??
                          "Answered from the accepted result fallback")}
          ...
          <Card key={entry.id} className="border-success/35 bg-success/10">
            <CardContent className="flex flex-col gap-2 p-3 text-sm sm:flex-row sm:items-center sm:justify-between">
              <span>
                ...
                {entry.createdTask?.title}
              </span>
              <Button asChild type="button" size="sm" variant="outline">
                <Link
                  to={localizeHref(locale, `/tasks/${entry.createdTask?.id}`)}
                >
