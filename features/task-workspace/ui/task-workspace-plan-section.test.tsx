...
describe("TaskWorkspacePlanSection", () => {
	it("keeps graph and command center in sync across create, accept, and run", async () => {
		...
		fireEvent.change(
			...
			{ target: { value: "Use fresh sources" } },
		);
		fireEvent.click(
		...
		fireEvent.change(
			...
			{ target: { value: "Remove the unreliable step" } },
		);
		fireEvent.click(
			within(screen.getByRole("dialog")).getByRole("button", {
				name: "Generate a new plan",
		...
		expect(
			screen.queryByTestId("execution-focus-header"),
		).not.toBeInTheDocument();
		expect(getOperationPanel()).toHaveTextContent("Review generated output");
		expect(
			screen.queryByTestId("current-runtime-activity"),
		).not.toBeInTheDocument();
		expect(
