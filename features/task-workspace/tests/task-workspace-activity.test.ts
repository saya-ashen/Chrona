...
describe("workspace activity helpers", () => {
  ...
  it("projects tool lifecycle without provider inputs, results, IDs, or error text", () => {
    const started = runtimeEventToWorkspaceActivity(runtimeEvent({
      sequence: 1,
      event: { type: "tool_started", tool: { category: "tool", label: "Runtime tool" }, label: "Read source" },
    }));
    const progress = runtimeEventToWorkspaceActivity(runtimeEvent({
      sequence: 2,
      event: { type: "tool_progress", tool: { category: "tool", label: "Runtime tool" }, label: "Read source" },
    }));
    const completed = runtimeEventToWorkspaceActivity(runtimeEvent({
      sequence: 3,
      event: { type: "tool_completed", tool: { category: "tool", label: "Runtime tool" }, label: "Read source", durationMs: 42 },
    }));
    const failed = runtimeEventToWorkspaceActivity(runtimeEvent({
      sequence: 4,
      event: { type: "tool_completed", tool: { category: "tool", label: "Runtime tool" }, label: "Read source", error: { code: "permission_denied" } },
    }));

    expect(started?.tool).toMatchObject({ name: "Runtime tool", label: "Read source", state: "started" });
    expect(progress?.tool).toMatchObject({ name: "Runtime tool", label: "Read source", state: "progress" });
    expect(completed?.tool).toMatchObject({ name: "Runtime tool", label: "Read source", durationMs: 42, state: "completed" });
    expect(failed).toMatchObject({ title: "Tool failed", summary: "Provider tool failed.", tone: "danger", tool: { state: "failed" } });
    expect(JSON.stringify([started, progress, completed, failed])).not.toContain("run-1");
  });
  ...
  it("keeps approval and run lifecycle status actionable", () => {
    ...
    }))).toMatchObject({ kind: "approval", title: "Approval required", tone: "warning" });

    expect(runtimeEventToWorkspaceActivity(runtimeEvent({
      event: { type: "run_status", status: "failed" },
    }))).toMatchObject({ kind: "provider_run", title: "Run status", summary: "failed", tone: "danger" });
  });

  it("drops unsafe text and raw provider events", () => {
    const unsafe = {
      ...runtimeEvent({}),
      event: { type: "reasoning_delta", text: "private transient reasoning" },
    } as unknown as WorkspaceRuntimeEvent;
    expect(runtimeEventToWorkspaceActivity(unsafe)).toBeNull();
  });
