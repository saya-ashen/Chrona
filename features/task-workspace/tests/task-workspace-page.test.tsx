...
import {
	...
	createTaskWorkspaceUiFixture,
	taskWorkspaceStateFixtures,
} from "@features/task-workspace/test";
...
vi.mock("../hooks/use-task-workspace-page-state", () => ({
	useTaskWorkspacePageState: (data: TaskPageData) => ({
		...
		workspaceEvents: [],
		headerSpec:
			data.header?.spec ?? createHeaderSpecFixture({ title: data.task.title }),
		// Per-test overrides for the `/execution/can-*` state paths the
		// mock header card consults to compute the visible primary action.
...
vi.mock("../ui/task-workspace-header-card", () => ({
	TaskWorkspaceHeaderCard: ({
		...
		const actionEntries = Object.entries(elements).filter(([key]) =>
...
vi.mock("../ui/task-workspace-plan-section", async () => {
	...
	const { createTaskWorkspaceExecutionConsoleView } = await import(
		"../model/task-workspace-query"
	);
	...
	return {
		TaskWorkspacePlanSection: ({
			...
			onApplyPlan?: (plan: { id?: string; status?: string }) => void;
		}) => {
			const [selectedNodeId, setSelectedNodeId] = React.useState<string | null>(
				null,
			);
			const selectedNode =
				graphPlan?.nodes.find((node) => node.id === selectedNodeId) ?? null;
			...
			return (
				<section aria-label="workspace plan section">
					...
					<p>
						detail-refresh:
						{consoleView.nodeDetail.autoRefreshEnabled ? "on" : "off"}
					</p>
					<p>
						detail-disabled:
						{consoleView.nodeDetail.disabledActionReason ?? "none"}
					</p>
					...
					{canAcceptPlan && plan ? (
						<button type="button" onClick={() => onApplyPlan?.(plan)}>
							Apply Plan
						</button>
					) : null}
...
describe("TaskWorkspacePage", () => {
	...
	it("links Goal tasks to their corresponding Goal instead of the Goal list", () => {
		...
		expect(
			screen.getByRole("heading", { level: 1, name: "Plan migration" }),
		).toBeInTheDocument();
		expect(screen.getAllByRole("banner")).toHaveLength(1);
		expect(
			screen.queryByRole("link", { name: "All Goals" }),
	...
	it("renders dominant task identity, state treatment, and ranked primary action", () => {
		...
		expect(
			screen.getByRole("region", { name: "Workspace state" }),
		).toBeInTheDocument();
	...
	it("[PLAN-020] keeps generated plans reviewable before acceptance", () => {
		...
		mocks.executionState = {
			...
			"/execution/show-generate-plan": false,
			"/execution/start-disabled": true,
			"/execution/start-disabled-reason":
		...
		expect(
			screen.getByText("generation:waiting_acceptance"),
		).toBeInTheDocument();
	...
	it("passes human-review data through the workspace page", () => {
		...
		expect(
			screen.getByText("latest-run:WaitingForApproval"),
		).toBeInTheDocument();
	...
	it("passes latest persisted activity to the assistant surface", () => {
		...
		expect(context.highlights).toContainEqual(
	...
	it("renders running task header progress with task-level actions", () => {
		...
		expect(
			screen.getByRole("heading", { name: "Launch task" }),
		).toBeInTheDocument();
	...
	it("renders loading, empty, and error workspace page treatments", () => {
		const cases = [
			{
				fixture: taskWorkspaceStateFixtures.loading,
				planStatus: "generating",
				label: "Idle",
			},
			{
				fixture: taskWorkspaceStateFixtures.empty,
				planStatus: "idle",
				label: "No plan yet",
			},
			{
		...
		for (const testCase of cases) {
			...
			mocks.plan =
				testCase.planStatus === "idle"
					? null
					: { id: `plan-${testCase.label}`, status: "accepted" };
			mocks.graphPlan = testCase.fixture.graphPlan;

			render(<TaskWorkspacePage data={testCase.fixture.pageData} />);

			expect(
				screen.getByText(`workspace-treatment:${testCase.label}`),
			).toBeInTheDocument();
			expect(
				screen.getByRole("region", { name: "Workspace state" }),
			).toBeInTheDocument();
	...
	it("keeps task-level actions visible for permission-limited workspaces", () => {
		...
		mocks.executionState = {
			...
			"/execution/start-disabled": true,
			"/execution/start-disabled-reason":
				"You can view this task, but cannot run it",
			"/execution/status": "ready",
		};
	...
	it("keeps long mobile fixture content visible without dropping workspace regions", () => {
		...
		expect(
			screen.getByText("workspace-treatment:Review required"),
		).toBeInTheDocument();
		expect(
			screen.getAllByText(/Confirm evidence and provide launch approval notes/)
				.length,
		).toBeGreaterThan(0);
	...
	it("updates node detail context when a different plan node is selected", () => {
		...
		render(
			<TaskWorkspacePage data={taskWorkspaceStateFixtures.running.pageData} />,
		);

		expect(screen.getByText("detail-title:execute")).toBeInTheDocument();
		fireEvent.click(
			screen.getByRole("button", { name: "Select Approve output" }),
		);
		...
		expect(
			screen.getByText(
				"detail-disabled:No actions are available for this node.",
			),
		).toBeInTheDocument();
	});
	...
	it("renders fixture-backed loading, planning, execution, blocked, failed, completed, and retry states", () => {
		...
		for (const state of states) {
			...
			mocks.planGenerationStatus =
				state === "loading" || state === "planning" ? "generating" : "accepted";
			mocks.plan = {
				id: `plan-${state}`,
				status:
					state === "loading" || state === "planning" ? "draft" : "accepted",
			};
			mocks.graphPlan = fixture.graphPlan;
			...
			expect(
				screen.getByText(`task:${fixture.pageData.task.title}`),
			).toBeInTheDocument();
			expect(screen.getByText("nodes:1")).toBeInTheDocument();
			expect(
				screen.getByText(
					`detail-title:${fixture.graphPlan.nodes[0]?.title ?? "No node selected"}`,
				),
			).toBeInTheDocument();
			expect(screen.getByLabelText("Execution flow")).toBeInTheDocument();
			expect(screen.getByLabelText("Current node details")).toBeInTheDocument();
