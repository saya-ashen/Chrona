...
import { compiledPlanToGraphPlan, taskPlanReadModelToGraphPlan } from "./task-plan-view-model";
import type { CompiledPlan, TaskPlanReadModel } from "@chrona/contracts"
...
describe("task-plan-view-model", () => {
  ...
  it("uses effective plan runtime status and preserves inactive labeled edges from read model", () => {
    ...
    expect(graphPlan?.steps.find((step) => step.id === "condition-1")?.resultEvidence).toBeNull();
  ...
  it("surfaces manual-action waiting results with blocked reasons as blocked nodes", () => {
    ...
    expect(graphPlan?.steps[0]).toMatchObject({
      ...
      result: { error: { present: true, message: "Node execution failed." }, waitKind: "manual_action" },
    });
