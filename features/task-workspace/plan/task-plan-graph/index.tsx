...
import { AUTO_FULL_MODE_MIN_WIDTH, DEFAULT_GRAPH_COPY } from "./constants";
...
function ensureGraphAnalytics(plan: TaskPlanGraphPlan): TaskPlanGraphPlan {
	...
	const analytics: PlanGraphAnalytics = {
		...
		attentionNodeIds: plan.nodes
			...
			.map((node) => node.id),
		blockedNodeIds: plan.nodes
			.filter((node) => node.status === "blocked")
			.map((node) => node.id),
		rankByNodeId: Object.fromEntries(
			plan.nodes.map((node, index) => [node.id, index]),
		),
		laneByNodeId: Object.fromEntries(
			plan.nodes.map((node, index) => [node.id, index]),
		),
...
function GraphShell({
	...
	const currentNode =
		planNodes.find((node) => node.id === currentStepId) ??
		planNodes.find(
			(node) => node.status === "active" || node.status === "in_progress",
		) ??
		null;
...
export function TaskPlanGraph({
	...
	const graphCopyRef = useRef<{ signature: string; value: GraphCopy } | null>(
		null,
	);
	if (
		!graphCopyRef.current ||
		graphCopyRef.current.signature !== graphCopySignature
	) {
		graphCopyRef.current = {
	...
	const adjustScroll = useCallback((leftRatio: number, topRatio: number) => {
		const scroll = graphRef.current?.querySelector<HTMLElement>(
			"[data-testid='task-plan-graph-scroll']",
		);
		if (!scroll) return;
		scroll.scrollLeft = Math.max(
			0,
			(scroll.scrollWidth - scroll.clientWidth) * leftRatio,
		);
		scroll.scrollTop = Math.max(
			0,
			(scroll.scrollHeight - scroll.clientHeight) * topRatio,
		);
	}, []);

	const handleZoomIn = useCallback(
		() => adjustScroll(0.5, 0.45),
		[adjustScroll],
	);
	const handleZoomOut = useCallback(() => adjustScroll(0, 0), [adjustScroll]);
	const handleFitGraph = useCallback(
		() => adjustScroll(0.5, 0.5),
		[adjustScroll],
	);
	const handleCenterCurrentNode = useCallback(
		() => adjustScroll(0.5, 0.35),
		[adjustScroll],
	);

	const { edgeLegend, nodeLegend } = useGraphLegend(graphCopy);
	const compact = useMemo(
		() => buildCompactViewModel(graphPlan, graphCopy),
		[graphCopy, graphPlan],
	);
	...
	const handleNodeSelect = useCallback(
		(nodeId: string) => {
			...
			onSelectedNodeChange?.(
				graphPlan.nodes.find((planNode) => planNode.id === nodeId) ?? null,
				graphPlan.nodes,
			);
		},
	...
	useEffect(() => {
		...
		if (
			selectedNodeId &&
			!graphPlan.nodes.some((node) => node.id === selectedNodeId)
		) {
			setSelectedNodeId(null);
		}
					...
					<DialogClose
						render={
							<button
								...
								className="flex size-9 items-center justify-center rounded-full border border-border bg-background text-muted-foreground transition hover:bg-muted hover:text-foreground"
							/>
						}
					>
						<X className="size-4" />
