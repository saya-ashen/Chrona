...
export function buildCompactViewModel(
	...
	const upstreamByNodeId: Record<string, string[]> = Object.fromEntries(
		plan.nodes.map((node) => [
			node.id,
			[...(plan.analytics.upstreamByNodeId[node.id] ?? [])],
		]),
	);
	const downstreamByNodeId: Record<string, string[]> = Object.fromEntries(
		plan.nodes.map((node) => [
			node.id,
			[...(plan.analytics.downstreamByNodeId[node.id] ?? [])],
		]),
	);
	...
	const stages = [...stageMap.entries()]
		...
		.map(([rank, ids]) => {
			...
			const stageEdges = plan.edges.flatMap((edge) => {
				const from = edge.from ?? edge.fromNodeId;
				const to = edge.to ?? edge.toNodeId;
				return from && to && stageNodeIds.has(from) && nodesById.has(to)
					? [{ id: edge.id, from, to }]
					: [];
			});
			return {
				...
				title:
					rank === 0
						? graphCopy.compactEntryLabel
						: `${graphCopy.compactStageLabel} ${rank + 1}`,
				nodes: stageNodes,
				edges: stageEdges,
				activeCount: members.filter((node) => node?.status === "active").length,
				attentionCount: members.filter(
					(node) =>
						node?.status === "waiting" ||
	...
	return {
		...
		summary: {
			...
			done: plan.nodes.filter(
				(node) => node.status === "done" || node.status === "skipped",
			).length,
			currentLabel: plan.currentStepId
				? (nodesById.get(plan.currentStepId)?.title ?? null)
				: null,
			statusLabel: plan.currentStepId
				? (nodesById.get(plan.currentStepId)?.statusLabel ??
					nodesById.get(plan.currentStepId)?.status ??
					null)
				: null,
		},
	};
...
function minimapNodeClass(tone: CompactStage["nodes"][number]["tone"]) {
	...
	if (tone === "attention" || tone === "upcoming")
...
export function CompactStageStrip({
	...
	const positionedStages = stages.map((stage, stageIndex) => {
		const lanes = [...new Set(stage.nodes.map((node) => node.lane))].sort(
		...
		return {
			...
			nodes: stage.nodes.map((node) => ({
				...
				y:
					MINI_PADDING_Y +
					(laneIndexByLane.get(node.lane) ?? 0) * MINI_LANE_GAP,
			})),
		};
	});
	const nodesById = new Map(
		positionedStages.flatMap((stage) =>
			stage.nodes.map((node) => [node.id, node]),
		),
	);
	const stageCount = Math.max(positionedStages.length, 1);
	const laneCount = Math.max(
		...positionedStages.map(
			(stage) => new Set(stage.nodes.map((node) => node.lane)).size,
		),
		1,
	);
	const width =
		MINI_PADDING_X * 2 +
		Math.max(stageCount - 1, 0) * MINI_STAGE_GAP +
		MINI_NODE_RADIUS * 2;
	const height =
		MINI_PADDING_Y * 2 +
		Math.max(laneCount - 1, 0) * MINI_LANE_GAP +
		MINI_NODE_RADIUS * 2;

	return (
		<div className="space-y-2" data-testid="task-plan-compact-line">
			<p className="px-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-primary">
				{graphCopy.graphStageMap}
			</p>
			<div
				className="min-w-0 overflow-x-auto"
				tabIndex={0}
				role="region"
				aria-label={graphCopy.graphStageMap}
			>
				<svg
						...
						stage.nodes.map((node) => (
							<g key={node.id}>
								...
								<circle
									...
									cy={node.y}
									r={MINI_NODE_RADIUS}
									className={cn("stroke-[1.5]", minimapNodeClass(node.tone))}
								>
				...
				return (
					<button
						...
						className={cn(
							...
							item.isDone
								? "border-border/70 bg-muted/35 opacity-75 shadow-none"
								: [toneStyle.border, toneStyle.bg, "shadow-sm"],
							item.isCurrent &&
								!item.isDone &&
								"border-primary/60 bg-primary-soft/80 shadow-md ring-1 ring-primary/25",
					...
					>
						...
						>
							...
							{item.isCurrent ? (
								<span className="text-primary">
									{graphCopy.compactCurrentNode}
								</span>
							) : null}
							{item.displayTone === "waiting" ? (
								<span className="text-primary">
									{graphCopy.compactNeedsAction}
								</span>
							) : null}
							{item.hasLinkedTask ? (
								<span className="text-muted-foreground">
									{graphCopy.compactLinkedTask}
								</span>
							) : null}
						</div>
						<p
						...
						<p
							className={cn(
								"mt-1 break-words text-xs leading-5 line-clamp-3",
								item.isDone
