...
type WorkspaceRuntimeEvent = Extract<
...
function runningDescription(input: ResolveTaskWorkspaceOperationStateInput) {
	...
	switch (latestRuntime.type) {
		case "text_delta":
			return latestRuntime.text || "Assistant output";
		case "reasoning_delta":
			return latestRuntime.text || "Provider reasoning";
		case "raw_event":
			return latestRuntime.rawEventType ?? "Provider event";
		case "tool_started":
		case "tool_progress":
