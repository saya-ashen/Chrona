...
export const taskWorkspaceQueryKeys = {
	...
	pagePrefix: (taskId: string) =>
		[...taskWorkspaceQueryKeys.all, "page", taskId] as const,
...
function buildTaskHeaderView(
	...
	const headerActions = actions.flatMap((action) =>
		action.id === "more"
			? [
					...(actions.some((candidate) => candidate.id === "pause")
						? []
						: [
								{
									id: "pause" as const,
									label: copy.pause,
									disabled: true,
									disabledReason: copy.noRunningExecutionToPause,
								},
							]),
					...(actions.some((candidate) => candidate.id === "stop")
						? []
						: [
								{
									id: "stop" as const,
									label: copy.stop,
									disabled: true,
									disabledReason: copy.noRunningExecutionToStop,
								},
							]),
					action,
				]
...
export async function dispatchTaskExecutionAction(
	...
	const scopedAction = workBlockId
		? { ...action, workBlockId, idempotencyKey }
		: { ...action, idempotencyKey };
	const ack = await apiJson<Omit<TaskWorkspaceCommandAck, "message">>(
		`/api/work/${encodeURIComponent(taskId)}/commands`,
		{
			method: "POST",
			body: JSON.stringify({ type: "execution.action", ...scopedAction }),
		},
	);
	return {
		...ack,
...
export async function retryTaskResultFinalization(taskId: string) {
	return apiJson<{
		...
		finalization: NonNullable<
			PublicPlanExecutionResult["planOutput"]
		>["finalization"];
		finalizedResult: NonNullable<
			PublicPlanExecutionResult["planOutput"]
		>["finalizedResult"];
