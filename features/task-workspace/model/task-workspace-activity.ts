...
function workspaceEventTimestamp(event: TaskWorkspaceSseEvent) {
	return (
		eventStringValue(event, "occurredAt") ??
		eventStringValue(event, "timestamp") ??
		undefined
	);
}

function workspaceEventTimestampWithFallback(
...
function isPlanGenerationProjectionUpdate(event: TaskWorkspaceSseEvent) {
	if (
		...
		return false;
	return (
		eventStringValue(event, "reason")?.startsWith("plan_generation.") ?? false
	);
}
...
export function workspaceEventToWorkspaceActivity(
	event: TaskWorkspaceSseEvent,
	index = 0,
	fallbackTimestamp?: string,
	...
	if (event.type === "command.accepted" || event.type === "command.failed") {
		const commandType =
			typeof event.commandType === "string" ? event.commandType : "command";
		const failed = event.type === "command.failed";
		const description =
			eventMessageValue(event) ??
			(failed ? `${commandType} failed.` : `${commandType} accepted.`);
		return {
...
function canReplaceToolProgress(
	left: WorkspaceActivityItem,
	right: WorkspaceActivityItem,
) {
	return (
		left.kind === "tool_progress" &&
...
export function runtimeEventToWorkspaceActivity(
	...
	if (value.type === "raw_event") {
		return {
			...base,
			kind: "provider_run",
			title: value.rawEventType
				? `Provider event: ${value.rawEventType}`
				: "Provider event",
...
export function runtimeEventsToWorkspaceActivity(
	events: WorkspaceRuntimeEvent[],
	limit = DEFAULT_LIMIT,
) {
	return mergeWorkspaceActivity(
		events
			.map((event, index) => runtimeEventToWorkspaceActivity(event, index))
