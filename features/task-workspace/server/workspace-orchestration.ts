...
export async function buildTaskWorkspaceStateSnapshot(
	...
	return {
		...
		"/plan/generation/error/title": session?.error?.title ?? session?.error?.code ?? null,
	...
	switch (event.type) {
		...
		case "stale":
			return {
				...
				"/plan/generation/error/title": "Plan changed during generation",
		...
		case "failed":
			return {
				...
				"/plan/generation/status": "failed",
				"/plan/generation/error/code": event.code,
				"/plan/generation/error/title": event.title ?? event.code,
				"/plan/generation/error/message": event.message,
				"/plan/generation/is-running": false,
				"/plan/generation/header-action-disabled": false,
			};
		...
		if (command.type === "plan.generate") {
			...
			publishTaskStateUpdate({
				...
				updates: {
					...
					"/plan/generation/error/title": null,
