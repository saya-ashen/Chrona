...
function patchState(
	key: string,
	updater: (state: TaskPlanSessionState) => TaskPlanSessionState,
) {
...
function isGenerationPhase(value: unknown): value is TaskPlanSessionState["phase"] {
	return (
		...
		value === "streaming" ||
		value === "loading_task" ||
		value === "requesting_provider"
	);
}

function applyStateSnapshotToSession(
	key: string,
	snapshot: Record<string, unknown>,
) {
	const get = (path: string): unknown =>
		path in snapshot ? snapshot[path] : getByPath(snapshot, path);
	const planStatus = get("/plan/status") as
		| AiPlanGenerationStatus
		| null
		| undefined;
	const generationStatusValue = get("/plan/generation/status");
	const generationStatus = isSessionStatus(generationStatusValue)
		? generationStatusValue
		: undefined;
	const generationIdValue = get("/plan/generation/id");
	const generationId =
		typeof generationIdValue === "string" || generationIdValue === null
			? generationIdValue
			: undefined;
	const headStateVersionValue = get("/plan/generation/head-state-version");
	const headStateVersion =
		typeof headStateVersionValue === "number" || headStateVersionValue === null
			? headStateVersionValue
			: undefined;
	const phaseValue = get("/plan/generation/phase");
	const phase = isGenerationPhase(phaseValue) ? phaseValue : undefined;
	const statusMessage = get("/plan/generation/statusMessage") as
		| string
		| null
		| undefined;
	const errorMessage = get("/plan/generation/error/message") as
		| string
		| null
		| undefined;
	const errorCode = get("/plan/generation/error/code") as
		| GeneratePlanErrorCode
		| null
		| undefined;
	...
	patchState(key, (state) => {
		...
		return {
			...state,
			generationId:
				generationId === undefined ? state.generationId : generationId,
			headStateVersion:
				headStateVersion === undefined
					? state.headStateVersion
					: headStateVersion,
			sessionStatus,
			...
			phase:
				phase == null
					...
					: phase,
			statusMessage:
				statusMessage === undefined ? state.statusMessage : statusMessage,
...
export async function startTaskPlanGenerationSession(input: {
	...
	try {
		...
		patchState(key, (state) => ({
			...
			connected: false,
			error:
				error instanceof Error ? error.message : "Failed to generate task plan",
		}));
		throw error;
	}
}

export async function stopTaskPlanGenerationSession(
	taskId: string,
	workBlockId?: string | null,
) {
	const key = sessionKey(taskId, workBlockId);
...
export function useTaskPlanGenerationSession(
	taskId?: string,
	workBlockIdOrOptions?: string | null | { hydrate?: boolean },
	maybeOptions?: { hydrate?: boolean },
) {
	const workBlockId =
		typeof workBlockIdOrOptions === "string" ? workBlockIdOrOptions : null;
	const options =
		typeof workBlockIdOrOptions === "object"
			? workBlockIdOrOptions
			: maybeOptions;
	...
	useEffect(() => {
		...
		patchState(key, (state) =>
			state.hydrated ? state : { ...state, hydrated: true },
		);
	}, [key, options?.hydrate]);
