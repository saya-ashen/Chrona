...
import {
  ...
  findPrimaryResultContext,
  formatDate,
} from "./goal-workspace-shared";
...
function outcomeEvidence(
  ...
  return confirmation.evidenceArtifactIds
    .map((id) => {
...
function primaryOutcomeView(
  ...
  return {
    ...
    sourceTask,
    summary,
    versionLabel: context?.asset?.currentVersion
...
export function PrimaryOutcome({
  ...
  return (
    <article className="w-full" aria-labelledby="goal-final-outcome">
...
function OutcomeConfirmation({
  ...
  confirmation: NonNullable<GoalData["outcome"]["confirmation"]>;
  copy: GoalCopy;
  locale: GoalLocale;
}) {
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <h3 id="goal-achievement-confirmation" className="mt-1 font-semibold">
...
function OutcomeSidebar({
  ...
  const { primary, confirmation, context, sourceTask, evidence, versionLabel } =
    view;
  return (
    <aside
    ...
    >
      <EvidenceList
        copy={copy}
        evidence={evidence}
        evidenceCount={confirmation?.evidenceArtifactIds.length ?? 0}
      />
      {primary ? (
        <section
  ...
  return (
    <section aria-labelledby="goal-supporting-evidence">
      <div className="flex items-center justify-between border-b pb-2">
        <h3 id="goal-supporting-evidence" className="text-sm font-semibold">
      ...
      {evidence.length ? (
        ...
        <p className="py-3 text-sm leading-6 text-muted-foreground">
          {copy.evidenceCount.replace("{count}", String(evidenceCount))}
        </p>
      )}
    </section>
  );
...
function OutcomeTechnicalDetails({
  copy,
  primary,
  sourceTask,
  versionLabel,
  ...
  copy: GoalCopy;
  primary: NonNullable<GoalData["outcome"]["primaryResult"]>;
  sourceTask: GoalData["tasks"][number] | undefined;
  versionLabel: string;
}) {
  return (
    <details className="group border-t pt-4">
      ...
      <dl className="mt-4 space-y-3 text-sm">
        <OutcomeTechnicalDetail
          label={copy.sourceTask}
          value={sourceTask?.title ?? primary.taskId}
        />
        <OutcomeTechnicalDetail
          label={copy.sourceRun}
          value={primary.runId}
          mono
        />
        <OutcomeTechnicalDetail
...
function OutcomeTechnicalDetail({
  label,
  value,
  mono = false,
}: {
  ...
  return (
    <div>
      ...
      <dd
        className={`mt-1 font-medium ${mono ? "break-all font-mono text-xs" : ""}`}
      >
        {value}
      </dd>
    </div>
