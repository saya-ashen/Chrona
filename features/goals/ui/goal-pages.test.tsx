...
const {
	...
	generateGoalReviewMock: vi.fn(async () => ({
...
describe("Goal pages", () => {
	...
	it("creates a Goal with a library-generated idempotency key", async () => {
		const { router } = renderInRouter(<GoalListPage goals={[]} copy={copy} />);
		...
		await waitFor(() =>
			expect(router.state.location.pathname).toBe("/en/goals/goal-created"),
		);
		expect(router.state.location.search).toBe("?review=initial");
	});
	...
	it("[GOAL-022] opens the matching task inspector from the continue-work action", async () => {
		const goal: GoalData = {
			...baseGoal,
			primaryAction: { kind: "continue_work", taskId: "task-focus" },
			projection: { ...baseGoal.projection, nextAction: "continue_work" },
		};
		const { router } = renderInRouter(
		...
		await waitFor(() =>
			expect(router.state.location.pathname).toBe(
				"/en/goals/goal-1/workbench/tasks/task-focus",
			),
		);
	});
	...
	it("opens the initial-plan deep link and starts a v2 review operation", async () => {
		...
		const { router } = renderInRouter(
			<GoalWorkspacePage goal={baseGoal} copy={copy} />,
			"/en/goals/goal-1?review=initial",
		);
		expect(
			screen.getByRole("heading", { name: "Review AI initial plan" }),
		).toBeInTheDocument();
		fireEvent.click(screen.getByRole("button", { name: "Generate AI review" }));
		await waitFor(() =>
			expect(generateGoalReviewMock).toHaveBeenCalledWith(
				"goal-1",
				{
					idempotencyKey: "goal-idempotency-key",
					mode: "initial",
	...
	it("uses the safe retry action for a failed v2 review without rendering the runtime failure", async () => {
			...
			reviewProposals: [
				{
					...
					modelName: null,
					summary: null,
					generationError: "provider internals",
					questions: [],
					partialOutput: null,
		...
		await waitFor(() =>
			expect(retryGoalReviewMock).toHaveBeenCalledWith(
				"goal-1",
				"proposal-failed",
				{ expectedVersion: 3 },
			),
	...
	it("collects runtime questions as bounded JSON answers", async () => {
		...
		fireEvent.change(screen.getByRole("textbox"), {
			target: { value: "Hiring managers" },
		});
		fireEvent.click(screen.getByRole("button", { name: "Continue review" }));
		await waitFor(() =>
			expect(answerGoalReviewMock).toHaveBeenCalledWith(
				"goal-1",
				"proposal-input",
				{
					expectedVersion: 4,
	...
	it("renders concise review cards with per-item and bulk decisions", async () => {
					...
					items: [
							...
							evidenceRefs: [
								{
									...
									label: "Actionable opportunity table",
								},
							],
						...
						},
						{
							...
							payload: {
								...
								description:
									"Confirm that the supervisor can admit candidates.",
								suggestedTask: {
							...
							rationale: "Evidence is needed before confirming the criterion.",
							evidenceRefs: [],
							warnings: [],
		...
		expect(
			screen.getByText("Update Current focus", { selector: "h3" }),
		).toBeInTheDocument();
		expect(
			screen.getByText("Close evidence gap: Verify the admissions path", {
				selector: "h3",
			}),
		).toBeInTheDocument();
		expect(screen.getByText("Verify the next outcome")).toBeInTheDocument();
		expect(
			within(screen.getByRole("dialog")).getByText("Confirm next bounded step"),
		).toBeInTheDocument();
		const rationaleSummary = screen.getByText(
			"Why this suggestion · 1 evidence reference(s) · 1 warning(s)",
		);
		...
		expect(
			screen.getByRole("button", {
				name: "Create suggested task: Verify the admissions path",
			}),
		).toBeEnabled();

		fireEvent.click(
			screen.getByRole("button", { name: "Apply this item: Current focus" }),
		);
		await waitFor(() =>
			expect(applyGoalReviewProposalMock).toHaveBeenCalledWith(
				"goal-1",
				"proposal-1",
				expect.objectContaining({
					...
					dependencyHashes: {
						"current-focus": "sha256:dependency",
						"evidence-gap": "sha256:evidence-gap",
					},
		...
		fireEvent.click(
			screen.getByRole("button", {
				name: "Reject this item: Verify the admissions path",
			}),
		);
		await waitFor(() =>
			expect(applyGoalReviewProposalMock).toHaveBeenLastCalledWith(
				"goal-1",
				"proposal-1",
				expect.objectContaining({
	...
	it("applies or rejects every pending review item without selection", async () => {
		...
		const first = renderInRouter(
			<GoalWorkspacePage goal={proposalGoal} copy={copy} />,
		);
		fireEvent.click(screen.getByRole("button", { name: "Start Goal review" }));
		...
		await waitFor(() =>
			expect(rejectGoalReviewProposalMock).toHaveBeenCalledWith(
				"goal-1",
				"proposal-bulk",
				{
					idempotencyKey: "goal-idempotency-key",
...
describe("Goal Review answer parsing", () => {
	it("preserves scalar, array, and object answer semantics", () => {
		expect(parseGoalReviewAnswer("Hiring managers", { type: "string" })).toBe(
			"Hiring managers",
		);
		expect(parseGoalReviewAnswer("true", { type: "boolean" })).toBe(true);
		expect(
			parseGoalReviewAnswer('["quality","speed"]', {
				type: "array",
				items: { type: "string" },
			}),
		).toEqual(["quality", "speed"]);
		expect(
			parseGoalReviewAnswer('{"audience":"Hiring managers"}', {
				type: "object",
			}),
		).toEqual({ audience: "Hiring managers" });
		expect(
			parseGoalReviewAnswer("not-json", { type: "array" }),
		).toBeUndefined();
	});
});
