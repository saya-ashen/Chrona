...
describe("Goal pages", () => {
  ...
  it("renders an itemized AI review proposal with explicit per-item decisions", () => {
    ...
    expect(
      screen.getByText("One focused change is ready for review."),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Accepted evidence supports a narrower next step."),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: "Operational brief: currentFocus" }),
    ).toBeChecked();
    expect(
      screen.getByRole("button", { name: "Reject proposal" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Apply review" }),
    ).toBeInTheDocument();
  });
