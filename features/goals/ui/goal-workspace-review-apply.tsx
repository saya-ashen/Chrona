...
function ReviewBody({ proposal, copy, pending, selected, setSelected, generate }: { proposal: Proposal; copy: GoalCopy; pending: boolean; selected: Record<string, boolean>; setSelected: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; generate: () => void }) {
  ...
  if (proposal.status === "Generating") return <Card><CardContent className="flex items-center gap-3 p-5 text-sm"><RefreshCw className="size-4 animate-spin" /><div><p className="font-medium">{copy.generatingReview}</p><p className="text-muted-foreground">{copy.proposalSource} · AI</p></div></CardContent></Card>;
