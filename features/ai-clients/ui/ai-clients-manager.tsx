...
export function AiClientsManager() {
  ...
  const fetchClients = useCallback(async () => {
    const [clientsData, providersData] = await Promise.all([aiClientsApi.list(), aiClientsApi.listRuntimeProviders()]);
    const available = normalizeRuntimeProviders(providersData).filter((provider) => provider.key !== "debug" || isDebugProviderVisible());
    setProviders(available);
    setClients((clientsData.clients ?? []).filter((client) => available.some((provider) => provider.key === client.type)));
    setLoading(false);
  }, []);
  ...
  const inspect = async (id: string) => {
    ...
    try {
      ...
      if (runtimeDiagnostics) setDiagnostics((current) => ({ ...current, [id]: runtimeDiagnostics }));
    } finally {
      setDiagnosticsLoading((current) => ({ ...current, [id]: false }));
    }
  };

  if (loading) return <div className="flex flex-col gap-3 p-1" aria-label={copy.loading}><Skeleton className="h-6 w-44" /><Skeleton className="h-24 w-full" /></div>;
...
function EmptyState({ copy, onAdd }: { copy: Record<string, string>; onAdd: () => void }) {
  return <Card className="border-dashed"><CardContent className="flex flex-col items-center gap-3 py-8 text-center text-sm text-muted-foreground"><p className="max-w-md">{copy.emptyState}</p><Button type="button" onClick={onAdd}>{copy.emptyStateCta}</Button></CardContent></Card>;
}
