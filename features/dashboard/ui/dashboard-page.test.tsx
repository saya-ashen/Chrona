...
vi.mock("@shared/ui", () => ({
	...
	UiSurfaceFrame: ({
		...
		<section aria-label={label} data-ui-surface-kind={kind} {...props}>
			{children}
			<span>{kind === "ai-authored" ? "AI generated" : ""}</span>
		</section>
	),
...
vi.mock("@features/task-workspace", () => ({
	SpecRenderer: ({ spec }: { spec: unknown }) => (
		<div>{spec ? "AI brief spec rendered" : null}</div>
	),
}));
...
const COPY = {
	...
	headline: {
		...
		completedOnly:
			"Today Chrona auto-completed {completed} {completedTaskLabel}. No task needs you right now.",
	...
	attention: {
		...
		description: "desc",
		empty:
			"All clear — no approvals, input requests, blockers, failed runs, or schedule risks need you.",
		kind: {
	...
	inProgress: {
		...
		description: "desc",
		empty:
			"Nothing is running. Start a task from the schedule or create a new one when you want Chrona to move.",
	},
	completed: {
		title: "Recent completions",
		totalLabel: "{n} recent completions shown",
		empty:
			"Completed task outputs will collect here once Chrona finishes work.",
	},
	digest: {
		title: "AI summary",
		...
		empty:
			"AI summary will appear here after Chrona generates it from dashboard facts.",
		outputLabel: "Output",
		category: {
...
} satisfies Messages["pages"]["dashboard"];
...
function completed(
	overrides: Partial<DashboardData["autoCompleted"][number]> = {},
): DashboardData["autoCompleted"][number] {
	return {
		taskId: "c1",
...
function renderDashboard(data: Partial<DashboardData> = {}) {
	return render(
		<DashboardPage
			data={makeData(data)}
			copy={COPY}
...
describe("DashboardPage", () => {
	it("[DASH-001] shows explicit empty states across an empty workspace", () => {
		renderDashboard();

		expect(
			screen.getByText(
				"Chrona is ready. Add a task, start work, or review recent activity here.",
			),
		).toBeInTheDocument();
		expect(
			screen.getByText(
		...
		expect(screen.getByText("No more today")).toBeInTheDocument();
		expect(
			screen.getByText(
				"Completed task outputs will collect here once Chrona finishes work.",
			),
		).toBeInTheDocument();
		expect(screen.getByText("0 recent completions shown")).toBeInTheDocument();
	});

	it("[DASH-005] shows upcoming task state, time, and task link", () => {
		renderDashboard({
			upcomingToday: [
				{
					taskId: "upcoming-1",
					title: "Prepare launch",
					stateView: stateView("Scheduled"),
					priority: "High",
					scheduleStatus: "Scheduled",
					scheduledStartAt: "2026-08-16T09:30:00.000Z",
					scheduledEndAt: "2026-08-16T10:30:00.000Z",
					dueAt: null,
					reason: null,
					stage: null,
					latestOutput: null,
		...
		expect(screen.getByText(/\d{1,2}:30/)).toBeInTheDocument();
	...
	it("renders the natural-language headline with both completed-today and attention counts", () => {
		...
		expect(
			screen.getByText(
				"Today Chrona auto-completed 2 tasks, and 1 task needs you.",
			),
		).toBeTruthy();
	});
	...
	it("shows running task state, stage, and task link in In Progress", () => {
			...
			inProgress: [
				{
					...
					stateView: stateView("running", "running"),
		...
		expect(screen.getByText("Build release notes")).toBeInTheDocument();
		expect(
			screen.getByText(/Running.*Collecting evidence/),
		).toBeInTheDocument();
		expect(screen.getByRole("link", { name: "Open" })).toHaveAttribute(
			"href",
			"/en/tasks/running-1",
	...
	it("shows recent completion output links", () => {
		renderDashboard({
			autoCompleted: [
				completed({
					taskId: "recent-1",
		...
		expect(
			screen.getByRole("link", { name: /research\.mdreport/i }),
		).toHaveAttribute("href", "/en/tasks/recent-1");
	});
	...
	it("shows the AI summary placeholder until a generated spec exists", () => {
		...
		expect(
			screen.queryByText("2 tasks auto-completed today"),
		).not.toBeInTheDocument();
		expect(screen.queryByText("Reports generated")).not.toBeInTheDocument();
		expect(
			screen.queryByRole("region", { name: "AI summary" }),
		).not.toBeInTheDocument();
	...
	it("labels rendered AI summary specs as AI generated", () => {
		...
		expect(screen.getByRole("region", { name: "AI summary" })).toHaveAttribute(
	...
	it("does not auto-request generation when dirty surface cannot generate", async () => {
		renderDashboard(
			makeData({ aiBrief: { ...dirtyAiBrief(), canGenerate: false } }),
		);
