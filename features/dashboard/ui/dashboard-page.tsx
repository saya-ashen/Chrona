					...
					<div className="min-w-0 space-y-2">
						...
						{aiBrief.generatedAt ? (
							<p className="text-xs text-muted-foreground">
								{copy.digest.aiBrief.updatedAt.replace(
									"{time}",
									formatRelative(aiBrief.generatedAt, copy.time),
								)}
							</p>
						) : null}
					</div>
							...
							>
								<div className="min-w-0">
									...
									<p className="truncate text-xs text-muted-foreground">
										{item.scheduleStatus ?? item.stateView.label}
									</p>
								</div>
				...
				{visibleEvents.length === 0 ? (
					...
					<ol className="divide-y border-t">
						{visibleEvents.map((event) => (
							<li
								key={event.id}
								className="grid gap-1 py-3 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-start sm:gap-3"
							>
	...
	return (
		<PageFrame mode="main" data-domain="dashboard" className="p-1 sm:p-2">
			<div className="w-full space-y-4 sm:space-y-5">
				...
				<div
					className={`grid grid-cols-1 gap-4 ${data.aiBrief?.status !== "disabled" ? "xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]" : ""}`}
				>
