...
export function TaskWorkspaceInspector({
	...
	commandCenter: {
		documents: { now: UiDocument; output: UiDocument; trail: UiDocument };
	} | null;
	...
	const hasCheckpointForm =
		currentExecution?.status === "waiting_for_user" && Boolean(operationPanel);
		...
		<aside
			className={
				showHeader
					...
					: "relative flex h-full min-h-0 flex-1 flex-col bg-transparent"
			}
					...
					<div className="min-w-0">
						...
						{consoleView.progress.totalSteps > 0 ? (
							<p className="mt-0.5 text-xs text-muted-foreground">
								{consoleView.progress.completedSteps}/
								{consoleView.progress.totalSteps} steps
							</p>
						) : null}
					</div>
				</div>
