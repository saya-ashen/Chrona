...
describe("TaskWorkspaceExecutionOverview", () => {
	...
	it("keeps approval waits in stage results instead of final result", () => {
		...
		expect(
			screen.getByRole("heading", { name: "Stage results" }),
		).toBeInTheDocument();
		expect(
			screen.queryByRole("heading", { name: "Final result" }),
		).not.toBeInTheDocument();
		expect(
			screen.queryByRole("status", { name: "Execution is producing output" }),
		).not.toBeInTheDocument();
	...
	it("shows a persisted node error even when Activity has no danger event", () => {
		...
		expect(screen.getByRole("alert")).toHaveTextContent(
			"Find matching PhD roles failed.",
		);
			...
			activity: [
				{
					...
					description:
						"The read command used an invalid selector and execution recovered.",
					tone: "danger",
	...
	it("shows finalization failure with retry instead of validated-result labeling", async () => {
		...
		expect(
			screen.getByRole("heading", { name: "Final result unavailable" }),
		).toBeInTheDocument();
	...
	it("keeps completed transcript prominent without competing with the result", async () => {
		...
		const transcriptButton = screen.getByRole("button", {
			name: /Open Agent transcript/,
		});
		expect(transcriptButton).toHaveAccessibleName(
			new RegExp(`${view.activity.length} events`),
		);
		expect(transcriptButton).toHaveClass("fixed", "right-4", "top-20");
		await user.click(transcriptButton);
		expect(
			screen.getByRole("dialog", { name: /Agent transcript/ }),
		).toBeInTheDocument();
	...
	it("keeps running status out of Results while activity stream stays live", () => {
		...
		expect(
			within(results).getByRole("region", { name: "Live execution activity" }),
		).toBeInTheDocument();
		expect(
			screen.queryByText("Generating report sections"),
		).not.toBeInTheDocument();
	});

	it("renders safe live tool lifecycle activity", async () => {
		const view = createTaskWorkspaceExecutionConsoleView(
			executionMonitoringWorkspaceFixtures.running,
		);
		renderOverview(view, {
		...
		expect(within(liveFeed).getByText("Running")).toBeInTheDocument();
		expect(
			screen.queryByRole("tab", { name: "Activity" }),
		).not.toBeInTheDocument();
		expect(
	...
	it("pauses live-feed auto-follow while the user reads older events", async () => {
		...
		const feed = screen
			.getByTestId("live-execution-feed")
			.querySelector('[aria-label="Execution events"]') as HTMLDivElement;
		...
		await userEvent
			.setup()
			.click(screen.getByRole("button", { name: "Return to latest" }));
	...
	it("renders a completed node summary when finalized result is absent", () => {
		...
		).toHaveTextContent("Research complete");
		expect(screen.getByText("Results available")).toBeInTheDocument();
		expect(
			screen.queryByRole("note", { name: "Current step result pending" }),
		).not.toBeInTheDocument();
	});
	...
	it("keeps only unrepresented artifacts in a collapsed secondary section", () => {
		...
		expect(artifactItems[0]?.props).toMatchObject({
			title: "Supporting notes",
		});
		expect(
