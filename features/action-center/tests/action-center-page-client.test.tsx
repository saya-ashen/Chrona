...
describe("ActionCenterPageClient", () => {
	...
	it.each([
		{
			scenario: "[ACTION-004] rejects an approval",
			button: "Reject",
			decision: "reject",
			feedback: undefined,
		},
		{
			scenario: "[ACTION-005] requests changes from an approval",
			button: "Edit and Approve",
			decision: "request_changes",
			feedback: "Needs edits before approval",
		},
	])("$scenario", async ({ button, decision, feedback }) => {
		const user = userEvent.setup();
		renderSingleItem({
			...
			riskLevel: "high",
			sourceTaskTitle: "Review adapter mapping",
			sourceTaskId: "task_approval",
			currentRunLabel: "run_approval",
		...
		expect(
			screen.queryByText("Review the proposed change"),
		).not.toBeInTheDocument();
	});
	...
	it("[ACTION-011] keeps an item visible and reports an action failure", async () => {
		const user = userEvent.setup();
		dispatchExecutionAction.mockRejectedValueOnce(
	...
	it("[ACTION-012] suppresses stale recovery actions when canonical state is completed", () => {
		renderSingleItem(
			{
				...
				actionType: "Blocked",
				riskLevel: "high",
				sourceTaskTitle: "Completed deploy",
				sourceTaskId: "task_completed",
				currentRunLabel: "run_completed",
				summary: "Stale blocked projection.",
				stateView: { state: "completed" },
			},
			{ resume: "Resume" },
		);
		...
		expect(screen.queryByText("Completed deploy")).not.toBeInTheDocument();
	});

	it("[ACTION-013] removes only the item whose action succeeds", async () => {
		const user = userEvent.setup();
		render(
			<ActionCenterPageClient
				workspaceId="ws_1"
				copy={{ ...copy, resume: "Resume" }}
				initialData={
					[
						{
							id: "blocked_first",
							...
							actionType: "Blocked",
							riskLevel: "high",
							sourceTaskTitle: "First blocked task",
							sourceTaskId: "task_first",
							workspaceId: "ws_1",
							currentRunLabel: null,
							detail: "First block",
							summary: "First item",
							consequence: "Execution stays paused.",
						},
						{
							id: "blocked_second",
							kind: "blocked",
							actionType: "Blocked",
							riskLevel: "high",
							sourceTaskTitle: "Second blocked task",
							sourceTaskId: "task_second",
							workspaceId: "ws_1",
							currentRunLabel: null,
							detail: "Second block",
							summary: "Second item",
		...
		await waitFor(() =>
			expect(screen.queryByText("First blocked task")).not.toBeInTheDocument(),
		);
