...
export function AssistantSurfaceDropdown() {
  ...
  const form = useForm<AssistantSurfaceFormValues>({
    defaultValues: { request: assistant.input },
    mode: "onChange",
    values: { request: assistant.input },
  });
  ...
  return (
    <Drawer open={assistant.isOpen} onOpenChange={(open) => { if (!open) assistant.close(); }} direction="top">
      ...
      >
        ...
        <div className="border-b border-border/60 bg-[linear-gradient(135deg,color-mix(in_oklab,var(--muted)_55%,var(--background)),color-mix(in_oklab,var(--primary-soft)_55%,var(--background)))] px-4 py-3 sm:px-5">
        ...
        <div className="grid max-h-[min(72dvh,640px)] gap-3 overflow-y-auto bg-muted/30 p-3 sm:p-4 lg:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)]">
          <section className="space-y-3 rounded-[1.2rem] border border-border/70 bg-card/85 p-3 shadow-sm backdrop-blur" aria-label={t("components.assistantSurface.contextDeck")}>
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="rounded-full border border-border/70 bg-background px-2 py-0.5 text-[11px] font-semibold text-muted-foreground">{surface.status}</span>
            </div>
            ...
            <div className="space-y-2">
              ...
              {surface.summaries.slice(0, 4).map((summary) => (
                <div key={summary.id} className="rounded-2xl border border-border/70 bg-background px-3 py-2">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">{summary.label}</p>
                  <p className="mt-0.5 text-sm font-medium text-foreground">{summary.value}</p>
                </div>
              ))}
            </div>
            {surface.status === "unavailable" ? (
              <p className="rounded-2xl border border-border bg-muted/50 px-3 py-2 text-sm text-muted-foreground">
          ...
          <aside className="flex min-h-[260px] flex-col rounded-[1.2rem] border border-border/70 bg-slate-950 p-3 text-white shadow-sm" aria-label={t("components.assistantSurface.commandDock")}>
              ...
              <section>
                ...
                <div className="mt-2 space-y-2">
                  {surface.recentProposals.length > 0 ? surface.recentProposals.map((proposal) => (
                    <Link key={proposal.id} to={localizeHref(locale, proposal.href)} className="block rounded-2xl border border-primary/40 bg-primary/15 px-3 py-2 text-sm font-semibold text-white hover:bg-primary/25">
                      {proposal.label}
                    </Link>
                  )) : (
            ...
            <form className="mt-3 flex gap-2 border-t border-white/10 pt-3" onSubmit={(event) => void form.handleSubmit(handleSubmit)(event)}>
              <FieldGroup className="min-w-0 flex-1 gap-0">
                <Field data-invalid={Boolean(form.formState.errors.request)}>
                  ...
                  <Input
                    {...form.register("request", { required: true, onChange: (event) => assistant.setInput(event.currentTarget.value) })}
              ...
              <Button type="submit" disabled={!request.trim()}>{t("components.assistantSurface.send")}</Button>
            </form>
          </aside>
        </div>
      </DrawerContent>
