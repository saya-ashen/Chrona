...
export function QueueCard({
  ...
  return (
    <Card
      ...
      onDragEnd={onDragEnd}
      className={cn(
        "group grid min-h-16 cursor-grab grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-xl bg-card/95 px-2.5 py-2 shadow-[0_3px_12px_rgba(15,23,42,0.06)] ring-border/70 transition-[background,border-color,box-shadow,opacity,transform] hover:bg-muted/35 hover:shadow-[0_8px_20px_rgba(15,23,42,0.08)] active:cursor-grabbing data-[size=sm]:py-2",
    ...
    >
      ...
      <Button
        ...
        className="h-auto min-w-0 justify-start rounded-md px-1 py-1 text-left font-normal hover:bg-muted/50"
      >
        <span className="min-w-0">
          <span className="block truncate text-sm font-medium leading-5 text-foreground">{item.title}</span>
