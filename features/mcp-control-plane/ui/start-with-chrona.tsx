...
const AI_CLIENTS_CHANGED_EVENT = "chrona:ai-clients-changed";
...
function stepState(
  ...
  if (stepIndex === 1) return !state.hasClients ? "next" : state.hasCreatedTask ? "done" : "current";
...
export function StartWithChrona({ className = "", createdTaskId = null, workspaceId, isComplete = false, onCreateTask, onCreateSafeDemo, onOpenCreatedTask }: StartWithChronaProps) {
  ...
  useEffect(() => {
    ...
    const refreshClients = () => {
      ...
      apiJson<AiClientsResponse>("/api/ai/clients")
        .then((payload) => {
          if (!cancelled && requestId === aiClientsRequestId.current) setHasClients(hasEnabledClient(payload.clients));
        })
        .catch(() => {
    ...
    refreshClients();
    window.addEventListener(AI_CLIENTS_CHANGED_EVENT, refreshClients);
    return () => {
      cancelled = true;
      window.removeEventListener(AI_CLIENTS_CHANGED_EVENT, refreshClients);
    };
  }, []);
  ...
  if (isComplete || hasClients === null || storedTask === null) return null;

  const steps: OnboardingStep[] = [
    { title: t("components.schedulePage.firstRunStepConnectAiTitle"), description: hasClients ? t("components.schedulePage.firstRunStepConnectAiDone") : t("components.schedulePage.firstRunStepConnectAi"), state: stepState(0, { hasClients, hasCreatedTask, isComplete }) },
    { title: t("components.schedulePage.firstRunStepCreateTaskTitle"), description: t("components.schedulePage.firstRunStepCreateTask"), state: stepState(1, { hasClients, hasCreatedTask, isComplete }) },
    { title: t("components.schedulePage.firstRunStepReviewPlanTitle"), description: t("components.schedulePage.firstRunStepReviewPlan"), state: stepState(2, { hasClients, hasCreatedTask, isComplete }) },
  ];

  const handlePrimaryAction = () => {
    if (!hasClients) {
      void navigate(localizeHref(locale, "/settings?panel=ai-clients"));
    } else if (currentTaskId) {
      if (onOpenCreatedTask) onOpenCreatedTask(currentTaskId);
      else void navigate(localizeHref(locale, `/tasks/${currentTaskId}`));
    } else if (hasCreatedTask) {
      void navigate(localizeHref(locale, "/tasks"));
    } else {
      onCreateTask?.();
    }
  };
    ...
    <section className={`rounded-3xl border border-primary/20 bg-primary-soft/70 p-4 text-sm shadow-sm ${className}`}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          ...
          {!hasCreatedTask && onCreateSafeDemo ? <Button type="button" variant="outline" size="default" onClick={onCreateSafeDemo}>{t("components.schedulePage.firstRunCreateSafeDemo")}</Button> : null}
        </div>
      </div>
    </section>
  );
